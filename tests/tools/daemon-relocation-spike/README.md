...
> `win-unpacked` build so that a **copied `CaPilot.exe`** — run with
...
The daemon is `fork()`ed from the app's own Electron binary (`CaPilot.exe`) with
...
round-trips the nonce, the daemon's main module is the copied `CaPilot.exe`, and
**no** loaded module resolves under `--app-dir`.

## Tiers (defined as data in `tier-file-set.mjs`)

Every tier includes the irreducible core: `CaPilot.exe`, `icudtl.dat`, both V8
...
process's **main module** is the copied `CaPilot.exe`, not the install-dir one.
...
- Whether `TIER_MINIMAL` (no top-level DLLs) boots `CaPilot.exe` as node at all, or
...
ship minimal; if `CaPilot.exe` fails to boot without the non-GPU DLLs, fall back to
