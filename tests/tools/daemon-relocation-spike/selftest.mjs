...
function makeSyntheticInventory() {
  ...
  return {
    appDir: 'C:\\App',
    unpackedRoot,
    hostExe: { name: 'CaPilot.exe', ...f('C:\\App\\CaPilot.exe') },
    runtimeData: [
...
function run() {
  ...
  for (const tier of TIERS) {
    ...
    check(`${tier}: has exe`, dests.includes('CaPilot.exe'))
  ...
  // ── Module-path filter ──────────────────────────────────────────────
  const appDir = 'C:\\Users\\me\\AppData\\Local\\Programs\\capilot'
  const modules = [
    'C:\\Users\\me\\AppData\\Local\\Programs\\capilot\\CaPilot.exe',
    'C:\\Windows\\System32\\kernel32.dll',
    'C:\\Users\\me\\AppData\\Local\\capilot-daemon-host\\CaPilot.exe'
  ]
  const resident = findAppDirResidentModules(modules, appDir)
  check('detects app-dir module', resident.length === 1)
  check('detects the right module', resident[0].toLowerCase().includes('programs\\capilot\\capilot.exe'))
  // Sibling-prefix must NOT match (C:\...\capilot vs C:\...\capilot-daemon-host).
  check(
    'sibling prefix not matched',
    findAppDirResidentModules(['C:\\a\\capilot-daemon-host\\x.dll'], 'C:\\a\\capilot').length === 0
  )
  // Forward/back-slash + case normalization.
  check(
    'slash + case normalized',
    findAppDirResidentModules(['c:/a/ORCA/x.dll'], 'C:\\A\\capilot').length === 1
  )
  check(
    'relocated host has zero app-dir modules',
    findAppDirResidentModules(
      ['C:\\work\\daemon-host\\CaPilot.exe', 'C:\\Windows\\System32\\ntdll.dll'],
      appDir
    ).length === 0
  )
