...
**Windows only.** Proves that a **crash of CaPilot's main process** does not orphan
...
On Windows, when CaPilot's main/renderer process crashed, open terminal PTYs were
...
`capilot-terminal-daemon.exe` (see
...
1. **Launch** the installed `CaPilot.exe` under an isolated `userData` dir
...
8. **Teardown** — close the relaunched app, then kill this run's scoped daemon
   **tree** (re-discovered fresh via `findDaemonProcesses(userData)`, which the
   ...
   process), never installs/uninstalls, and never touches any other CaPilot on the box.
...
node tests/tools/win-crash-survival-e2e/run.mjs --expect survival --exe-path "C:\Users\<you>\AppData\Local\Programs\capilot\CaPilot.exe"
```
...
| `--exe-path <path>`  | Installed `CaPilot.exe` to drive (default: per-user install under `%LOCALAPPDATA%\Programs\CaPilot`) |
...
- The crash kills **only** the real Electron main of the instance this harness
  ...
  name or a scanned pid, so a developer's live CaPilot (a different `userData`, out of
  scope) is untouched.
