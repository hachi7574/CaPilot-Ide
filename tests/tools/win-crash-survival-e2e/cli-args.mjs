...
// installed, packaged CaPilot.exe. So its only source is `--exe-path` (defaulting
...
Proves that force-killing ONLY CaPilot's main process (a real crash, no tree-kill)
...
Usage:
  node tests/tools/win-crash-survival-e2e/run.mjs --expect <profile> [--exe-path <CaPilot.exe>] [options]
...
Options:
  --exe-path <path>        Installed CaPilot.exe to drive (default: the per-user
                           install under %LOCALAPPDATA%\\Programs\\CaPilot). The
...
export function parseArgs(argv) {
  ...
  const opts = {
    ...
    // "no CaPilot.exe found" default-resolution error.
    exePath:
...
function validate(opts, exePathFlagPresent, argv) {
  ...
  if (exePathFlagPresent && takeValue(argv, '--exe-path') === undefined) {
    ...
    errors.push(
      'No installed CaPilot.exe found under %LOCALAPPDATA%\\Programs — pass --exe-path <CaPilot.exe>'
    )
