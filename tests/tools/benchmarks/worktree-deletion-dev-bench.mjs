#!/usr/bin/env node
/**
 * A/B benchmark for worktree deletion against real CaPilot dev instances.
 *
 * Usage:
...
function createFixture(instanceLabel) {
  const root = mkdtempSync(path.join(os.tmpdir(), `capilot-delete-bench-${instanceLabel}-`))
  ...
  run('git', ['config', 'user.email', 'worktree-delete-bench@capilot.invalid'], repoPath)
  run('git', ['config', 'user.name', 'CaPilot Worktree Delete Bench'], repoPath)
  writeFileSync(path.join(repoPath, 'README.md'), '# CaPilot worktree deletion benchmark\n')
...
async function findOrcaPage(browser) {
  ...
  throw new Error('CaPilot renderer with window.__store was not found')
}
