...
1. **Desktop CaPilot running** (`capilot status --json`).
...
capilot worktree list --environment <name> --json | head
capilot terminal list --environment <name> --json | head
```
...
# Restart-proxy: idle + capilot open + refresh storm + open (does not kill desktop)
ORCA_FREEZE_ENV=paired-remote \
...
| `restart-proxy`               | `capilot open` + refresh storm + open (post-restart discovery; no process kill)                  |
| `lockup-storm`                | Idle + flood + reconnect + **concurrent** open fan-out + **mid-storm `capilot status` watchdog** |
...
| **lockup-storm** (parallel open + overlap refresh) | 12–16  | 45–60s | 64–80 @ p20–32 | **27–35s** batches; some `Terminal reveal timed out` | **HARD stalls + reveal timeouts; app still answers `capilot status` ~150ms** |
...
| Post-storm `capilot status`    | **~113ms**                          |
| Force Quit required         | **no**                              |

Bar for full-app freeze in the harness: continuous **≥30s** window where `capilot status` hangs/fails or stays ≥15s slow (`evaluateFullAppFreeze` / `foreverUiLockupObserved`).
...
- `restart-proxy` does **not** kill CaPilot; it runs `capilot open` + refresh RPCs only.
- Manual capture if UI fully freezes: `sample CaPilot 5 -file ~/Desktop/capilot-freeze-sample.txt`

The scripts honor `ORCA_CLI_COMMAND`, then use `capilot-dev` in a dev runtime, `capilot-ide` on Linux, and `capilot` elsewhere.
