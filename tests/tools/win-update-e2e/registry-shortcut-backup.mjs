...
/**
 * Read-only discovery of the machine's current CaPilot install registry state.
...
export function discoverInstallRegistryState() {
  const ps = [
    ...
    `  if ($dn -eq 'CaPilot' -or ($il -and $il -match '\\\\Programs\\\\capilot')) {`,
    ...
    `  if ($il -and ($sn -eq 'CaPilot' -or $il -match 'capilot')) {`,
...
/** The two directories NSIS writes CaPilot shortcuts into: Start Menu + Desktop. */
...
/** Read-only: list CaPilot *.lnk shortcuts under the given dirs (recursive). */
...
function collectOrcaLnks(dir, out) {
  ...
  for (const entry of entries) {
    ...
    } else if (entry.isFile() && /\.lnk$/i.test(entry.name) && /capilot/i.test(entry.name)) {
      out.push(full)
    }
  }
}
...
/**
 * Snapshot the shared install registry keys + CaPilot shortcuts before an isolated
 ...
 * Pre-existing shortcuts are copied back; test-created CaPilot shortcuts are removed.
...
function printMismatchWarning(manifest, expected, actual) {
  ...
  console.error('!! Your REAL CaPilot install pointer may be hijacked to the test directory.')
  console.error('!! The next real CaPilot update could install into the test location.')
...
if (process.argv[1] && path.resolve(process.argv[1]) === import.meta.filename) {
  ...
  console.log('[registry-shortcut-backup] CaPilot shortcuts that would be backed up:')
