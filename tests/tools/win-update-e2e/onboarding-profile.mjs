// Persistence seed for a fresh packaged CaPilot profile: dismisses onboarding and
...
// capilot-data.json` BEFORE launch dismisses it. The app derives the Projects list
...
//
// IMPORTANT: seed only BEFORE the first launch. The app rewrites capilot-data.json
 ...
 * for it. A real checkout (init + one commit) is required — CaPilot treats a
...
export function createSeededRepo(dir) {
  ...
  git('init', '-b', 'main')
  git('config', 'user.email', 'win-update-e2e@capilot.test')
...
export function seedFreshProfile(userDataDir, profile) {
  ...
  writeFileSync(path.join(userDataDir, 'capilot-data.json'), `${JSON.stringify(profile, null, 2)}\n`)
}
