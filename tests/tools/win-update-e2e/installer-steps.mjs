...
//
// CaPilot ships a per-user oneClick NSIS installer (electron-builder defaults:
// oneClick=true, perMachine=false) named capilot-windows-setup.exe. One-click
// silent mode is `<setup.exe> /S`; the app installs under
// %LOCALAPPDATA%\Programs\<dir> and the exe is CaPilot.exe. The install dir casing
// is not guaranteed (observed lowercase "capilot" on a dev box), so the exe is
...
const PRODUCT_NAME = 'CaPilot'
const EXE_NAME = 'CaPilot.exe'
...
export function resolveInstaller({ localPath, releaseTag, assetPattern }) {
  ...
  const outDir = mkdtempSync(path.join(tmpdir(), 'capilot-e2e-installer-'))
...
export function silentInstall(setupExe, { timeoutMs = 180_000, installDir = null } = {}) {
  ...
  // On update runs the old CaPilot.exe already exists, so wait for the exe whose
...
/**
 * Locate the installed CaPilot.exe. In isolated mode (`installDir` set), the exe is
 * at a known fixed path (<installDir>\CaPilot.exe). Otherwise it is discovered
 ...
 * developer's REAL CaPilot. It additionally refuses to run against the default
  ...
  if (!allowDefaultLocation && pathsEqual(resolved, path.join(programsRoot(), PRODUCT_NAME))) {
    throw new Error(
      `Refusing to uninstall the default install location "${resolved}" — this is where a ` +
        `developer's REAL CaPilot lives. Isolated mode must target a separate --install-dir.`
    )
  }
