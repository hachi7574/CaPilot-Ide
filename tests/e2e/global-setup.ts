...
export const TEST_REPO_PATH_FILE = path.join(os.tmpdir(), 'capilot-e2e-test-repo-path.txt')
...
export default function globalSetup(): void {
  ...
  if (
    ...
    // Why: the SSH specs deploy CaPilot's relay from out/relay. The
  ...
  const testRepoDir = realpathSync(mkdtempSync(path.join(os.tmpdir(), 'capilot-e2e-repo-')))
  ...
  const worktreeDir = path.join(testRepoDir, '..', `capilot-e2e-worktree-${randomUUID()}`)
