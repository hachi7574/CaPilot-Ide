...
const scratch = mkdtempSync(path.join(os.tmpdir(), 'capilot-headless-stalled-stream-'))
...
test('recovers an ACK-starved stream from an isolated headless CaPilot host @headful', async ({
  testRepoPath
}) => {
