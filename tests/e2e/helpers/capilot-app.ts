/**
 * Shared Electron fixture for CaPilot E2E tests.
 *
...
type OrcaTestFixtures = {
  ...
  // developer's shell or already-running CaPilot instance.
...
const ORCA_E2E_SLOWMO_MS = ((): number => {
  ...
  if (!Number.isFinite(parsed)) {
    console.warn(
      `[capilot-e2e] ORCA_E2E_SLOWMO_MS="${ORCA_E2E_SLOWMO_MS_RAW}" is not a number; ignoring (using 0).`
    )
    return 0
  }
...
/**
 * Extended Playwright test with CaPilot-specific fixtures.
 *
 * `orcaPage` — the main CaPilot renderer window.
 *
...
export const test = base.extend<OrcaTestFixtures, OrcaWorkerFixtures>({
  ...
  electronApp: async (
    ...
    const userDataDir = mkdtempSync(path.join(os.tmpdir(), 'capilot-e2e-userdata-'))
    ...
    // CaPilot's own agent runtime) set it so Electron behaves as a plain Node
