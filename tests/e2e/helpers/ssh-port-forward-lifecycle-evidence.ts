...
export function startRemoteHttpListener(
  ...
  execDockerSshRelayTargetCommand(
    target,
    [
      `nohup node -e ${shellQuote(script)} >/tmp/capilot-http-${port}.log 2>&1 < /dev/null &`,
      `echo $! >/tmp/capilot-http-${port}.pid`
    ].join(' ')
  )
  return Number(execDockerSshRelayTargetCommand(target, `cat /tmp/capilot-http-${port}.pid`))
}
...
export function readRemoteListenerIdentity(
  ...
  const pid = Number(execDockerSshRelayTargetCommand(target, `cat /tmp/capilot-http-${port}.pid`))
  return {
    pid,
