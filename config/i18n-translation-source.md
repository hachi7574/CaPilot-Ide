...
Reduce the recurring engineering and human work required to localize CaPilot
...
Use gettext PO as CaPilot's canonical bilingual translation source, under a
constrained profile:

- `msgctxt` — the stable CaPilot message ID;
...
dependency. CaPilot must not require network access or a translation service to
...
CaPilot will support only the subset needed for its message model. PR B must
define and validate that subset rather than accepting arbitrary PO documents.

Each supported entry must carry:

- one stable CaPilot message ID (`msgctxt`), unique within the file;
...
Plurals use CaPilot's existing per-key convention: each i18next plural-suffixed
...
- XML is the format most likely to let a hand-edited community PR fail on a
  ...
  external editors produce whole-file diffs; CaPilot's translation contributions
  arrive overwhelmingly as direct catalog PRs.

If a translation provider requires XLIFF, CaPilot should add a deterministic
...
translator context, or review state. Meeting CaPilot's requirements would require
...
own platform but are not a suitable authority for CaPilot's macOS, Linux,
...
not become CaPilot's sole source of truth or a runtime dependency. Selecting one
...
to establish a format decision. CaPilot therefore chooses based on its own
