...
const wrapperPath = path.join(projectDir, 'config', 'scripts', 'capilot-dev.mjs')

describe('capilot-dev package bin', () => {
  it('uses a Node entrypoint for cross-platform package installs', () => {
    expect(packageJson.bin['capilot-dev']).toBe('./config/scripts/capilot-dev.mjs')
    expect(readFileSync(wrapperPath, 'utf8')).toMatch(/^#!\/usr\/bin\/env node\n/)
  })

  it('runs the dev CLI through Node without requiring Bash', () => {
    const root = mkdtempSync(path.join(tmpdir(), 'capilot-dev-bin-'))
