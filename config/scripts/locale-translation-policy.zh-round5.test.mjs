...
describe('locale-translation-policy zh round 5', () => {
  it('fixes brand spacing, hosted review, and CaPilot Mobile regressions', () => {
    expect(
    ...
    expect(
      repairTranslatedValue({
        ...
        enValue: 'Show CaPilot Mobile Button',
        localeValue: '显示 CaPilot 移动按钮',
        locale: 'zh'
      })
    ).toBe('显示 CaPilot Mobile 按钮')
    expect(
      repairTranslatedValue({
        key: 'auto.components.settings.GitPane.e02ea23a32',
        enValue: 'CaPilot Attribution',
        localeValue: 'Orca归属',
        locale: 'zh'
      })
    ).toBe('CaPilot 归因')
  ...
  it('keeps workflow terms in English', () => {
    ...
    expect(
      repairTranslatedValue({
        ...
        enValue:
          'Git could not safely delete branch "{{value0}}"{{value1}}, so CaPilot kept it to avoid losing local commits.',
        localeValue:
          'Git 无法安全删除分支“{{value0}}”{{value1}}，因此 CaPilot 保留它以避免丢失本地提交。',
        locale: 'zh'
      })
    ).toBe('Git 无法安全删除分支“{{value0}}”{{value1}}，因此 CaPilot 保留它以避免丢失本地 commits。')
  })
