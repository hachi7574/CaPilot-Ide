...
// Why: capilot-cli now ships a hybrid discovery stub, so its version-sensitive command
...
const guidePath = join(projectDir, 'skill-guides', 'capilot-cli.md')
const stubPath = join(projectDir, 'skills', 'capilot-cli', 'SKILL.md')
// Why: orchestration and capilot-emulator also ship hybrid stubs now, so their version-sensitive
...
const emulatorSkillPath = join(projectDir, 'skill-guides', 'capilot-emulator.md')

function readSkill(path = guidePath) {
  return readFileSync(path, 'utf8')
}

describe('capilot CLI skill guidance', () => {
  it('keeps independent worktree lineage separate from Git base selection', () => {
    const skill = readSkill()

    expect(skill).toContain('`--no-parent` only controls CaPilot lineage')
    expect(skill).toContain('omit `--base-branch` so CaPilot uses the repo default base')
  ...
  it('keeps browser injection guidance narrow and avoids literal secret examples', () => {
    ...
    expect(skill).toContain('`capilot eval` expressions, or `capilot exec` commands')
...
describe('capilot CLI install stub', () => {
  ...
  it('does not mistake resolution or execution failures for an older binary', () => {
    ...
    // Falling through can silently pair a version-matched guide with the wrong CaPilot build.
