...
describe('dev CLI terminal wrappers', () => {
  it('writes profile-scoped Windows wrappers for worker terminals', () => {
    const root = mkdtempSync(path.join(tmpdir(), 'capilot-dev-terminal-wrapper-'))
    ...
    const wrapper = readFileSync(path.join(userDataPath, 'cli', 'bin', 'capilot-dev.cmd'), 'utf8')
    ...
    expect(readFileSync(path.join(userDataPath, 'cli', 'bin', 'capilot.cmd'), 'utf8')).toBe(wrapper)
    expect(readFileSync(path.join(root, 'out', 'bin', 'capilot-dev.cmd'), 'utf8')).toBe(wrapper)
    expect(readFileSync(path.join(root, 'out', 'bin', 'capilot.cmd'), 'utf8')).toBe(wrapper)
  })

  it('escapes literal percent signs in every Windows batch path', () => {
    const root = path.join(mkdtempSync(path.join(tmpdir(), 'capilot-dev-terminal-wrapper-')), '%repo%')
    const userDataPath = path.join(root, '%profile%')
  ...
  it('writes executable-style POSIX wrappers with the same profile identity', () => {
    ...
    const wrapper = readFileSync(path.join(userDataPath, 'cli', 'bin', 'capilot-dev'), 'utf8')
    ...
    expect(readFileSync(path.join(userDataPath, 'cli', 'bin', 'capilot'), 'utf8')).toBe(wrapper)
    expect(readFileSync(path.join(root, 'out', 'bin', 'capilot-dev'), 'utf8')).toBe(wrapper)
    expect(readFileSync(path.join(root, 'out', 'bin', 'capilot'), 'utf8')).toBe(wrapper)
  })
})
