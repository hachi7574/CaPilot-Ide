...
// Why: capilot-linear and its legacy linear-tickets alias now ship hybrid discovery stubs, so
...
const canonicalGuidePath = join(projectDir, 'skill-guides', 'capilot-linear.md')
const legacyGuidePath = join(projectDir, 'skill-guides', 'linear-tickets.md')
const canonicalStubPath = join(projectDir, 'skills', 'capilot-linear', 'SKILL.md')
...
describe('capilot-linear skill guidance', () => {
  it('keeps canonical and legacy Linear guide bodies from drifting', () => {
    ...
    expect(canonical).toContain('name: capilot-linear')
  ...
  it('documents targeted project discovery in both skill names', () => {
    ...
    for (const skill of [canonical, legacy]) {
      expect(skill).toContain('capilot linear project list [--query <text>]')
...
describe('capilot-linear install stubs', () => {
  const cases = [
    { name: 'capilot-linear', stubPath: canonicalStubPath, guidePath: canonicalGuidePath },
  ...
  for (const { name, stubPath, guidePath } of cases) {
    ...
    it(`drops the changing command reference from the installable ${name} file`, () => {
      ...
      expect(stub).not.toContain('capilot linear search')
      expect(stub).not.toContain('capilot linear comment')
