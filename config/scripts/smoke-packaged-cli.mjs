...
function getPackagedCliPath(appDir) {
  if (process.platform === 'darwin' || appDir.endsWith('.app')) {
    return join(appDir, 'Contents', 'Resources', 'bin', 'capilot')
  }
  if (process.platform === 'win32') {
    return join(appDir, 'resources', 'bin', 'capilot.exe')
  }
  return join(appDir, 'resources', 'bin', 'capilot-ide')
}

const appDir = resolve(readAppDirArg(process.argv.slice(2)))
const tempRoot = await mkdtemp(join(tmpdir(), 'capilot-packaged-cli-smoke-'))
...
try {
  ...
  assert(list.topics.some((topic) => topic.name === 'capilot-cli'))
  assert.match((await run(['skills', 'get', 'capilot-cli'])).stdout, /name: capilot-cli/)
  ...
  const update = JSON.parse(
    (await run(['skills', 'update', '--skill', 'capilot-cli', '--dry-run', '--json'])).stdout
  )
...
// Why: on Windows the launcher above spawns the copied CaPilot.exe (and its crashpad/utility children)
