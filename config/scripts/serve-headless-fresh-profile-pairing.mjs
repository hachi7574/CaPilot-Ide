...
const orcaDevScript = path.join(scriptDir, 'capilot-dev.mjs')
...
const profileDir =
  fixedProfileDir ?? mkdtempSync(path.join(tmpdir(), 'capilot-headless-pairing-profile-'))
...
Object.assign(childEnv, {
  // Why: a fresh temporary CaPilot profile must not make the default Codex lane
...
console.error(`[headless-pairing] starting: capilot-dev serve --json${formatForwardedArgs(serveArgs)}`)
...
/**
 * Parses wrapper flags and forwards everything else to `capilot serve`.
 */
 ...
 */
function printHelp() {
  console.log(`Usage: node config/scripts/serve-headless-fresh-profile-pairing.mjs [--keep] [capilot serve flags]

Starts capilot-dev serve --json with a fresh isolated userData profile, ensures Electron's dev runtime is usable, and prints the pairing URL.
...
function printReadyLine(line) {
  ...
  console.log(`CaPilot server ready: ${payload.boundEndpoint ?? 'websocket unavailable'}`)
...
function stopChild(signal) {
  ...
  if (process.platform === 'win32' && child.pid) {
    // Why: child.kill() only targets capilot-dev on Windows; taskkill walks the
  ...
  if (process.platform !== 'win32' && child.pid) {
    // Why: capilot-dev synchronously owns the CLI child, which owns Electron; kill
...
function cleanupProfile() {
  ...
  if (!existsSync(profileDir) || !profileDir.includes('capilot-headless-pairing-profile-')) {
