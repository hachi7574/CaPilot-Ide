...
function withTempFile(callback) {
  const dir = mkdtempSync(join(tmpdir(), 'capilot-inner-signature-'))
  const filePath = join(dir, 'CaPilot.exe')
...
describe('verify-windows-inner-signature', () => {
  ...
  it('rejects missing, nonexistent, and directory executable paths before PowerShell', () => {
    ...
    expect(() => validateExecutablePath(join(tmpdir(), 'missing-CaPilot.exe'))).toThrow(
      /does not exist/
    )
  ...
  it('runs PowerShell with an argument array and fails on stderr or nonzero exit', () => {
    ...
    expect(getPowerShellSignatureJson('C:\\Path With Spaces\\CaPilot.exe', spawnSyncImpl)).toBe(
      JSON.stringify(validSignature)
    )
    expect(calls[0].command).toBe('pwsh')
    expect(calls[0].args).toContain('-Command')
    expect(calls[0].args.at(-1)).not.toBe('C:\\Path With Spaces\\CaPilot.exe')
    expect(calls[0].options).toEqual(
      expect.objectContaining({
        encoding: 'utf8',
        env: expect.objectContaining({
          ORCA_WINDOWS_INNER_EXECUTABLE: 'C:\\Path With Spaces\\CaPilot.exe'
        })
      })
    )

    expect(() =>
      getPowerShellSignatureJson('CaPilot.exe', () => ({ status: 0, stdout: '{}', stderr: 'warning' }))
    ).toThrow(/stderr/)
    expect(() =>
      getPowerShellSignatureJson('CaPilot.exe', () => ({ status: 7, stdout: '', stderr: '' }))
    ).toThrow(/exit code 7/)
  })
