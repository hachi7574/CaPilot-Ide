...
function hashPackagedPluginTree(root) {
  ...
  visit(root)
  const hash = createHash('sha256').update('capilot-plugin-tree-v1\0')
...
function verifyPackagedPluginResources(resourcesDir) {
  ...
  readJsonFile(join(launchRoot, 'capilot-marketplace.json'), 'marketplace index')
  ...
  for (const entry of index.plugins) {
    ...
    const manifest = readJsonFile(join(pluginRoot, 'capilot-plugin.json'), 'plugin manifest')
