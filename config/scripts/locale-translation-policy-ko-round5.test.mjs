  ...
  it('fixes Korean round 5 review, integration, and search keyword regressions', () => {
    ...
    expect(
      repairTranslatedValue({
        ...
        enValue: 'Install the CaPilot skill so agents know to use the CaPilot CLI.',
        localeValue: '에이전트가 CaPilot CLI 사용 방법을 알 수 있도록 CaPilot 기술을 설치합니다.',
        locale: 'ko'
      })
    ).toBe('agents가 CaPilot CLI를 사용하도록 CaPilot 스킬을 설치하세요.')
    expect(
      repairTranslatedValue({
