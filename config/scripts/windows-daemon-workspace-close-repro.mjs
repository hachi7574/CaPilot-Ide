 ...
 * graceful-then-immediate kill pair emitted when CaPilot closes a workspace.
...
async function main() {
  ...
  const scratch = mkdtempSync(join(tmpdir(), 'capilot-windows-daemon-close-'))
  const socketPath = `\\\\.\\pipe\\capilot-daemon-close-${process.pid}-${randomUUID()}`
