...
This file is a discovery stub, not the usage guide. The full, version-matched CaPilot Linear
reference is served by the `capilot` binary itself — kept out of this file on purpose so it can
never drift from the binary that will actually run your commands.

Engage CaPilot's Linear CLI (`capilot linear ...`) whenever you work a Linear-linked task: read
...
```text
ORCA skills get capilot-linear
```
...
`ORCA skills get capilot-linear`. Beyond these commands, ask the user rather than guessing a
