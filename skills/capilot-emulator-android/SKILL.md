...
# CaPilot Emulator (Android)

This file is a discovery stub, not the usage guide. The full, version-matched CaPilot Android
emulator reference is served by the `capilot` binary itself — kept out of this file on purpose
so it can never drift from the binary that will actually run your commands.

Engage CaPilot whenever you drive an adb-connected Android emulator or device from inside the
CaPilot app: listing/booting AVDs, taps, swipes, typing, hardware buttons (including Back and
Recents), rotation, app install/launch, runtime permissions, the accessibility tree, and
logcat. It is cross-platform (Windows, Linux, macOS) and complements the capilot-emulator (iOS)
and capilot-cli skills.
...
```text
ORCA skills get capilot-emulator-android
```
...
`ORCA skills get capilot-emulator-android`. Beyond these commands, ask the user rather than
