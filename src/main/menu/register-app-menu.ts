...
function buildAndApplyMenu(options: RegisterAppMenuOptions): void {
  ...
  const featureTourItem: Electron.MenuItemConstructorOptions = {
    label: translateMain('menu.exploreOrca', 'Explore CaPilot'),
    click: (_menuItem, window) => onOpenFeatureTour(window)
  }

  const setupGuideItem: Electron.MenuItemConstructorOptions = {
    label: translateMain('menu.gettingStarted', 'Getting Started with CaPilot'),
  ...
  // redundant "CaPilot" entry with roles that don't apply, so we omit it there
    ...
    submenu: [
      ...
      { role: 'copy' },
      {
        ...
        click: () => {
          ...
          // control, so raw Electron paste cannot know which CaPilot surface owns it.
  ...
  const appearanceSubmenu: Electron.MenuItemConstructorOptions = {
    ...
    submenu: [
      ...
      {
        label: translateMain('menu.showMobileButton', 'Show CaPilot Mobile Button'),
        type: 'checkbox',
