 ...
 * first launch.
 *
 * Why: CaPilot's "drop URL into agent input as a draft" flow injects the URL
...
export function markCodexProjectTrusted(workspacePath: string): void {
  ...
  // Why: CaPilot-launched Codex runs with an CaPilot-owned CODEX_HOME, so the trust
...
function canonicalize(p: string): string {
  ...
  // (capilot caches realpath()'d worktree paths) matches the agent's lookup.
  try {
    if (existsSync(p)) {
