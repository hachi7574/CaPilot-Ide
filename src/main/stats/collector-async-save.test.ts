...
const statsPath = (): string => join(userDataDir, 'capilot-stats.json')
...
describe('StatsCollector async debounced save', () => {
  beforeEach(() => {
    userDataDir = mkdtempSync(join(tmpdir(), 'capilot-stats-test-'))
    gate.blocked = false
  ...
  it('writes valid JSON via the async fs/promises path on the debounced timer, no stray temp files', async () => {
    ...
    await vi.waitFor(() => {
      expect(readdirSync(userDataDir)).toContain('capilot-stats.json')
    })
