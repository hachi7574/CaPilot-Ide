...
describe('OpenCode hook plugin source', () => {
  ...
  it('resolves hook coords from the endpoint file before falling back to process.env', () => {
    // Why: a forked session freezes the prior CaPilot's PORT/TOKEN in env; prefer the on-disk endpoint file or it posts to a dead port after restart.
    const source = _internals.getOpenCodePluginSource()
    ...
    expect(source).toContain('"X-CaPilot-Agent-Hook-Token": coords.token')
  })
...
describe('OpenCodeHookService buildPtyEnv / clearPty round-trip', () => {
  ...
  beforeAll(() => {
    userDataDir = mkdtempSync(join(tmpdir(), 'capilot-opencode-hooks-'))
  ...
  it('writes a shared OPENCODE_CONFIG_DIR and installs the plugin file', () => {
    ...
    const pluginPath = join(env.OPENCODE_CONFIG_DIR!, 'plugins', 'capilot-opencode-status.js')
  ...
  it('buildPtyEnv preserves a user-set OPENCODE_CONFIG_DIR when the id is unusable', () => {
    ...
    const service = new OpenCodeHookService()
    const userDir = mkdtempSync(join(tmpdir(), 'capilot-opencode-userdir-'))
    try {
  ...
  it('works end-to-end for a plain UUID id (non-daemon path)', () => {
    ...
    expect(env.OPENCODE_CONFIG_DIR).toBe(join(userDataDir, 'opencode-hooks', 'shared'))
    expect(existsSync(join(env.OPENCODE_CONFIG_DIR!, 'plugins', 'capilot-opencode-status.js'))).toBe(
      true
    )
...
describe('OpenCodeHookService overlay mode (user OPENCODE_CONFIG_DIR set)', () => {
  ...
  beforeAll(() => {
    userDataDir = mkdtempSync(join(tmpdir(), 'capilot-opencode-overlay-userdata-'))
    getPathMock.mockImplementation((name: string) => {
      if (name === 'userData') {
        return userDataDir
      }
  ...
  beforeEach(() => {
    userConfigDir = mkdtempSync(join(tmpdir(), 'capilot-opencode-overlay-userconfig-'))
  ...
  it('builds an overlay under userData and exposes user config + CaPilot plugin together', () => {
    const service = new OpenCodeHookService()
    ...
    // CaPilot's status plugin is a sibling, not a replacement.
    const orcaPluginPath = join(env.OPENCODE_CONFIG_DIR!, 'plugins', 'capilot-opencode-status.js')
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    () => {
      // Why: only plugins/ needs per-entry mirroring (CaPilot drops a sibling); other entries are single symlinks so user edits propagate live.
      const service = new OpenCodeHookService()
      const env = service.buildPtyEnv(ptyId, userConfigDir)
      ...
      // plugins/ must be a real dir in the overlay so CaPilot can drop its sibling plugin.
  ...
  it("does not overwrite a user plugin file with the same filename as CaPilot's plugin", () => {
    // Why: a user plugin named capilot-opencode-status.js must not be symlinked into the overlay, or writeFileSync would clobber it.
    const userOrcaSentinel = 'USER OWNED ORCA-NAMED PLUGIN — DO NOT CLOBBER'
    writeFileSync(join(userConfigDir, 'plugins', 'capilot-opencode-status.js'), userOrcaSentinel)

    const service = new OpenCodeHookService()
    const env = service.buildPtyEnv(ptyId, userConfigDir)

    // User's source file must be untouched.
    expect(readFileSync(join(userConfigDir, 'plugins', 'capilot-opencode-status.js'), 'utf8')).toBe(
      userOrcaSentinel
    )

    // Overlay copy is CaPilot's real plugin source, not the user's file.
    const overlayPlugin = readFileSync(
      join(env.OPENCODE_CONFIG_DIR!, 'plugins', 'capilot-opencode-status.js'),
      'utf8'
    )
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    () => {
      // Why: writing CaPilot's plugin through a symlinked plugins/ would leak into the user's fs (docs/opencode-config-dir-collision.md).
      const realPluginsDir = mkdtempSync(join(tmpdir(), 'capilot-real-plugins-'))
      try {
        ...
        // The user's real filesystem must NOT receive CaPilot's status plugin.
        expect(existsSync(join(realPluginsDir, 'capilot-opencode-status.js'))).toBe(false)
        ...
        // CaPilot's status plugin lands in the overlay only.
        expect(
          existsSync(join(env.OPENCODE_CONFIG_DIR!, 'plugins', 'capilot-opencode-status.js'))
        ).toBe(true)
  ...
  it("preserves the user's OPENCODE_CONFIG_DIR when the path does not exist", () => {
    // Why: leave a nonexistent user path alone so OpenCode surfaces the typo instead of CaPilot silently hiding it.
    const service = new OpenCodeHookService()
    const missingPath = join(tmpdir(), `capilot-opencode-nope-${Date.now()}`)
  ...
  it('rebuilding the overlay for the same ptyId does not corrupt the user dir', () => {
    ...
    expect(
      readFileSync(join(env.OPENCODE_CONFIG_DIR!, 'plugins', 'capilot-opencode-status.js'), 'utf8')
