...
const ORCA_OPENCODE_PLUGIN_FILE = 'capilot-opencode-status.js'
...
const OPENCODE_OVERLAY_MANIFEST_FILE = '.capilot-opencode-overlay-manifest.json'
...
export function getOpenCodeFamilyPluginSource(hookPathname: string): string {
  ...
  return [
    ...
    "// OpenCode's Node process (not CaPilot's) and has no access to server.ts's",
    ...
    '// which reads the endpoint file. The file only changes on CaPilot restart',
    ...
    '      // writeEndpointFile on the CaPilot side) allocates a fresh inode on',
    ...
    '      console.warn("[capilot-hook] failed to parse endpoint file:", err.message);',
    '    }',
    '    return null;',
    '  }',
    '}',
    ...
    '  // frozen when OpenCode was fork()ed — stale after an CaPilot restart. The',
    '  // file is rewritten on every CaPilot start(), so sourcing it per post lets',
    '  // a long-running OpenCode session reach the current server. Falls back',
    '  // to process.env when the file is absent (first-run / pre-endpoint-file / CaPilot',
    ...
    '// per turn through CaPilot (loopback HTTP -> main JSON parse -> status compare',
    ...
    '// flip the root CaPilot pane to done or overwrite the parent turn preview.',
    ...
    '  // rewritten by a newer CaPilot since the last call. Pane/tab/worktree IDs',
    '  // stay on process.env because they are per-PTY (stable for the life of',
    '  // the OpenCode process), not per-CaPilot-instance.',
    '  const coords = resolveHookCoords();',
    ...
    '    // because CaPilot is unavailable or the local loopback request failed.',
    '    return false;',
    '  } finally {',
    '    clearTimeout(timeout);',
    '  }',
    ...
    '          // publish a final idle so CaPilot does not retain a dead owner.',
...
export class OpenCodeHookService {
  ...
  buildPtyEnv(ptyId: string, existingConfigDir?: string | undefined): Record<string, string> {
    if (!isUsableId(ptyId)) {
      // Why: on a bad id, still preserve a user-set OPENCODE_CONFIG_DIR; only the CaPilot status plugin is forfeited.
  ...
  // Why: mirror user config entries as symlinks so edits propagate live; only plugins/ becomes a real overlay dir so CaPilot can drop a sibling plugin file.
  private mirrorUserConfig(sourceDir: string, overlayDir: string): void {
    ...
    // Why: overlays persist across terminals; remove only CaPilot-mirrored paths so stale user config clears but OpenCode runtime dirs (node_modules) survive.
      ...
      if (entry.name === 'plugins') {
        ...
        if ((!isSymlink && entry.isDirectory()) || isLinkPointingToDir) {
          ...
          for (const pluginEntry of readdirSync(resolvedSource, { withFileTypes: true })) {
            // Why: skip a user plugin sharing CaPilot's filename; mirroring it would let writePluginIntoOverlay clobber the user's file.
