...
export function getZshEnvTemplate(zshDir: string, headerPrefix = ''): string {
  const header = headerPrefix
    ? `CaPilot ${headerPrefix} zsh shell-ready wrapper`
    : 'CaPilot zsh shell-ready wrapper'
  return `# ${header}
...
# CaPilot PTYs never source another CaPilot wrapper recursively.
...
if [[ ${checks.join(' && ')} ]]; then
  ...
  # CaPilot restores its wrapper dir afterward so zsh still loads wrapper files.
...
# Why: after CaPilot's last wrapper file has loaded, the interactive shell should
