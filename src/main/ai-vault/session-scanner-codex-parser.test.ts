...
describe('parseCodexSessionFile', () => {
  it('uses user-message events instead of later injected user-role records', async () => {
    const root = await mkdtemp(join(tmpdir(), 'capilot-ai-vault-codex-last-prompt-'))
    tempRoots.push(root)
  ...
  it('does not double-count usage when token count formats switch', async () => {
    const root = await mkdtemp(join(tmpdir(), 'capilot-ai-vault-codex-token-switch-'))
    tempRoots.push(root)
  ...
  it('extracts the model from turn context, latest turn winning', async () => {
    const root = await mkdtemp(join(tmpdir(), 'capilot-ai-vault-codex-model-'))
    tempRoots.push(root)
