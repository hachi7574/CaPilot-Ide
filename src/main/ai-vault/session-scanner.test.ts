...
describe('scanAiVaultSessions', () => {
  it('indexes Claude and Codex transcripts with resume commands', async () => {
    const root = await mkdtemp(join(tmpdir(), 'capilot-ai-vault-'))
    tempRoots.push(root)
  ...
  it('indexes Codex sessions from CaPilot runtime homes with resumable commands', async () => {
    const root = await mkdtemp(join(tmpdir(), 'capilot-ai-vault-codex-runtime-'))
    tempRoots.push(root)
    const roots = isolatedScanRoots(root)
      ...
      jsonLines([
        {
          ...
          payload: {
            ...
            cwd: '/Users/nwparker/capilot/workspaces/capilot/mem4'
          }
        },
        {
    ...
    expect(result.sessions[0]).toMatchObject({
      ...
      resumeCommand: `cd '/Users/nwparker/capilot/workspaces/capilot/mem4' && CODEX_HOME='${runtimeHome}' codex resume '019e9693-64fc-7370-9c18-7e625c595d0f'`
    })
  })

  it('indexes WSL home session roots', async () => {
    const root = await mkdtemp(join(tmpdir(), 'capilot-ai-vault-wsl-'))
    tempRoots.push(root)
    const roots = isolatedScanRoots(root)
    const wslHome = join(root, 'wsl', 'Ubuntu', 'home', 'ada')
    await mkdir(join(wslHome, '.claude', 'projects', 'repo'), { recursive: true })
    await mkdir(
      join(wslHome, '.local', 'share', 'capilot', 'codex-runtime-home', 'home', 'sessions'),
      {
        recursive: true
      }
    )
  ...
  it('skips hidden Codex context blocks when choosing session titles', async () => {
    const root = await mkdtemp(join(tmpdir(), 'capilot-ai-vault-codex-hidden-context-'))
    tempRoots.push(root)
    const roots = isolatedScanRoots(root)
  ...
  it('indexes every supported agent transcript format with native resume commands', async () => {
    const root = await mkdtemp(join(tmpdir(), 'capilot-ai-vault-all-agents-'))
    tempRoots.push(root)
    const roots = isolatedScanRoots(root)
  ...
  it('captures an in-progress OMP model from model_change before any assistant reply', async () => {
    ...
    const root = await mkdtemp(join(tmpdir(), 'capilot-ai-vault-omp-mc-'))
    tempRoots.push(root)
    const roots = isolatedScanRoots(root)
    await mkdir(roots.ompSessionsDir, { recursive: true })
    await writeFile(
  ...
  it('strips newline-heavy Grok user_query envelopes without regex matching', async () => {
    ...
    const root = await mkdtemp(join(tmpdir(), 'capilot-ai-vault-grok-large-'))
    tempRoots.push(root)
    const roots = isolatedScanRoots(root)
