...
/**
 * Counts the macOS TCC consent dialogs that name CaPilot as the responsible
 ...
 * CaPilot and puts CaPilot's name on the dialog, so users read it as CaPilot snooping.
 *
...
/** True when this dialog is one macOS raised in CaPilot's name for a watched file-access service. */
...
/**
 * Watches for CaPilot-attributed TCC dialogs. macOS-only; `start()` is a no-op
