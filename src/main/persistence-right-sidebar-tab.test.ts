...
vi.mock('electron', () => ({
  app: {
    getPath: () => '/tmp/capilot-persistence-right-sidebar-tab-test'
  },
  safeStorage: {
...
describe('normalizeRightSidebarTab', () => {
  ...
  it('preserves well-formed plugin panel tabs', () => {
    expect(normalizeRightSidebarTab('plugin:capilot-samples.my-plugin/dashboard')).toBe(
      'plugin:capilot-samples.my-plugin/dashboard'
    )
  })

  it('normalizes malformed plugin tabs to the default tab', () => {
    expect(normalizeRightSidebarTab('plugin:capilot-samples.my-plugin')).toBe('explorer')
    expect(normalizeRightSidebarTab('plugin:capilot-samples.my-plugin/panel/extra')).toBe('explorer')
