...
const globallyUpdatableNames = new Set(['computer-use', 'capilot-cli', 'orchestration'])
...
describe('skill freshness name-scoped update eligibility', () => {
  ...
  it.each([
    ...
    (status, topology) => {
      ...
      expect(
        eligible([
          placement('capilot-cli'),
          placement('capilot-cli', { id: `outlier-${status}-${topology}`, status, topology })
        ])
      ).toEqual(['capilot-cli'])
    }
  )
  ...
  it.each(['unrecognized', 'inaccessible', 'newer-known'] as const)(
    ...
    (status) => {
      ...
      expect(
        eligible([
          placement('capilot-cli', { id: 'blocked-canonical', status }),
          placement('capilot-cli', {
            id: 'capilot-cli-claude',
            rootId: 'home-claude',
            topology: 'provider-alias',
  ...
  it('still updates the canonical copy when a clean standalone duplicate exists', () => {
    ...
    expect(
      eligible([
        placement('capilot-cli'),
        placement('capilot-cli', {
          id: 'capilot-cli-gemini',
          rootId: 'home-gemini',
          unresolvedPath: '/home/.gemini/skills/capilot-cli',
          resolvedPath: '/home/.gemini/skills/capilot-cli',
  ...
  it('does not offer an official canonical copy missing from the updater lock (#10791)', () => {
    expect(eligibleSkillUpdateNames([placement('capilot-cli')], new Set())).toEqual([])
  })

  it('builds only an explicit, deterministic global command', () => {
    expect(buildTargetedSkillUpdateCommand(['orchestration', 'capilot-cli', 'capilot-cli'])).toBe(
      'npx skills update capilot-cli orchestration --global'
    )
    expect(buildTargetedSkillUpdateCommand([])).toBeNull()
    expect(buildTargetedSkillUpdateCommand(['capilot-cli;echo unsafe'])).toBeNull()
  })
})
