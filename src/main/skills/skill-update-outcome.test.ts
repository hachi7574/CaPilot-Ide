...
describe('skillUpdateFailedNames', () => {
  it('treats a convergent copy that is now current as landed', () => {
    expect(
      skillUpdateFailedNames(['capilot-cli'], [placement('capilot-cli', 'current')], noLocks)
    ).toEqual([])
  })

  it('reports a copy the run left outdated', () => {
    expect(
      skillUpdateFailedNames(['capilot-cli'], [placement('capilot-cli', 'outdated')], noLocks)
    ).toEqual(['capilot-cli'])
  })
  ...
  it('reports a half-written bundle instead of reading it as success', () => {
    ...
    expect(
      skillUpdateFailedNames(['capilot-cli'], [placement('capilot-cli', 'unrecognized')], noLocks)
    ).toEqual(['capilot-cli'])
  })

  it('reports an unreadable copy', () => {
    expect(
      skillUpdateFailedNames(['capilot-cli'], [placement('capilot-cli', 'inaccessible')], noLocks)
    ).toEqual(['capilot-cli'])
  })

  it('reports a skill the run removed outright', () => {
    expect(skillUpdateFailedNames(['capilot-cli'], [], noLocks)).toEqual(['capilot-cli'])
  })
  ...
  it('accepts a revision newer than this build ships', () => {
    ...
    expect(
      skillUpdateFailedNames(['capilot-cli'], [placement('capilot-cli', 'newer-known')], noLocks)
    ).toEqual([])
  })
  ...
  it('ignores placements the update command never writes to', () => {
    expect(
      skillUpdateFailedNames(
        ['capilot-cli'],
        [placement('capilot-cli', 'current'), placement('capilot-cli', 'outdated', 'plugin-cache')],
        noLocks
      )
    ).toEqual([])
  })
  ...
  it('fails the name when any convergent alias was left behind', () => {
    expect(
      skillUpdateFailedNames(
        ['capilot-cli'],
        [placement('capilot-cli', 'current'), placement('capilot-cli', 'outdated', 'provider-alias')],
        noLocks
      )
    ).toEqual(['capilot-cli'])
  })
  ...
  it('judges each requested name independently', () => {
    expect(
      skillUpdateFailedNames(
        ['capilot-cli', 'orchestration'],
        [placement('capilot-cli', 'current'), placement('orchestration', 'outdated')],
        noLocks
      )
  ...
  it('treats unrecognized content whose tree sha matches the lock as landed', () => {
    ...
    expect(
      skillUpdateFailedNames(
        ['capilot-cli'],
        [placement('capilot-cli', 'unrecognized', 'canonical-copy', 'ahead-of-bundle')],
        new Map([['capilot-cli', 'ahead-of-bundle']])
      )
    ).toEqual([])
  })
  ...
  it('still reports unrecognized content whose bytes do not match the lock', () => {
    expect(
      skillUpdateFailedNames(
        ['capilot-cli'],
        [placement('capilot-cli', 'unrecognized', 'canonical-copy', 'half-written-bytes')],
        new Map([['capilot-cli', 'ahead-of-bundle']])
      )
    ).toEqual(['capilot-cli'])
  })
  ...
  it('never forgives an outdated copy, even at the lock hash', () => {
    ...
    expect(
      skillUpdateFailedNames(
        ['capilot-cli'],
        [placement('capilot-cli', 'outdated', 'canonical-copy', 'locked-revision')],
        new Map([['capilot-cli', 'locked-revision']])
      )
    ).toEqual(['capilot-cli'])
  })
  ...
  it('does not let a lock-matching canonical copy excuse a degraded alias', () => {
    expect(
      skillUpdateFailedNames(
        ...
        [
          placement('capilot-cli', 'unrecognized', 'canonical-copy', 'ahead-of-bundle'),
          placement('capilot-cli', 'inaccessible', 'provider-alias')
        ],
        new Map([['capilot-cli', 'ahead-of-bundle']])
      )
    ).toEqual(['capilot-cli'])
...
describe('skillUpdateFailedNames over a real inventory', () => {
  ...
  async function postCutFixture(): Promise<{ homeDir: string; installedTreeSha: string }> {
    const root = await mkdtemp(join(tmpdir(), 'capilot-skill-outcome-'))
    temporaryDirectories.push(root)
    const homeDir = join(root, 'home')
    const skillDir = join(homeDir, '.agents', 'skills', 'capilot-cli')
    ...
    const current = await readFile(join(repoRoot, 'skills', 'capilot-cli', 'SKILL.md'))
    await writeFile(
  ...
  it('accepts post-cut source content when the lock records exactly those bytes', async () => {
    ...
    const canonical = inventory.installations.filter(
      (entry) => entry.name === 'capilot-cli' && entry.topology === 'canonical-copy'
    )
    expect(canonical).toHaveLength(1)
    expect(canonical[0].status).toBe('newer-known')
    expect(canonical[0].installedReleaseRevision).toBeNull()

    expect(skillUpdateFailedNames(['capilot-cli'], inventory.installations, locks)).toEqual([])
  })
