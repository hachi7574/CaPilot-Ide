...
/**
 * Proves a host managed home is an CaPilot-owned account directory and cannot
...
export function assertOwnedHostCodexManagedHomePath({
  ...
  const markerPath = join(canonicalCandidate, '.capilot-managed-home')
  let markerIsRegularFile: boolean
  try {
    markerIsRegularFile = lstatSync(markerPath).isFile()
  } catch (error) {
    throw new Error('Managed Codex home is missing CaPilot ownership marker.', { cause: error })
  }
