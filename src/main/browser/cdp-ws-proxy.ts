...
export class CdpWsProxy {
  ...
  private handleHttpRequest(req: IncomingMessage, res: ServerResponse): void {
    ...
    if (url === '/json/version' || url === '/json/version/') {
      ...
      // "CaPilot/CdpWsProxy" leaks that this is an embedded automation surface, which
  ...
  private handleClientMessage(client: WebSocket, raw: string): void {
    ...
    if (msg.method === 'Browser.getVersion') {
      // Why: returning "CaPilot/Electron" identifies this as an embedded automation
    ...
    // Use the same direct webContents reload path as CaPilot's own browser.reload.
  ...
  private nextSyntheticPageSessionId(): string {
    ...
    return this.nextClientSessionOrdinal === 1
      ? 'capilot-proxy-session'
      : `capilot-proxy-session-${this.nextClientSessionOrdinal}`
  }
  ...
  private nextSyntheticBrowserSessionId(): string {
    ...
    return this.nextClientBrowserSessionOrdinal === 1
      ? 'capilot-proxy-browser-session'
      : `capilot-proxy-browser-session-${this.nextClientBrowserSessionOrdinal}`
  }
  ...
  private async reloadWithLifecycle(
    ...
    if (unsupportedParam) {
      this.sendError(
        clientId,
        `Page.reload parameter "${unsupportedParam}" is not supported for CaPilot tab reloads`,
        client
      )
      return
    }
