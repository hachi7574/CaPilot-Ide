...
const HIDE_BROWSER_GRAB_OVERLAY_SCRIPT = `(function(){
  ...
  document.querySelectorAll('[data-capilot-browser-annotation-overlay]').forEach(function(el) {
    el.setAttribute('data-capilot-previous-display', el.style.display || '');
...
const RESTORE_BROWSER_GRAB_OVERLAY_SCRIPT = `(function(){
  ...
  document.querySelectorAll('[data-capilot-browser-annotation-overlay]').forEach(function(el) {
    el.style.display = el.getAttribute('data-capilot-previous-display') || '';
    el.removeAttribute('data-capilot-previous-display');
  });
})()`
