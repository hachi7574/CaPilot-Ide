...
export class CdpBridge {
  ...
  private getActiveGuest(): Electron.WebContents {
    ...
    if (tabs.length === 0) {
      throw new BrowserError(
        'browser_no_tab',
        'No browser tab is open. Use the CaPilot UI to open a browser tab first.'
      )
    }
    if (tabs.length === 1) {
      ...
      throw new BrowserError(
        'browser_no_tab',
        "Multiple browser tabs are open. Run 'capilot tab list' and 'capilot tab switch --index <n>' to select one."
      )
    }
    ...
    if (!guest || guest.isDestroyed()) {
      ...
      throw new BrowserError(
        ...
        "The active browser tab was closed. Run 'capilot tab list' to find remaining tabs."
      )
    }
    return guest
  }
  ...
  private async resolveRef(
    ...
    if (!state.snapshotResult) {
      throw new BrowserError(
        'browser_stale_ref',
        "No snapshot exists for this tab. Run 'capilot snapshot' first."
      )
    }
    ...
    if (!entry) {
      throw new BrowserError(
        ...
        `Element ref ${ref} was not found. Run 'capilot snapshot' to see available refs.`
      )
    }
    ...
    if (!entry.sessionId) {
      ...
      if (state.navigationId && currentNavId !== state.navigationId) {
        ...
        throw new BrowserError(
          'browser_stale_ref',
          "The page has navigated since the last snapshot. Run 'capilot snapshot' to get fresh refs."
        )
      }
    }
    ...
    try {
      ...
      throw new BrowserError(
        'browser_stale_ref',
        `Element ${ref} no longer exists in the DOM. Run 'capilot snapshot' to get fresh refs.`
      )
    }
  }
