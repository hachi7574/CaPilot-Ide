...
describe('fetchMiniMaxRateLimits', () => {
  ...
  it('sets the cookie jar and sends browser-like request headers', async () => {
    ...
    expect(sessionFromPartitionMock).toHaveBeenCalledWith('capilot-minimax-rate-limit-fetch')
    ...
    expect(init.headers['User-Agent']).not.toContain('capilot-minimax-usage')
  })
