...
// macOS convention is `~/Library/Application Support/CaPilot/logs/`; Windows and
...
function getUserDataDir(): string {
  try {
    ...
    const home = homedir()
    if (platform() === 'darwin') {
      return join(home, 'Library', 'Application Support', 'CaPilot')
    }
    if (platform() === 'win32') {
      return join(process.env.APPDATA ?? home, 'CaPilot')
    }
    return join(home, '.config', 'CaPilot')
  }
}
