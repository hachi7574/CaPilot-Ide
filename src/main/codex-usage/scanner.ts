...
export function getCodexSessionsDirectory(): string {
  // Why: CaPilot-launched Codex processes receive an CaPilot-owned CODEX_HOME, so
...
function hasLegacyCopiedSessionBridgeMarkers(): boolean {
  return existsSync(join(getOrcaManagedCodexHomePath(), '.capilot-session-copies'))
}
...
export async function attributeCodexUsageEvent(
  ...
  if (event.cwd) {
    ...
    if (worktree) {
      ...
      // Why: all-local mode should still collapse repeated off-CaPilot sessions by
