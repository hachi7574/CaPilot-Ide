...
describe('startLocalBuildFeed', () => {
  it('serves only tokenized manifest and validated artifact routes', async () => {
    const directory = await mkdtemp(join(tmpdir(), 'capilot-local-feed-'))
    const artifactPath = join(directory, 'capilot-macos-arm64.zip')
    ...
    const candidate = {
      ...
      artifacts: new Map([['capilot-macos-arm64.zip', { file: artifactFile, size: 3 }]]),
