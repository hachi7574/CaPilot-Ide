...
describe('DroidHookService', () => {
  ...
  beforeEach(() => {
    homeDir = mkdtempSync(join(tmpdir(), 'capilot-droid-home-'))
    userDataDir = mkdtempSync(join(tmpdir(), 'capilot-droid-user-data-'))
  ...
  it.skipIf(process.platform !== 'win32')(
    ...
    () => {
      const spaceHome = join(tmpdir(), 'capilot droid home with spaces')
