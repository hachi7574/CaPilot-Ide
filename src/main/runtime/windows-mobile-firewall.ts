...
const FIREWALL_RULE_NAME = 'CaPilot.MobilePairing'
const FIREWALL_RULE_DISPLAY_NAME = 'CaPilot Mobile Pairing'
...
New-NetFirewallRule -Name ${quotePowerShell(FIREWALL_RULE_NAME)} -DisplayName ${quotePowerShell(FIREWALL_RULE_DISPLAY_NAME)} -Description 'Allows CaPilot Mobile to connect to this CaPilot desktop on private networks.' -Direction Inbound -Action Allow -Enabled True -Profile Private -Protocol TCP -LocalPort ${port} -Program ${quotePowerShell(executablePath)} -EdgeTraversalPolicy Block | Out-Null`
}
