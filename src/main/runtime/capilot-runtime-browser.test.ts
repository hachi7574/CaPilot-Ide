...
import type { RuntimeBrowserCommandHost } from './capilot-runtime-browser'
    ...
    profiles: new Map([
      ...
      [
        'profile-isolated',
        {
          id: 'profile-isolated',
          scope: 'isolated',
          partition: 'persist:capilot-browser-session-profile-isolated',
          label: 'Isolated',
          source: null
        }
      ]
...
describe('RuntimeBrowserCommands browser screencast', () => {
  beforeEach(() => {
    ...
    browserSessionRegistryMock.resolveKnownPartition.mockImplementation(
      (profileId: string | null | undefined) => {
        if (!profileId) {
          return 'persist:capilot-browser'
        }
  ...
  it('waits for explicit worktree browser registration after requesting a hidden mount', async () => {
    const { RuntimeBrowserCommands } = await import('./capilot-runtime-browser')
    const send = vi.fn()
    const bridge = {
