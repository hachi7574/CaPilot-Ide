...
import { awaitRuntimeFileWatcherUnsubscribes, RuntimeFileCommands } from './capilot-runtime-files'
...
describe('RuntimeFileCommands', () => {
  ...
  it('rejects legacy paired SSH mutations before selecting a filesystem provider', async () => {
    ...
    ).rejects.toThrow('newer CaPilot client')
  ...
  describe('resolveTerminalPath', () => {
    ...
    async function tempFile(name: string, content: string): Promise<string> {
      const dir = await mkdtemp(join(tmpdir(), 'capilot-terminal-artifact-'))
      tempDirs.push(dir)
