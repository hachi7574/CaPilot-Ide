...
> = {
  permission_denied: {
    ...
    fallback:
      "CaPilot couldn't write its runtime file. Check permissions on CaPilot's data folder, then restart."
  },
  storage_unavailable: {
    key: 'runtimeRpc.startupFailure.guidance.storageUnavailable',
    fallback: 'Your disk may be full or read-only. Free up space, then restart CaPilot.'
  },
  invalid_path: {
    key: 'runtimeRpc.startupFailure.guidance.invalidPath',
    fallback:
      "CaPilot's data folder may be missing, moved, or at a path that is too long. Restore it or use a shorter path, then restart CaPilot."
  },
  address_in_use: {
    key: 'runtimeRpc.startupFailure.guidance.addressInUse',
    fallback: 'Another process may be holding the port. Restart CaPilot to try again.'
  },
  unknown: {
    key: 'runtimeRpc.startupFailure.guidance.unknown',
    fallback: 'Restart CaPilot to try again.'
  }
}
...
function createRuntimeRpcStartupFailureDialogOptions(error: unknown): MessageBoxOptions {
  ...
  return {
    ...
    cancelId: 0,
    noLink: true,
    title: translateMain('runtimeRpc.startupFailure.title', 'CaPilot CLI unavailable'),
    message: translateMain(
      'runtimeRpc.startupFailure.message',
      "CaPilot couldn't start its local command transport."
    ),
    detail: translateMain(
      'runtimeRpc.startupFailure.detail',
      'CaPilot will continue to work, but commands such as capilot status, capilot terminal, and orchestration are unavailable for this session.\n\n{{guidance}}\n\nCause: {{cause}}',
