...
describe('runtime RPC startup failure reporting', () => {
  ...
  it.each([
    ...
    const error = Object.assign(new Error('/Users/private/capilot-runtime.json'), { code })

    expect(classifyRuntimeRpcStartFailure(error)).toBe(expected)
  })

  it('classifies a code carried on a wrapped cause', () => {
    const error = new Error('failed to publish capilot-runtime.json', {
  ...
  it('walks past an unmapped wrapper code to the mapped cause', () => {
    const error = Object.assign(new Error('failed to publish capilot-runtime.json'), {
  ...
  it('records a privacy-safe telemetry event', () => {
    ...
    const error = Object.assign(new Error('/Users/private/capilot-runtime.json'), { code: 'EACCES' })
  ...
  it('shows the CLI impact and local cause', async () => {
    ...
    expect(showMessageBoxMock).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        type: 'error',
        title: 'CaPilot CLI unavailable',
        message: "CaPilot couldn't start its local command transport.",
        detail: expect.stringMatching(
          /capilot status.*capilot terminal.*orchestration.*Cause: metadata write failed/s
        )
      })
    )
  })
  ...
  it.each([
    ['EACCES', "Check permissions on CaPilot's data folder"],
    ['EPERM', "Check permissions on CaPilot's data folder"],
    ...
    ['ENOENT', "CaPilot's data folder may be missing"],
  ...
  it('falls back to a plain restart when the cause is unrecognised', async () => {
    ...
    const detail = showMessageBoxMock.mock.calls[0]?.[1]?.detail as string
    expect(detail).toContain('Restart CaPilot to try again.')
    expect(detail).not.toContain("Check permissions on CaPilot's data folder")
  })
