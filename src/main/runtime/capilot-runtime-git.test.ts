...
import { RuntimeGitCommands, type ResolvedRuntimeGitWorktree } from './capilot-runtime-git'
...
describe('RuntimeGitCommands', () => {
  ...
  it('aborts a local merge through the resolved worktree', async () => {
    const worktreePath = mkdtempSync(join(tmpdir(), 'capilot-runtime-git-'))
  ...
  // getStatus which untracked entries are CaPilot's own (issue #10451); nothing else
  ...
  it('shares one linked-issue attach across both pull-request branches', async () => {
    ...
    for (const connectionId of [undefined, 'conn-1']) {
      const worktreePath = connectionId
        ? '/home/tester/wt'
        : mkdtempSync(join(tmpdir(), 'capilot-runtime-git-'))
      if (!connectionId) {
        tempDirs.push(worktreePath)
      }
      mocks.getPullRequestDraftContext.mockResolvedValue(context)
