...
describe('runtime metadata', () => {
  ...
  it('writes and reads runtime metadata atomically', () => {
    const userDataPath = mkdtempSync(join(tmpdir(), 'capilot-runtime-metadata-'))
  ...
  it('clears the runtime metadata file', () => {
    ...
    expect(getRuntimeMetadataPath(userDataPath)).toContain('capilot-runtime.json')
  })
  ...
  it.runIf(process.platform !== 'win32')(
    ...
    () => {
      const userDataPath = mkdtempSync(join(tmpdir(), 'capilot-runtime-secure-files-'))
      tempDirs.push(userDataPath)
      ...
      for (const path of [
        join(userDataPath, 'capilot-devices.json'),
        join(userDataPath, 'capilot-e2ee-keypair.json'),
  ...
  it.runIf(process.platform !== 'win32')(
    ...
    () => {
      const userDataPath = mkdtempSync(join(tmpdir(), 'capilot-runtime-existing-secure-files-'))
      tempDirs.push(userDataPath)
      ...
      const devicesPath = join(userDataPath, 'capilot-devices.json')
      const keypairPath = join(userDataPath, 'capilot-e2ee-keypair.json')
  ...
  it('replaces oversized E2EE keypair files instead of reading them as metadata', () => {
    const userDataPath = mkdtempSync(join(tmpdir(), 'capilot-runtime-large-keypair-'))
    tempDirs.push(userDataPath)
    const keypairPath = join(userDataPath, 'capilot-e2ee-keypair.json')
