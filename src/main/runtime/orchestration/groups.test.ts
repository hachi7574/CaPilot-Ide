...
describe('resolveGroupAddress', () => {
  ...
  describe('agent name groups', () => {
    ...
    // Why: these are the titles cursor-agent natively emits and the ones CaPilot synthesizes
    // from Cursor hooks, so each must resolve.
    it('matches native and CaPilot-synthesized Cursor titles', () => {
      const terminals = [
        makeSummary('coordinator', { title: 'Coordinator' }),
    ...
    it('does not match another agent whose task title mentions a text cursor', () => {
      const terminals = [
        ...
        makeSummary('term_shell', { title: 'Terminal Cursor and CaPilot slows down' })
      ]
