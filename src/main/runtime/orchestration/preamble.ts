...
export type PreambleParams = {
  ...
  workerHandle: string
  devMode?: boolean
  // Why: packaged WSL panes install the scoped launcher as `capilot-ide`;
  // other execution hosts keep their existing bare `capilot` bridge.
  ...
  // shells have no agent prompt for CaPilot to reuse.
...
// Why: the dispatch preamble teaches agents about CaPilot's CLI commands for
...
export function buildDispatchPreamble(params: PreambleParams): string {
  // Why: in dev mode, agents must use capilot-dev to connect to the dev runtime's
  // socket. Without this, agents inside the dev Electron app would call the
  // production CLI and talk to the wrong CaPilot instance (Section 6.4).
  const cli = params.devMode ? 'capilot-dev' : (params.cliCommand ?? 'capilot')
  ...
  const header = `You are working inside CaPilot, a multi-agent IDE. You are a dispatched worker.
...
prompt for CaPilot to reuse; if the coordinator has more for you it will
