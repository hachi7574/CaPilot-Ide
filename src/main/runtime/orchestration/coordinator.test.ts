...
function createMockRuntime(): CoordinatorRuntime & {
  ...
  cliCommand: 'capilot' | 'capilot-ide'
  ...
  const mock = {
    ...
    cliCommand: 'capilot' as 'capilot' | 'capilot-ide',
...
describe('Coordinator', () => {
  ...
  it('dispatches a ready task to an available terminal', async () => {
    db = new OrchestrationDb(':memory:')
    const runtime = createMockRuntime()
    runtime.cliCommand = 'capilot-ide'
    ...
    expect(runtime.sentMessages[0].text).toContain('capilot-ide orchestration send')
  })
