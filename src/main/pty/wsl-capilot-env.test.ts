...
} from './wsl-capilot-env'

describe('addOrcaWslInteropEnv', () => {
  it('marks the CaPilot terminal handle for Windows to WSL env import', () => {
  ...
  it('marks OMP status and hook env for Windows to WSL import', () => {
    const env: Record<string, string> = {
      ...
      ORCA_USER_DATA_PATH: 'C:\\Users\\jin\\AppData\\Roaming\\CaPilot',
      ORCA_CLI_COMMAND: 'capilot-ide',
      ORCA_OMP_STATUS_EXTENSION: 'C:\\Users\\jin\\.omp\\agent\\extensions\\capilot-agent-status.ts',
  ...
  it('path-translates a Windows hook endpoint but passes a guest-side one untouched', () => {
    const windowsEnv: Record<string, string> = {
      ORCA_AGENT_HOOK_ENDPOINT: 'C:\\Users\\jin\\AppData\\Roaming\\CaPilot\\agent-hooks\\endpoint.cmd'
    }
    ...
    const guestEnv: Record<string, string> = {
      ORCA_AGENT_HOOK_ENDPOINT: '/home/jin/.capilot-wsl/agent-hooks/port-4567/endpoint.env'
    }
  ...
  it('crosses a guest-side OpenCode config overlay untranslated (/u)', () => {
    const env: Record<string, string> = {
      OPENCODE_CONFIG_DIR: '/home/jin/.capilot-relay/opencode-overlays/abc',
      ORCA_OPENCODE_CONFIG_DIR: '/home/jin/.capilot-relay/opencode-overlays/abc'
    }
    addOrcaWslInteropEnv(env)
  ...
  it('never crosses a Windows OpenCode config dir into the guest', () => {
    ...
    // would adopt CaPilot's Windows overlay as its config root.
    const env: Record<string, string> = {
      OPENCODE_CONFIG_DIR: 'C:\\Users\\jin\\AppData\\Roaming\\CaPilot\\opencode-overlays\\abc',
      ORCA_OPENCODE_CONFIG_DIR: 'C:\\Users\\jin\\AppData\\Roaming\\CaPilot\\opencode-overlays\\abc'
    }
    addOrcaWslInteropEnv(env)
