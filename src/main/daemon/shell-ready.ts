...
const SHELL_READY_MARKER = '\\033]777;capilot-shell-ready\\007'
...
function getShellReadyWrapperRoot(): string {
  ...
  return join(userDataPath || tmpdir(), userDataPath ? 'shell-ready' : 'capilot-shell-ready')
}

// Why: if our own process inherited ZDOTDIR from a parent shell that was
// itself an CaPilot PTY (e.g. the user launched CaPilot from a terminal inside a
// running CaPilot), that ZDOTDIR points at an CaPilot shell-ready wrapper dir.
...
//
// Any path component ending in `/shell-ready/zsh` is an CaPilot wrapper dir
// (regardless of whether it came from this daemon's userData, a packaged
// CaPilot, or a different dev build). Treat it as if ZDOTDIR were unset so the
...
export function getDaemonBashShellReadyRcfileContent(): string {
  return `# CaPilot daemon bash shell-ready wrapper
...
# Why: enable bracketed paste so CaPilot can deliver a multiline startup prompt as
...
__orca_restore_agent_teams_path
# Why: user startup files may set the default OpenCode config after CaPilot's
# spawn env; restore the CaPilot-managed config dir before the first prompt.
[[ -n "\${ORCA_OPENCODE_CONFIG_DIR:-}" ]] && export OPENCODE_CONFIG_DIR="\${ORCA_OPENCODE_CONFIG_DIR}"
[[ -n "\${ORCA_MIMOCODE_HOME:-}" ]] && export MIMOCODE_HOME="\${ORCA_MIMOCODE_HOME}"
${getPosixOmpShellWrapper()}
# Why: Codex must keep using CaPilot's runtime CODEX_HOME after profile scripts.
...
__orca_osc133_precmd() {
  ...
  # displaced by one of CaPilot's own hooks.
...
# while its re-arm can no longer silence CaPilot's command-start signal.
...
export function getDaemonZshShellReadyRcfileContent(): string {
  return `# CaPilot daemon zsh shell-ready wrapper
${getZshStartupFileSourceBlock({
  fileName: '.zshrc',
...
# Why: prepend so CaPilot captures $? before user prompt hooks can overwrite it.
...
function ensureShellReadyWrappers(): void {
  ...
  const zshProfile = `# CaPilot daemon zsh shell-ready wrapper
${getZshStartupFileSourceBlock({ fileName: '.zprofile' })}
`
  const zshRc = getDaemonZshShellReadyRcfileContent()
  const zshLogin = `# CaPilot daemon zsh shell-ready wrapper
