...
function buildInstallFixture(root: string): void {
  ...
  writeFileSync(join(root, 'CaPilot.exe'), 'exe-bytes')
...
beforeEach(() => {
  ...
  setProcessProp('execPath', join(installDir, 'CaPilot.exe'))
...
describe('buildDaemonHostManifest', () => {
  it('mirrors the win-unpacked layout: exe + data blobs + resources tree, no GPU DLLs', () => {
    ...
    // so the NSIS updater's name-based `taskkill /IM CaPilot.exe` can't kill it.
    expect(byDest.get('capilot-terminal-daemon.exe')?.kind).toBe('file')
    expect(byDest.has('CaPilot.exe')).toBe(false)
    const exeOp = ops.find((op) => op.sourcePath === 'C:\\app\\CaPilot.exe')
    expect(exeOp?.destRel).not.toBe('CaPilot.exe')
...
describe('materializeRelocatedDaemonHost', () => {
  it('copies the tree, writes the marker, and returns mirrored fork paths', () => {
    ...
    const dest = join(localAppDataDir, 'CaPilot', 'daemon-host', '9.9.9')
    expect(result?.execPath).toBe(join(dest, 'capilot-terminal-daemon.exe'))
  ...
  it('fails open on a missing required input, leaving no dest or staging dir', () => {
    ...
    const hostRoot = join(localAppDataDir, 'CaPilot', 'daemon-host')
  ...
  it('returns null off win32', () => {
    ...
    expect(existsSync(join(localAppDataDir, 'CaPilot', 'daemon-host'))).toBe(false)
  })
})
...
describe('getRelocatedDaemonHost', () => {
  it('returns null when the marker version does not match the current version', () => {
    ...
    writeFileSync(join(dest, 'CaPilot.exe'), 'exe')
...
describe('pruneOldDaemonHosts', () => {
  it('removes unpinned non-current version dirs, keeping current and pinned', () => {
    const root = join(localAppDataDir, 'CaPilot', 'daemon-host')
