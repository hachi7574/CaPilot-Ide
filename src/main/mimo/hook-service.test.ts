...
describe('MimoCodeHookService buildPtyEnv', () => {
  ...
  beforeEach(() => {
    userDataDir = mkdtempSync(join(tmpdir(), 'capilot-mimocode-userdata-'))
    ...
    mimocodeHome = mkdtempSync(join(tmpdir(), 'capilot-mimocode-home-'))
    ...
    writeFileSync(join(configDir, 'plugins', 'capilot-mimocode-status.js'), 'USER PLUGIN')
  })
  ...
  it('mirrors user config into shared overlay and installs CaPilot status plugin', () => {
    ...
    const orcaPlugin = join(overlayHome, 'config', 'plugins', 'capilot-mimocode-status.js')
    expect(existsSync(orcaPlugin)).toBe(true)
    expect(readFileSync(orcaPlugin, 'utf8')).toContain('/hook/mimo-code')

    expect(
      readFileSync(join(mimocodeHome, 'config', 'plugins', 'capilot-mimocode-status.js'), 'utf8')
    ).toBe('USER PLUGIN')
  })
  ...
  it('reuses the overlay home on a second buildPtyEnv call', () => {
    ...
    expect(
      readFileSync(join(overlayHome, 'config', 'plugins', 'capilot-mimocode-status.js'), 'utf8')
