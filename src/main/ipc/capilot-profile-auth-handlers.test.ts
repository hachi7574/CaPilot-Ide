...
vi.mock('../capilot-profiles/profile-index-store', () => ({
...
vi.mock('../capilot-profiles/profile-project-transfer', () => ({
  transferOrcaProfileProject: vi.fn()
}))

vi.mock('../capilot-profiles/profile-cloud-service', () => ({
...
import { registerOrcaProfileHandlers } from './capilot-profiles'
...
describe('registerOrcaProfileHandlers auth channels', () => {
  ...
  it('returns auth status for the current profile', async () => {
    ...
    expect(getCurrentOrcaProfileAuthStatusMock).toHaveBeenCalledWith('/tmp/capilot-user-data')
  })
  ...
  it('connects and signs out the current profile through the cloud service', async () => {
    ...
    expect(connectCurrentOrcaProfileMock).toHaveBeenCalledWith('/tmp/capilot-user-data')
    expect(signOutCurrentOrcaProfileMock).toHaveBeenCalledWith('/tmp/capilot-user-data')
  })
  ...
  it('refreshes profile auth through the cloud service', async () => {
    ...
    expect(refreshCurrentOrcaProfileAuthMock).toHaveBeenCalledWith('/tmp/capilot-user-data')
  })
  ...
  it('validates organization selection before calling the cloud service', async () => {
    ...
    expect(selectCurrentOrcaProfileOrgMock).toHaveBeenCalledWith('/tmp/capilot-user-data', 'org-1')
  ...
  it('creates cloud-linked profiles with trimmed optional args', async () => {
    ...
    expect(createCloudLinkedOrcaProfileMock).toHaveBeenCalledWith('/tmp/capilot-user-data', {
      orgId: 'org-1',
      name: 'Acme'
    })
  })
