...
vi.mock('../capilot-profiles/profile-storage-paths', () => ({
  getProfileUserDataPath: () => '/tmp/capilot-user-data'
}))

vi.mock('../capilot-profiles/profile-cloud-org-members-service', () => ({
...
describe('registerOrcaProfileOrgMemberHandlers', () => {
  ...
  it('forwards a valid invite to the service with a trimmed email', async () => {
    ...
    expect(inviteOrcaProfileOrgMemberMock).toHaveBeenCalledWith('/tmp/capilot-user-data', {
      orgId: 'org-1',
      email: 'new@example.com',
      role: 'admin'
    })
  ...
  it('forwards remove and revoke with validated args', async () => {
    ...
    expect(removeOrcaProfileOrgMemberMock).toHaveBeenCalledWith('/tmp/capilot-user-data', {
      orgId: 'org-1',
      userId: 'user-2'
    })
    await invoke('orcaProfiles:orgInviteRevoke', { orgId: 'org-1', email: 'gone@b.com' })
    expect(revokeOrcaProfileOrgInviteMock).toHaveBeenCalledWith('/tmp/capilot-user-data', {
      orgId: 'org-1',
      email: 'gone@b.com'
    })
  })
