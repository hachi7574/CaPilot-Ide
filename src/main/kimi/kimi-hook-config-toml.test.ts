...
const COMMAND =
  "if [ -x '/home/u/.capilot/agent-hooks/kimi-hook.sh' ]; then /bin/sh '/home/u/.capilot/agent-hooks/kimi-hook.sh'; fi"
...
describe('kimi managed hooks TOML block', () => {
  ...
  it('is idempotent — reinstalling does not duplicate the block', () => {
    ...
    const markerCount = (twice.match(/capilot-managed-kimi-hooks \(/g) ?? []).length
