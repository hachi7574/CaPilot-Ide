...
// so CaPilot manages only its own marker-delimited block: install rewrites the
...
// Why: mirror the Claude-compatible events CaPilot normalizes for status. Kimi uses
...
const BLOCK_START = '# >>> capilot-managed-kimi-hooks (managed by CaPilot; do not edit) >>>'
const BLOCK_END = '# <<< capilot-managed-kimi-hooks <<<'
...
// CaPilot generates; escape both defensively anyway.
...
// CaPilot-managed script (by filename, so a moved userData path is still swept).
