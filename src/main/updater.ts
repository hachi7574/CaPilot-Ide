...
function deferHeadlessServeInstall(phase: 'download' | 'install', version: string): boolean {
  ...
  if (lastInstallDeferralVersion[phase] !== diagnosticVersion) {
    ...
    recordUpdaterLifecycle(
      ...
      {
        level: 'warn',
        message: 'Update install deferred while hosting capilot serve'
      }
    )
  }
  sendErrorStatus(
    'This capilot serve process was not started by an update-capable supervisor. Keep it running and update CaPilot through its service manager.',
    true
  )
  return true
}
  ...
  try {
    await withUpdaterSpan({ stage: 'install' }, async (span) => {
      ...
      if (
        ...
        sendErrorStatus(
          'Could not prepare the supervised server restart. CaPilot remains running.',
          true
        )
...
function scheduleAutomaticUpdateCheck(delayMs: number): void {
  ...
  autoUpdateCheckTimer = setTimeout(() => {
    // Why: CaPilot runs for days, so keep the next background check scheduled in the main process rather than tying it to relaunches or renderer lifetime.
...
const LINUX_PACKAGE_RECOVERY_MESSAGES: Record<LinuxPackageRecoveryUnavailableReason, string> = {
  ...
  'hash-mismatch':
    'The downloaded package no longer matches the verified release, so CaPilot will not hand it to a package manager. Download the update again, or get it from the official release page.',
  'read-failed':
    'CaPilot could not read the downloaded package. Download the update again, or get it from the official release page.',
  'no-sudo':
    'No sudo command was found in the system directories, so CaPilot cannot build a safe install command. Show the package and install it with your package manager.',
  'no-package-manager':
    'No supported package manager was found in the system directories, so CaPilot cannot build a safe install command. Show the package and install it with your package manager.',
  // Defensive: capture only ever tracks absolute cache paths, so this reports a bug rather than a machine state.
  'invalid-package-path':
    'The downloaded package is not at a usable path, so CaPilot cannot build a safe install command. Show the package and install it with your package manager.'
}
