...
internal static class OrcaRemoteCliLauncher
{
    private static int Main(string[] args)
    {
        try
        {
            ...
            {
                Console.Error.WriteLine("CaPilot SSH CLI bridge cannot find Node.js at \"{0}\"", nodePath);
                return 1;
            }
            if (!File.Exists(relayPath))
            {
                Console.Error.WriteLine("CaPilot SSH CLI bridge cannot find the relay at \"{0}\"", relayPath);
                return 1;
            }
        ...
        {
            Console.Error.WriteLine("Unable to start the CaPilot SSH CLI bridge: {0}", error.Message);
            return 1;
        }
    }
    ...
    private static string BuildArguments(string relayPath, string socketPath, string credentialFile, string[] args)
    {
        ...
        AppendArgument(commandLine, "--capilot-cli");
...
function createWindowsLauncherCompileCommand(
  ...
  return powerShellCommand(
    [
      ...
      `if (-not (Test-Path -LiteralPath ${powerShellLiteral(launcherPath)} -PathType Leaf)) { Write-Error 'The CaPilot SSH CLI launcher compiler produced no executable.'; exit 1 }`,
      // Why: remove the legacy %* bridge only after a successful compile, so a
      // host missing csc.exe keeps its existing CLI (capilot.exe shadows capilot.cmd).
...
export function createRemoteCliInstallPlan(env: RemoteCliInstallEnv): RemoteCliInstallPlan {
  if (isWindowsRemoteHost(env.hostPlatform)) {
    const launcherFileName = 'capilot.exe'
    const sourceFileName = 'capilot-launcher.cs'
    ...
    const legacyShimPath = joinRemotePath(env.hostPlatform, env.binDir, 'capilot.cmd')
  ...
  const launcherPath = joinRemotePath(env.hostPlatform, env.binDir, 'capilot')
  return {
    launcherPath,
    files: [
      {
        ...
        contents: [
          ...
          '  echo "CaPilot SSH CLI bridge cannot find the relay socket: $ORCA_RELAY_SOCKET_PATH" >&2',
          '  exit 1',
          'fi',
          'exec "$ORCA_RELAY_NODE_PATH" "$ORCA_RELAY_DIR/relay.js" --sock-path "$ORCA_RELAY_SOCKET_PATH" --credential-file "$ORCA_RELAY_CREDENTIAL_FILE" --capilot-cli "$@"',
          ''
        ].join('\n')
      }
    ],
