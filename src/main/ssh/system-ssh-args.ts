...
export function buildSshArgs(target: SshTarget, options?: SystemSshBuildArgsOptions): string[] {
  ...
  if (options?.gssapiOnly) {
    ...
    // credential prompt; failure belongs to CaPilot's existing ssh2 prompt path.
...
function shouldDisableOrcaControlMaster(
  ...
  // Why: unresolved ssh_config aliases could otherwise share one CaPilot socket
...
function hasUserConfiguredControlMaster(
  ...
  // created, but they do not create the setup-burst master CaPilot needs.
  return (
