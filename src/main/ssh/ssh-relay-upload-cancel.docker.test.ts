...
const REMOTE_REPO = '/tmp/capilot-pr-10207-real-repo'
...
function startTarget(): TargetFixture {
  ...
  const tempDir = mkdtempSync(join(tmpdir(), 'capilot-pr10207-ssh-'))
  ...
  const containerName = `capilot-pr10207-${randomUUID().slice(0, 12)}`
  run(
    'docker',
    [
      'run',
...
function readInventory(fixture: TargetFixture, remoteRelayDir: string): RemoteInventory {
  const stagePool = '/root/.capilot-remote/.upload-stages'
...
describe.skipIf(!RUN_REVIEW_ORACLE)('SSH relay upload cancellation recovery', () => {
  ...
  it('aborts a live SFTP upload after remote bytes arrive without creating the shared lock', async () => {
    ...
    const remoteRelayDir = `/root/.capilot-remote/relay-${relayVersion}`
    ...
    const partialRemoteBytes = Number(
      dockerExec(
        activeFixture,
        "find /root/.capilot-remote/.upload-stages -type f -path '*/slot-*/payload/relay.js' -printf '%s\\n'"
      )
    )
  ...
  it('recovers cancellation with bounded safe reclamation and bounded real version GC', async () => {
    ...
    if (expected === 'recovered') {
      ...
      const foreignTarget = '/root/capilot-pr10207-foreign-stage-target'
      const stagePool = '/root/.capilot-remote/.upload-stages'
      const symlinkStage = `${stagePool}/slot-2`
      const ownerMarker = '.capilot-upload-owner'
      const identityMarker = '.capilot-upload-identity'
      dockerExec(
        activeFixture,
        [
          ...
          `i=0; while [ "$i" -lt 15197 ]; do mkdir ${shellQuote(`/root/.capilot-remote/relay-${relayVersion}.upload-scale-`)}"$i"; i=$((i + 1)); done`
        ].join(' && ')
      )
      ...
      const listing = await execCommand(
        cleanupConnection,
        listRelayBaseDirsCommand(getRemoteHostPlatform('linux-arm64'), '/root/.capilot-remote')
      )
      ...
      const scaleEntries = Number(
        dockerExec(
          activeFixture,
          `find /root/.capilot-remote -mindepth 1 -maxdepth 1 -type d -name ${shellQuote(`relay-${relayVersion}.upload-scale-*`)} | wc -l`
        )
      )
    ...
    if (expected === 'recovered') {
      ...
      expect(finalInventory!.uploadStages).toEqual([
        '/root/.capilot-remote/.upload-stages/slot-0',
        '/root/.capilot-remote/.upload-stages/slot-0.original',
        '/root/.capilot-remote/.upload-stages/slot-2'
      ])
    } else {
