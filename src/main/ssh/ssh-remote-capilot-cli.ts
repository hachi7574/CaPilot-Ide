...
const HOST_INTERACTIVE_COMMANDS: Record<string, string> = {
  serve:
    'capilot serve starts a foreground headless CaPilot server and cannot run through the SSH relay bridge. Run it directly on the machine that should host CaPilot.',
  'claude-teams':
    'capilot claude-teams starts an interactive Claude Code session and cannot run through the SSH relay bridge. Run it in a terminal on the CaPilot host machine.',
  'agent-teams-tmux':
    'capilot agent-teams-tmux is a tmux pane shim for the CaPilot host machine and cannot run through the SSH relay bridge.',
  'account add':
    'capilot account add runs an interactive agent login and cannot run through the buffered SSH relay bridge. Run it directly in a terminal on the CaPilot host machine.'
}
...
async function dispatchRemoteCli(
  ...
  switch (command) {
    ...
    case 'orchestration check':
      return await call(
        ...
        'orchestration.check',
        {
          ...
          compatibilityCliCommand: 'capilot',
    ...
    default:
      ...
      throw new Error(
        `Unsupported SSH CaPilot CLI command: ${command} (full CaPilot CLI bridge unavailable: ${passthroughFailureReason})`
      )
  }
}
