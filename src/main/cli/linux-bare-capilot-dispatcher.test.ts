...
import { installLinuxBareOrcaDispatcher } from './linux-bare-capilot-dispatcher'

const created: string[] = []

async function makeFixture(): Promise<{ homePath: string; resourcesPath: string }> {
  const root = await mkdtemp(join(tmpdir(), 'capilot-bare-dispatcher-'))
  created.push(root)
  const resourcesPath = join(root, 'resources')
  // The bundled capilot-ide launcher must exist for the dispatcher to be written.
  await mkdir(join(resourcesPath, 'bin'), { recursive: true })
  await writeFile(join(resourcesPath, 'bin', 'capilot-ide'), '#!/usr/bin/env bash\n', 'utf8')
...
describe('installLinuxBareOrcaDispatcher', () => {
  it('writes an executable bare-capilot dispatcher that execs the bundled capilot-ide launcher', async () => {
    ...
    const expectedTarget = join(resourcesPath, 'bin', 'capilot-ide')
    expect(result.state).toBe('installed')
    expect(result.target).toBe(expectedTarget)
    expect(result.dispatcherPath).toBe(join(homePath, '.local', 'bin', 'capilot'))
  ...
  it('quotes a resources path containing spaces so the exec line cannot be split', async () => {
    const root = await mkdtemp(join(tmpdir(), 'capilot-bare-dispatcher-space-'))
    created.push(root)
  ...
  it('execs the stable AppImage (not the ephemeral mount) when running from an AppImage', async () => {
    const { homePath, resourcesPath } = await makeFixture()
    const appImagePath = join(homePath, 'Applications', 'CaPilot.AppImage')
  ...
  it('skips (does not clobber) a user-owned capilot already at ~/.local/bin', async () => {
    const { homePath, resourcesPath } = await makeFixture()
    const dispatcherPath = join(homePath, '.local', 'bin', 'capilot')
  ...
  it('skips when the bundled capilot-ide launcher is missing from the build', async () => {
    const root = await mkdtemp(join(tmpdir(), 'capilot-bare-dispatcher-nolauncher-'))
    created.push(root)
