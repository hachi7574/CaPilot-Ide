...
function makeHostStatus(
  launcherPath = 'C:\\Users\\me\\AppData\\Local\\Programs\\CaPilot\\resources\\bin\\capilot.exe'
) {
  return {
    platform: 'win32',
    commandName: 'capilot',
    commandPath: launcherPath,
    pathDirectory: 'C:\\Users\\me\\AppData\\Local\\Programs\\CaPilot\\resources\\bin',
    pathConfigured: true,
    launcherPath,
...
set -euo pipefail
# CaPilot managed WSL CLI launcher
# ORCA_WIN_LAUNCHER_B64=QzpcUHJvZ3JhbSBGaWxlc1xPcmNhXHJlc291cmNlc1xiaW5cb3JjYS5jbWQ=
ORCA_WIN_LAUNCHER='C:\\Program Files\\CaPilot\\resources\\bin\\capilot.cmd'
ORCA_BRIDGE_PS1='/home/alice/.local/share/capilot/capilot-wsl-bridge.ps1'
...
function createWslRunner(
  ...
  const commandPath = '/home/alice/.local/bin/capilot-ide'
  const legacyCommandPath = '/home/alice/.local/bin/capilot'
  const bridgePath = '/home/alice/.local/share/capilot/capilot-wsl-bridge.ps1'
  ...
  const runner = vi.fn(async (_distro: string, command: string) => {
    ...
    if (command.includes('cat > "$command_tmp"')) {
      ...
      if (
        ...
        !files.get(bridgePath)?.includes('# CaPilot managed WSL CLI PowerShell bridge')
      ) {
      ...
      if (files.get(legacyCommandPath)?.includes('# CaPilot managed WSL CLI launcher')) {
    ...
    if (command.includes('rm -f')) {
      ...
      if (
        ...
        files.get(legacyCommandPath)?.includes('# CaPilot managed WSL CLI launcher')
      ) {
        files.delete(legacyCommandPath)
      }
      return ''
...
describe('WslCliInstaller', () => {
  ...
  it('installs a WSL launcher that forwards to the Windows CaPilot launcher', async () => {
    ...
    await expect(installer.getStatus()).resolves.toMatchObject({
      state: 'not_installed',
      commandPath: '/home/alice/.local/bin/capilot-ide'
    })
    ...
    expect(wsl.getFile()).toBe(
      _internals.buildWslLauncher(
        'C:\\Users\\me\\AppData\\Local\\Programs\\CaPilot\\resources\\bin\\capilot.exe',
        '/home/alice/.local/share/capilot/capilot-wsl-bridge.ps1'
      )
    )
    ...
    expect(installCommand).toContain("legacy_command_path='/home/alice/.local/bin/capilot'")
  ...
  it('derives the shared WSL bridge path for current and legacy command names', () => {
    expect(_internals.getBridgePathFromCommandPath('/home/alice/.local/bin/capilot-ide')).toBe(
      '/home/alice/.local/share/capilot/capilot-wsl-bridge.ps1'
    )
    expect(_internals.getBridgePathFromCommandPath('/home/alice/.local/bin/capilot')).toBe(
      '/home/alice/.local/share/capilot/capilot-wsl-bridge.ps1'
    )
  })
  ...
  it('reports installed WSL launchers whose bin directory is missing from PATH', async () => {
    ...
    const installer = new WslCliInstaller({
      platform: 'win32',
      distro: 'Ubuntu',
      hostInstaller: { getStatus: async () => makeHostStatus('C:\\CaPilot\\capilot.cmd') },
      wslRunner: wsl.runner
    })
  ...
  it('accepts current managed WSL scripts with an extra heredoc trailing newline', async () => {
    ...
    const installer = new WslCliInstaller({
      ...
      wslRunner: async (distro, command) => {
        if (command.includes('cat /home/alice/.local/share/capilot/capilot-wsl-bridge.ps1')) {
    ...
    await expect(installer.getStatus()).resolves.toMatchObject({
      state: 'installed',
      currentTarget: 'C:\\CaPilot\\capilot.cmd'
    })
  })
  ...
  it('generates a launcher that forwards arguments through a PowerShell file bridge', () => {
    const launcher = _internals.buildWslLauncher(
      'C:\\Program Files\\CaPilot\\capilot.cmd',
      '/home/alice/.local/share/capilot/capilot-wsl-bridge.ps1'
    )
    ...
    expect(launcher).toContain(
      'CaPilot WSL CLI requires Windows interop and could not find powershell.exe.'
    )
  ...
  it('marks stale managed launchers that point at the old app bin instead of packaged resources', async () => {
    const oldLauncher = _internals.buildWslLauncher(
      'C:\\Users\\me\\AppData\\Local\\Programs\\CaPilot\\bin\\capilot.cmd',
      '/home/alice/.local/share/capilot/capilot-wsl-bridge.ps1'
    )
    ...
    await expect(installer.getStatus()).resolves.toMatchObject({
      state: 'stale',
      currentTarget: 'C:\\Users\\me\\AppData\\Local\\Programs\\CaPilot\\bin\\capilot.cmd',
      launcherPath: 'C:\\Users\\me\\AppData\\Local\\Programs\\CaPilot\\resources\\bin\\capilot.exe'
    })

    await expect(installer.install()).resolves.toMatchObject({
      state: 'installed',
      currentTarget: 'C:\\Users\\me\\AppData\\Local\\Programs\\CaPilot\\resources\\bin\\capilot.exe'
    })
  })

  it('repairs the frozen pre-rc4 registration so orchestration send/reply reach native rc4', async () => {
    const nativeLauncher = 'C:\\Program Files\\CaPilot\\resources\\bin\\capilot.exe'
    ...
    const simulateRc4Launch = (args: string[]): number => {
      ...
      return target?.toLowerCase().endsWith('capilot.cmd') &&
  ...
  it('repairs a managed launcher whose bridge is missing, but preserves a conflicting bridge', async () => {
    const nativeLauncher = 'C:\\CaPilot\\resources\\bin\\capilot.exe'
    ...
    const staleBridge = createWslRunner(PRE_RC4_MANAGED_WSL_LAUNCHER, true, {
      initialBridge: '# CaPilot managed WSL CLI PowerShell bridge\nWrite-Output "stale"\n'
    })
  ...
  it('moves a legacy-only managed registration to capilot-ide without touching unmanaged names', async () => {
    const nativeLauncher = 'C:\\CaPilot\\resources\\bin\\capilot.exe'
  ...
  it('keeps the pre-rc4 files on a transactional replacement failure', async () => {
    ...
    const installer = new WslCliInstaller({
      ...
      distro: 'Ubuntu',
      hostInstaller: {
        getStatus: async () => makeHostStatus('C:\\Program Files\\CaPilot\\resources\\bin\\capilot.exe')
      },
      wslRunner: wsl.runner
    })
    ...
    // would leave a window where a concurrent shell finds no capilot-ide at all.
    expect(installCommand).not.toContain('command_backup')
    expect(installCommand).not.toContain(`mv -f '/home/alice/.local/bin/capilot-ide'`)
  })

  it.skipIf(process.platform === 'win32')(
    'rolls both files back when the command replacement fails after the bridge move',
    async () => {
      const root = await mkdtemp(join(tmpdir(), 'capilot-wsl-cli-rollback-'))
      const home = join(root, 'home with spaces')
      const commandPath = join(home, '.local', 'bin', 'capilot-ide')
      const bridgePath = join(home, '.local', 'share', 'capilot', 'capilot-wsl-bridge.ps1')
      const bridge = _internals.buildWslBridgeScript()
      await mkdir(join(home, '.local', 'bin'), { recursive: true })
      await mkdir(join(home, '.local', 'share', 'capilot'), { recursive: true })
  ...
  it('is idempotent after repairing an old managed registration', async () => {
    const nativeLauncher = 'D:\\Custom CaPilot\\resources\\bin\\capilot.exe'
    const wsl = createWslRunner(PRE_RC4_MANAGED_WSL_LAUNCHER)
    const installer = new WslCliInstaller({
      platform: 'win32',
      distro: 'Ubuntu',
    ...
    expect(wsl.getFile()).toContain("ORCA_WIN_LAUNCHER='D:\\Custom CaPilot\\resources\\bin\\capilot.exe'")
  })
