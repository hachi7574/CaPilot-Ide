...
import { buildBareOrcaCliScript } from './linux-bare-capilot-dispatcher'

const SHIM_DIR_NAME = 'linux-capilot-cli-shim'
...
// Why: on Linux the CLI installs as `capilot-ide` so it never shadows the GNOME
// CaPilot screen reader at /usr/bin/capilot — but agent-facing surfaces (skills,
// dispatch preambles, CLI hints) all invoke bare `capilot`, so on stock Ubuntu an
// agent inside an CaPilot terminal would launch the screen reader instead
// (stablyai/orca#7904). Prepending this userData-scoped shim dir to managed-PTY
// PATH makes bare `capilot` resolve to the CaPilot CLI inside CaPilot terminals only,
...
export function ensureLinuxTerminalOrcaCliShimDir(
  ...
  const shimPath = join(shimDir, 'capilot')
  try {
