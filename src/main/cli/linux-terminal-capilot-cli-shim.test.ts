...
import { ensureLinuxTerminalOrcaCliShimDir } from './linux-terminal-capilot-cli-shim'

const created: string[] = []

async function makeFixture(): Promise<{ userDataPath: string; resourcesPath: string }> {
  const root = await mkdtemp(join(tmpdir(), 'capilot-terminal-cli-shim-'))
  created.push(root)
  const resourcesPath = join(root, 'resources')
  // The bundled capilot-ide launcher must exist for the shim to be written.
  mkdirSync(join(resourcesPath, 'bin'), { recursive: true })
  writeFileSync(join(resourcesPath, 'bin', 'capilot-ide'), '#!/usr/bin/env bash\n', 'utf8')
...
describe('ensureLinuxTerminalOrcaCliShimDir', () => {
  it('writes an executable bare-capilot shim that execs the bundled capilot-ide launcher', async () => {
    ...
    expect(shimDir).toBe(join(userDataPath, 'linux-capilot-cli-shim'))
    const content = readFileSync(join(shimDir!, 'capilot'), 'utf8')
    // Single-quoted so a resources path with shell metacharacters can't break out.
    expect(content).toContain(`exec '${join(resourcesPath, 'bin', 'capilot-ide')}' "$@"`)
    const mode = statSync(join(shimDir!, 'capilot')).mode & 0o777
  ...
  it('memoizes per userDataPath and re-asserts the exec bit for a stale shim', async () => {
    ...
    const shimPath = join(first!, 'capilot')
    ...
    const root = await mkdtemp(join(tmpdir(), 'capilot-terminal-cli-shim-2-'))
    created.push(root)
    const otherUserData = join(root, 'user-data')
    mkdirSync(join(otherUserData, 'linux-capilot-cli-shim'), { recursive: true })
    writeFileSync(join(otherUserData, 'linux-capilot-cli-shim', 'capilot'), 'stale contents', 'utf8')
    chmodSync(join(otherUserData, 'linux-capilot-cli-shim', 'capilot'), 0o644)
    ...
    const healedPath = join(healed!, 'capilot')
    expect(readFileSync(healedPath, 'utf8')).toContain('capilot-ide')
  ...
  it('execs the stable AppImage (not the ephemeral mount) when running from an AppImage', async () => {
    const { userDataPath, resourcesPath } = await makeFixture()
    const appImagePath = join(userDataPath, 'Applications', 'CaPilot.AppImage')
  ...
  it('returns null (and does not memoize) when the bundled launcher is missing', async () => {
    const root = await mkdtemp(join(tmpdir(), 'capilot-terminal-cli-shim-missing-'))
    created.push(root)
    ...
    expect(recovered).toBe(join(userDataPath, 'linux-capilot-cli-shim'))
  })
})
