...
// our own file idempotently but never clobber a user's own ~/.local/bin/capilot.
const DISPATCHER_MARKER = '# capilot-serve-bare-capilot-dispatcher'

export type LinuxBareOrcaDispatcherOptions = {
  /** Packaged app resources root; the bundled `capilot-ide` launcher lives under it. */
...
export type LinuxBareOrcaDispatcherResult = {
  ...
  /** What the dispatcher execs: the stable AppImage, or the bundled capilot-ide. */
  target: string | null
}

// Why: on Linux the CLI installs as `capilot-ide`, not bare `capilot`, to avoid
// shadowing GNOME CaPilot's /usr/bin/capilot. But the Claude Team launcher typed into
// the initial managed terminal invokes the literal `capilot claude-teams`, so a
// headless serve box needs a bare-`capilot` dispatcher on the managed-terminal PATH
...
export async function installLinuxBareOrcaDispatcher(
  ...
  const dispatcherPath = join(options.homePath ?? homedir(), '.local', 'bin', 'capilot')
  ...
  // `capilot` untouched rather than silently clobbering it on every serve start.
...
/** Bare-`capilot` script that execs the CaPilot CLI: the stable AppImage when running
 *  from one, otherwise the bundled `capilot-ide` launcher. Shared by the serve
