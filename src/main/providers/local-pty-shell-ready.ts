...
const SHELL_READY_MARKER_ESCAPED = '\\033]777;capilot-shell-ready\\007'
...
// Why: an CaPilot wrapper ZDOTDIR recurses; treat it as unset and fall back to HOME.
...
export function getBashShellReadyRcfileContent(): string {
  return `# CaPilot bash shell-ready wrapper
...
# PATH edits, hooks, and prompt init in CaPilot startup-command shells.
...
export function getZshShellReadyRcfileContent(): string {
  return `# CaPilot zsh shell-ready wrapper
${getZshStartupFileSourceBlock({
  fileName: '.zshrc',
...
[[ -n "\${ORCA_MIMOCODE_HOME:-}" ]] && export MIMOCODE_HOME="\${ORCA_MIMOCODE_HOME}"
  ${getPosixOmpShellWrapper()}
  # Why: Codex must keep using CaPilot's runtime CODEX_HOME after rc files.
  [[ -n "\${ORCA_CODEX_HOME:-}" ]] && export CODEX_HOME="\${ORCA_CODEX_HOME}"
fi
__orca_osc133_precmd() {
  local exit_code=$?
...
export function ensureShellReadyWrappersAt(root = getShellReadyWrapperRoot()): void {
  ...
  const zshProfile = `# CaPilot zsh shell-ready wrapper
${getZshStartupFileSourceBlock({ fileName: '.zprofile' })}
`
  const zshRc = getZshShellReadyRcfileContent()
  const zshLogin = `# CaPilot zsh shell-ready wrapper
...
export function writeStartupCommandWhenShellReady(
  ...
  // Why: only CaPilot-wrapped bash/zsh have bracketed-paste active; other shells use the raw path to avoid echoing the ESC[200~ markers.
