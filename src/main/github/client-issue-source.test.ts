...
describe('GitHub issue source split', () => {
  ...
  it('uses upstream for issues and origin for PRs in mixed recent results', async () => {
    ...
    getOwnerRepoMock.mockResolvedValueOnce({ owner: 'fork', repo: 'capilot' })
    ghExecFileAsyncMock
    ...
    expect(ghExecFileAsyncMock).toHaveBeenNthCalledWith(2, prListArgs('fork/capilot'), {
      cwd: '/repo-root'
    })
  })
  ...
  it('lists SSH repo work items with explicit owner/repo and no local cwd', async () => {
    ...
    await listWorkItems('/home/jinwoo/capilot', 10, undefined, undefined, 'auto', 'openclaw-2')
    ...
    expect(getOwnerRepoMock).toHaveBeenCalledWith('/home/jinwoo/capilot', 'openclaw-2', {})
    expect(ghExecFileAsyncMock).toHaveBeenNthCalledWith(1, issueSearchArgs('stablyai/orca'), {})
    expect(ghExecFileAsyncMock).toHaveBeenNthCalledWith(2, prListArgs('fork/capilot'), {})
  })
  ...
  it('uses upstream for issue-only queries and origin for PR-only queries', async () => {
    ...
    expect(ghExecFileAsyncMock).toHaveBeenCalledWith(
      expect.arrayContaining(['--repo', 'fork/capilot']),
      { cwd: '/repo-root' }
    )
  })
  ...
  it("falls back to origin for PRs when preference='upstream' and upstream is missing", async () => {
    ...
    expect(result.sources).toEqual({
      issues: { owner: 'fork', repo: 'capilot' },
      prs: { owner: 'fork', repo: 'capilot' },
      originCandidate: { owner: 'fork', repo: 'capilot' },
  ...
  it('counts default work items across upstream issues and origin PRs', async () => {
    ...
    expect(ghExecFileAsyncMock).toHaveBeenNthCalledWith(
      ...
      [
        ...
        '--cache',
        '120s',
        `search/issues?q=${encodeURIComponent('repo:fork/capilot is:pull-request is:open')}&per_page=1`,
        '--jq',
        '.total_count'
      ],
      { cwd: '/repo-root' }
  ...
  it('pins typed PR metadata to explicit origin when upstream has the same number', async () => {
    ...
    expect(
      ghExecFileAsyncMock.mock.calls.some((call) =>
        (call[0] as string[]).some((arg) => arg.includes('upstream/capilot'))
      )
    ).toBe(false)
  ...
  it('does not probe a second PR repository after a non-not-found failure', async () => {
    resolvePRRepositoryCandidatesMock.mockResolvedValueOnce({
      candidates: [
        ...
        { owner: 'fork', repo: 'capilot', host: 'github.com' }
      ],
      headRepo: { owner: 'fork', repo: 'capilot', host: 'github.com' }
    })
    ghExecFileAsyncMock
    ...
    expect(
      ghExecFileAsyncMock.mock.calls.some(([args]) =>
        (args as string[]).some((arg) => arg.includes('fork/capilot'))
      )
    ).toBe(false)
  })
  ...
  describe('per-repo issue-source preference', () => {
    ...
    it("preference='auto' + no upstream → queries origin", async () => {
      ...
      getOwnerRepoMock.mockResolvedValueOnce({ owner: 'solo', repo: 'capilot' })
      ghExecFileAsyncMock.mockResolvedValueOnce({ stdout: '[]' }).mockResolvedValueOnce({
        stdout: '[]'
      })
      ...
      expect(ghExecFileAsyncMock).toHaveBeenNthCalledWith(1, issueSearchArgs('solo/capilot'), {
        cwd: '/repo-root'
      })
    })
    ...
    it("preference='upstream' + no upstream → falls back to origin with fellBack=true", async () => {
      ...
      expect(decodedIssueSearchPath(0)).toContain('q=repo:solo/capilot is:issue is:open')
      expect(result.issueSourceFellBack).toBe(true)
    })
    ...
    it("preference='origin' + upstream exists → queries origin (not upstream)", async () => {
      ...
      expect(decodedIssueSearchPath(0)).toContain('q=repo:fork/capilot is:issue is:open')
    })
