...
describe('CaPilot cloud profile service', () => {
  beforeEach(() => {
    userDataPath = mkdtempSync(join(tmpdir(), 'capilot-cloud-service-'))
  ...
  it('does not report a saved cloud session as connected when cloud config is unavailable', async () => {
    ...
    expect(getCurrentOrcaProfileAuthStatus(userDataPath)).toMatchObject({
      ...
      persistence: 'encrypted',
      cloud: cloudSummary,
      setupMessage: 'CaPilot Cloud sign-in is not configured for this build.'
    })
