...
describe('profile index store', () => {
  beforeEach(() => {
    testState.dir = mkdtempSync(join(tmpdir(), 'capilot-profile-test-'))
  })
  ...
  it('creates the default local profile and copies legacy state without deleting it', async () => {
    ...
    writeFileSync(join(testState.dir, 'capilot-data.json'), JSON.stringify(legacyState), 'utf-8')
    writeFileSync(
      join(testState.dir, 'capilot-data.json.bak.0'),
    ...
    expect(activeProfile.dataFile).toBe(
      join(testState.dir, 'profiles', DEFAULT_LOCAL_ORCA_PROFILE_ID, 'capilot-data.json')
    )
    ...
    expect(existsSync(join(testState.dir, 'capilot-data.json'))).toBe(true)
  ...
  it('uses an existing active profile data file without overwriting it from legacy state', async () => {
    ...
    writeFileSync(join(profileDirectory, 'capilot-data.json'), JSON.stringify(profileData), 'utf-8')
    writeFileSync(
    ...
    expect(activeProfile.dataFile).toBe(join(profileDirectory, 'capilot-data.json'))
