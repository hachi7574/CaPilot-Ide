...
function devSelf(): OrcaOrgMember {
  return {
    ...
    email: cleanEnvString(process.env.ORCA_CLOUD_DEV_EMAIL, 'dev@capilot.local'),
    displayName: cleanEnvString(process.env.ORCA_CLOUD_DEV_DISPLAY_NAME, 'CaPilot Dev'),
    role: 'owner'
  }
}
  ...
  return {
    members: [
      devSelf(),
      {
        ...
        email: 'teammate@capilot.local',
        displayName: 'Dev Teammate',
        role: 'admin'
      },
      // Why: userId null exercises the "hasn't signed in to CaPilot yet" disabled row.
      { userId: null, email: 'invited-member@capilot.local', displayName: undefined, role: 'member' }
    ],
    pendingInvites: [{ email: 'pending@capilot.local', role: 'member', createdAt: Date.now() }]
  }
}
