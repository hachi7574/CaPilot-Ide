...
import type { OrcaOrgMembersRoster } from '../../shared/capilot-profiles'
...
describe('CaPilot cloud org members service (configured)', () => {
  beforeEach(() => {
    userDataPath = mkdtempSync(join(tmpdir(), 'capilot-org-members-'))
...
describe('CaPilot cloud org members service (dev auth)', () => {
  beforeEach(() => {
    userDataPath = mkdtempSync(join(tmpdir(), 'capilot-org-members-dev-'))
    runWithFreshOrcaCloudSessionMock.mockReset()
    vi.unstubAllEnvs()
  ...
  it('mutates the dev roster across invite and revoke', async () => {
    ...
    expect(afterInvite.roster.pendingInvites.some((i) => i.email === 'fresh@capilot.local')).toBe(true)
    ...
    await expect(
      revokeOrcaProfileOrgInvite(userDataPath, { orgId, email: 'fresh@capilot.local' })
    ).resolves.toEqual({ status: 'ok' })
    await expect(
      revokeOrcaProfileOrgInvite(userDataPath, { orgId, email: 'fresh@capilot.local' })
    ).resolves.toEqual({ status: 'not-found' })
