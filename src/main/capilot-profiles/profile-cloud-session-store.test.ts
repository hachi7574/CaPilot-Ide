...
describe('CaPilot cloud session store', () => {
  beforeEach(() => {
    userDataPath = mkdtempSync(join(tmpdir(), 'capilot-cloud-session-'))
    vi.unstubAllEnvs()
  ...
  it('scopes memory-only sessions by user-data path and profile ID', async () => {
    safeStorageMock.isEncryptionAvailable.mockReturnValue(false)
    const otherUserDataPath = mkdtempSync(join(tmpdir(), 'capilot-cloud-session-other-'))
    const store = await loadSessionStore()
    const session = makeSession()
