...
// duplicates disk; `capilot.yaml` names the ones every worktree should share instead.
...
/** Every path CaPilot may have symlinked into a worktree: the per-user Worktree
 *  Shared Paths setting plus the repo's `capilot.yaml` shared directories.
 *
...
/** Resolve `worktree.sharedDirectories` from the repo-root `capilot.yaml` to
