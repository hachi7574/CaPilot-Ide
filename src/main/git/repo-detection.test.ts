...
describe('isGitRepo', () => {
  ...
  beforeEach(() => {
    tmpDir = mkdtempSync(path.join(tmpdir(), 'capilot-repo-detect-'))
  })
  ...
  it('accepts a linked worktree when git itself cannot be run', () => {
    ...
    git(realRepo, [
      ...
      '-c',
      'user.email=capilot@example.com',
      'commit',
      '--allow-empty',
      '--message',
      'initial'
  ...
  it('rejects a .git file that points at a missing gitdir when git cannot be run', () => {
    ...
    mkdirSync(fakeRepo)
    writeFileSync(path.join(fakeRepo, '.git'), 'gitdir: /missing/capilot/gitdir')
...
describe('getLinkedWorktreeMainRepoRoot', () => {
  ...
  beforeEach(() => {
    tmpDir = mkdtempSync(path.join(tmpdir(), 'capilot-linked-worktree-'))
  })
  ...
  function initRepoWithCommit(repoRoot: string): void {
    ...
    git(repoRoot, ['config', 'user.email', 'test@capilot.test'])
    git(repoRoot, ['config', 'user.name', 'CaPilot Test'])
