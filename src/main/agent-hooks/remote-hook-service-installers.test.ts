...
describe('remote hook service installers', () => {
  ...
  it('installs remote Codex hooks with matching trust entries', async () => {
    ...
    for (const eventName of [
      ...
      expect(command).toContain('/home/dev/.capilot/agent-hooks/codex-hook.sh')
      expect(command).toMatch(/^if \[ -f /)
    }
    expect(fs.files.get('/home/dev/.capilot/agent-hooks/codex-hook.sh')).toContain('#!/bin/sh')
    expect(fs.modes.get('/home/dev/.capilot/agent-hooks/codex-hook.sh')).toBe(0o755)
  ...
  it('installs Codex hooks into an explicit redirected CODEX_HOME (WSL managed runtime home)', async () => {
    const runtimeHome = '/home/dev/.local/share/capilot/codex-runtime-home/home'
    const { sftp, fs } = createFakeSftp({
    ...
    expect(hooks.hooks.Stop?.[0]?.hooks?.[0]?.command).toContain(
      '/home/dev/.capilot/agent-hooks/codex-hook.sh'
    )
...
`
    ...
    for (const eventName of ['BeforeAgent', 'AfterAgent', 'AfterTool', 'BeforeTool']) {
      ...
      expect(command).toContain('/home/dev/.capilot/agent-hooks/gemini-hook.sh')
      expect(command).toMatch(/^if \[ -f /)
    }
    ...
    for (const eventName of ['PreInvocation', 'PostInvocation', 'Stop']) {
      const command = antigravityConfig['capilot-status'][eventName]?.[0]?.command
      expect(command).toContain('/home/dev/.capilot/agent-hooks/antigravity-hook.sh')
      expect(command).toContain(`ORCA_ANTIGRAVITY_EVENT='${eventName}'`)
    }
    expect(antigravityConfig['capilot-status'].PreToolUse).toBeUndefined()
    for (const eventName of ['PostToolUse']) {
      const definition = antigravityConfig['capilot-status'][eventName]?.[0]
    ...
    const ampPlugin = amp.fs.files.get('/home/dev/.config/amp/plugins/capilot-agent-status.ts')
    ...
    for (const eventName of [
      ...
      expect(definition?.command).toContain('/home/dev/.capilot/agent-hooks/cursor-hook.sh')
    ...
    for (const eventName of ['PreToolUse', 'PostToolUse', 'Stop']) {
      ...
      const command = definition?.hooks?.[0]?.command
      expect(command).toContain('/home/dev/.capilot/agent-hooks/command-code-hook.sh')
      expect(command).toMatch(/^if \[ -f /)
    }
    ...
    const grokConfig = JSON.parse(grok.fs.files.get('/home/dev/.grok/hooks/capilot-status.json')!) as {
      hooks: Record<string, { matcher?: string; hooks?: { command: string }[] }[]>
    }
    for (const eventName of [
      'SessionStart',
      ...
      const command = definition?.hooks?.[0]?.command
      expect(command).toContain('/home/dev/.capilot/agent-hooks/grok-hook.sh')
      expect(command).toMatch(/^if \[ -f /)
    }
    ...
    for (const eventName of [
      ...
      const command = definition?.hooks?.[0]?.command
      expect(command).toContain('/home/dev/.capilot/agent-hooks/devin-hook.sh')
      expect(command).toMatch(/^if \[ -f /)
    }
    ...
    expect(devin.fs.files.get('/home/dev/.capilot/agent-hooks/devin-hook.sh')).toContain('/hook/devin')
  })
  ...
  it('installs remote Grok config in the explicit guest GROK_HOME', async () => {
    ...
    expect(status.configPath).toBe('/srv/grok profile/hooks/capilot-status.json')
    expect(fs.files.has('/srv/grok profile/hooks/capilot-status.json')).toBe(true)
    expect(fs.files.has('/home/dev/.grok/hooks/capilot-status.json')).toBe(false)
    const script = fs.files.get('/home/dev/.capilot/agent-hooks/grok-hook.sh')!
  ...
  it.each(['relative/grok', '/bad\\grok', `/${'x'.repeat(4096)}`])(
    ...
    async (remoteGrokHome) => {
      ...
      expect(status.configPath).toBe('/home/dev/.grok/hooks/capilot-status.json')
      expect(fs.files.has('/home/dev/.grok/hooks/capilot-status.json')).toBe(true)
    }
  )
  ...
  it('installs remote Kimi hooks as a managed config.toml block preserving user config', async () => {
    ...
    expect(config).toContain('/home/dev/.capilot/agent-hooks/kimi-hook.sh')
    expect(config).toMatch(/command = "if \[ -f /)
    expect(fs.files.get('/home/dev/.capilot/agent-hooks/kimi-hook.sh')).toContain('/hook/kimi')
  })
  ...
  it('does not overwrite malformed remote Devin JSONC', async () => {
    ...
    try {
      ...
      expect(fs.files.get('/home/dev/.capilot/agent-hooks/devin-hook.sh')).toBeUndefined()
    } finally {
      warn.mockRestore()
    }
  })
  ...
  it('removes stale remote Antigravity PreToolUse hooks while installing SSH hooks', async () => {
    ...
    const config = JSON.parse(fs.files.get('/home/dev/.gemini/config/hooks.json')!) as {
      'capilot-status': Record<string, { hooks?: { command: string }[] }[]>
    }
    expect(config['capilot-status'].PreToolUse).toBeUndefined()
    const postToolCommands = config['capilot-status'].PostToolUse.flatMap((definition) =>
  ...
  it('installs remote Copilot hooks under the user-level hooks directory', async () => {
    ...
    fs.files.set(
      '/home/dev/.copilot/hooks/capilot.json',
      JSON.stringify({
        version: 99,
    ...
    expect(status.state).toBe('installed')
    expect(status.configPath).toBe('/home/dev/.copilot/hooks/capilot.json')
    const config = JSON.parse(fs.files.get('/home/dev/.copilot/hooks/capilot.json')!) as {
      version: number
    ...
    for (const eventName of [
      ...
      expect(definition?.bash).toContain('/home/dev/.capilot/agent-hooks/copilot-hook.sh')
    ...
    expect(fs.files.get('/home/dev/.capilot/agent-hooks/copilot-hook.sh')).toContain('#!/bin/sh')
    expect(fs.modes.get('/home/dev/.capilot/agent-hooks/copilot-hook.sh')).toBe(0o755)
  })
  ...
  it('installs remote Droid hooks into Factory settings.json (issue #7253)', async () => {
    ...
    for (const eventName of [
      ...
      const definition = config.hooks[eventName]?.[0]
      const command = definition?.hooks?.[0]?.command
      expect(command).toContain('/home/dev/.capilot/agent-hooks/droid-hook.sh')
      expect(command).toMatch(/^if \[ -f /)
    }
    ...
    const script = fs.files.get('/home/dev/.capilot/agent-hooks/droid-hook.sh')
    expect(script).toContain('#!/bin/sh')
    expect(script).toContain('/hook/droid')
    expect(fs.modes.get('/home/dev/.capilot/agent-hooks/droid-hook.sh')).toBe(0o755)
  })
  ...
  it('does not overwrite a malformed remote Factory settings.json', async () => {
    ...
    expect(fs.files.get('/home/dev/.capilot/agent-hooks/droid-hook.sh')).toBeUndefined()
  })
  ...
  it('installs remote Hermes plugin files and enables the plugin', async () => {
    ...
    expect(fs.files.get('/home/dev/.hermes/plugins/capilot-status/plugin.yaml')).toContain(
      'pre_llm_call'
    )
    expect(fs.files.get('/home/dev/.hermes/plugins/capilot-status/__init__.py')).toContain(
      '/hook/hermes'
    )
    expect(fs.files.get('/home/dev/.hermes/config.yaml')).toContain('capilot-status')
  })

  it('does not overwrite a remote user-authored Amp plugin file', async () => {
    const { sftp, fs } = createFakeSftp({
      '/home/dev/.config/amp/plugins/capilot-agent-status.ts':
    ...
    expect(fs.files.get('/home/dev/.config/amp/plugins/capilot-agent-status.ts')).toBe(
      'export default function userPlugin() {}\n'
    )
  })
})
