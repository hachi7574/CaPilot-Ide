...
describe('PluginCommandRegistry', () => {
  ...
  it('uses saved effective bindings to recover conflicting plugins', () => {
    ...
    registry.reconcile(
      [first, second],
      () => true,
      { 'plugin:capilot-samples.first/tasks': ['Mod+Shift+T'] },
      'linux'
    )
  ...
  it('rejects conflicting saved bindings within one plugin', () => {
    ...
    registry.reconcile(
      ...
      () => true,
      {
        'plugin:capilot-samples.aliases/tasks': ['Mod+Alt+T'],
        'plugin:capilot-samples.aliases/sidebar': ['Mod+Alt+T']
      },
      'linux'
    )
