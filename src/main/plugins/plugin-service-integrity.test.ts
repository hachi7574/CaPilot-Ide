...
async function createInstalledPlugin(options: { worker: boolean }): Promise<{
  ...
  const userDataPath = await mkdtemp(join(tmpdir(), 'capilot-plugin-service-integrity-'))
  ...
  await writeFile(join(stagingDir, 'capilot-plugin.json'), JSON.stringify(manifest))
