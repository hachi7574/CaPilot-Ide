...
const pluginKey = 'capilot-samples.demo'
...
async function pluginRoot(pluginManifest = manifest()): Promise<string> {
  const root = await mkdtemp(join(tmpdir(), 'capilot-plugin-reconcile-'))
  roots.push(root)
  await writeFile(join(root, 'capilot-plugin.json'), JSON.stringify(pluginManifest))
...
describe('PluginService worker reconciliation', () => {
  it('blocks every runtime surface until a saved override resolves a content conflict', async () => {
    ...
    const service = new PluginService({
      ...
      getDisabledPlugins: () => [],
      getPluginConsents: () => ({
        'capilot-samples.first': fingerprintPluginConsent(firstManifest, firstHash.hash),
        'capilot-samples.second': fingerprintPluginConsent(secondManifest, secondHash.hash)
      }),
    ...
    expect(service.activationError('capilot-samples.first')).toContain('conflicts')
    expect(service.getGrantedCapabilities('capilot-samples.first')).toBeNull()
    await expect(service.invokeCommand('capilot-samples.first', 'run')).rejects.toThrow('not enabled')
    await expect(service.panels.readEntry('capilot-samples.first', 'panel')).resolves.toBeNull()
    ...
    keybindings = { 'plugin:capilot-samples.first/run': ['Mod+Shift+T'] }
    await service.reconcileActivationState()

    expect(service.activationError('capilot-samples.first')).toBeNull()
    await expect(service.panels.readEntry('capilot-samples.first', 'panel')).resolves.toMatchObject({
      html: expect.stringContaining('<h1>Panel</h1>')
    })
    await expect(service.invokeCommand('capilot-samples.first', 'run')).resolves.toBeNull()
