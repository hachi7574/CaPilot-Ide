...
const pluginKey = 'capilot-samples.recipes'
...
async function pluginRoot(): Promise<string> {
  const root = await mkdtemp(join(tmpdir(), 'capilot-plugin-kill-content-'))
  roots.push(root)
  await Promise.all([mkdir(join(root, 'locales')), mkdir(join(root, 'recipes'))])
  await Promise.all([
    writeFile(join(root, 'capilot-plugin.json'), JSON.stringify(contentManifest())),
