...
export const SINGLE_INSTANCE_LOCK_FAILURE_MESSAGE =
  '[single-instance] Another CaPilot instance is already running for this userData profile; exiting this launch after requesting the existing window. If no CaPilot process is running, this may be an Electron/macOS single-instance lock failure.'
...
export const SINGLE_INSTANCE_LOCK_BYPASS_MESSAGE =
  '[single-instance] ORCA_BYPASS_SINGLE_INSTANCE_LOCK=1 is set; bypassing the packaged macOS single-instance lock for diagnostics. Do not use this with another CaPilot instance running for the same profile.'

/**
 * Why: CaPilot writes two canonical discovery files into `<userData>/`:
 * `capilot-runtime.json` (RPC endpoint + authToken for the bundled CLI) and
 ...
 * instance quits, metadata points at a dead pid and `capilot status` reports
 ...
 * way dev (`capilot-dev` userData) and packaged (`capilot` userData) runs lock in
