...
const BASE_APP_NAME = 'CaPilot'
const BASE_APP_USER_MODEL_ID = 'com.capilot.ide'
...
export type DevInstanceIdentity = AppIdentity & {
  ...
  // creating a new one per branch and re-prompting. Distinct from prod's 'CaPilot'.
  appName: string
}
...
export function getDevInstanceIdentity(
  ...
  return {
    name: dockTitle,
    // Why: one stable Keychain key ('CaPilot Dev Safe Storage') for all dev
