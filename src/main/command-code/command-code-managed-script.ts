...
export function buildCommandCodeManagedScript(
  ...
  if (target === 'local' && process.platform === 'win32') {
    return [
      ...
      'if exist "%APPDATA%\\capilot-dev\\agent-hooks" for /r "%APPDATA%\\capilot-dev\\agent-hooks" %%F in (endpoint.cmd) do call :maybeSourceEndpoint "%%~fF"',
      'if "%ORCA_AGENT_HOOK_TOKEN%"=="" if exist "%APPDATA%\\capilot\\agent-hooks" for /r "%APPDATA%\\capilot\\agent-hooks" %%F in (endpoint.cmd) do call :maybeSourceEndpoint "%%~fF"',
      'exit /b 0',
      ':maybeSourceEndpoint',
  ...
  return [
    ...
    '# still has CaPilot pane/hook metadata, so recover it before posting.',
    ...
    '  for endpoint in \\',
    '    "$HOME/Library/Application Support/capilot-dev/agent-hooks"/*/endpoint.env \\',
    '    "$HOME/Library/Application Support/capilot-dev/agent-hooks/endpoint.env" \\',
    '    "${XDG_CONFIG_HOME:-$HOME/.config}/capilot-dev/agent-hooks"/*/endpoint.env \\',
    '    "${XDG_CONFIG_HOME:-$HOME/.config}/capilot-dev/agent-hooks/endpoint.env" \\',
    '    "$HOME/Library/Application Support/capilot/agent-hooks/endpoint.env" \\',
    '    "${XDG_CONFIG_HOME:-$HOME/.config}/capilot/agent-hooks/endpoint.env"; do',
    '    [ -r "$endpoint" ] || continue',
