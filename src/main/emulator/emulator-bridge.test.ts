...
import { RuntimeEmulatorCommands } from '../runtime/capilot-runtime-emulator'
...
describe('EmulatorBridge helper ownership', () => {
  ...
  it('stops the previous CaPilot-managed helper when a worktree switches devices', async () => {
  ...
  it('shuts down the previous CaPilot-managed device when requested', async () => {
    const bridge = new EmulatorBridge()
    bridge.registerActiveEmulator('wt-1', session('device-old'), { managed: true })
  ...
  it('only kills CaPilot-managed helpers during app shutdown cleanup', async () => {
    const bridge = new EmulatorBridge()
