...
describe('resolveExternalEditorLaunchSpec', () => {
  ...
  it('prefers the JetBrains *64.exe colocated with the resolved idea.cmd on Windows', () => {
    ...
    expect(
      resolveExternalEditorLaunchSpec('idea', 'C:\\workspaces\\capilot', {
        platform: 'win32',
      ...
      spawnArgs: ['C:\\workspaces\\capilot']
    })
  ...
  it('prefers a colocated .exe over an idea64.cmd shim when the user names idea64', () => {
    ...
    expect(
      resolveExternalEditorLaunchSpec('goland64', 'C:\\workspaces\\capilot', {
        platform: 'win32',
  ...
  it.each([
    ['code', 'C:\\Tools\\code.cmd', ['C:\\workspaces\\capilot']],
    [
      'cursor',
      'C:\\Users\\me\\AppData\\Local\\Programs\\cursor\\bin\\cursor.cmd',
      ['--new-window', 'C:\\workspaces\\capilot']
    ]
    ...
    expect(
      resolveExternalEditorLaunchSpec(command, 'C:\\workspaces\\capilot', { platform: 'win32' })
    ).toEqual({
      kind: 'executable',
      hideWindowsConsole: true,
  ...
  it('runs GUI compound Windows commands verbatim instead of re-parsing them under start', () => {
    expect(
      resolveExternalEditorLaunchSpec('code --reuse-window', 'C:\\workspaces\\capilot', {
        platform: 'win32'
      })
    ).toEqual({
      kind: 'shell',
      hideWindowsConsole: true,
      spawnCmd: getCmdExePath(),
      spawnArgs: ['/d', '/s', '/c', 'code --reuse-window C:\\workspaces\\capilot']
    })
  })
  ...
  it('shows the Windows console for NeoVim shell commands with arguments', () => {
    expect(
      resolveExternalEditorLaunchSpec('nvim --clean', 'C:\\workspaces\\capilot', {
        platform: 'win32'
      })
    ).toEqual({
      kind: 'shell',
      hideWindowsConsole: false,
      spawnCmd: getCmdExePath(),
      spawnArgs: ['/d', '/s', '/c', 'nvim --clean C:\\workspaces\\capilot']
    })
  })
  ...
  it.each(['C:\\workspaces\\capilot', '\\\\server\\share\\project'])(
