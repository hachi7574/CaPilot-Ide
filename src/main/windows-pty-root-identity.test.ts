...
/** Windows: services.exe → svchost.exe, a chain that never reaches CaPilot. */
...
describe('classifyWindowsTreeKillTarget', () => {
  ...
  it('accepts a winpty shell reached through the winpty-agent hop', () => {
    ...
    // parent is winpty-agent.exe rather than CaPilot itself.
    const rows = [link(ORCA_PID, 900), link(6100, ORCA_PID), link(4242, 6100)]
    expect(classifyWindowsTreeKillTarget(4242, rows, ORCA_PID)).toBe('own')
  })

  it('documents that a recycled PID under another CaPilot pane still classifies as own', () => {
  ...
  it('rejects a chain longer than the ConPTY/winpty depth even if CaPilot is above it', () => {
    const rows = [
      link(ORCA_PID, 900),
      link(10, ORCA_PID),
      link(11, 10),
...
describe('verifyWindowsTreeKillTarget scan volume', () => {
  const ROWS_JSON = JSON.stringify([
    { ProcessId: ORCA_PID, ParentProcessId: 900, Name: 'capilot.exe', CommandLine: 'capilot.exe' },
