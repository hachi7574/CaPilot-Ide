...
function getProvenancePath(runtimeHomePath: string): string {
  return join(runtimeHomePath, '.capilot-hook-trust-provenance.json')
}
...
/**
 * Records the runtime config.toml trust state CaPilot leaves behind after an
 * install/refresh, so the next launch can tell "entry CaPilot wrote" apart from
...
/**
 * Promotes hook approvals the user made inside CaPilot-launched Codex (written
...
function promoteCodexRuntimeHookApprovalsToSystemUnsafe(runtimeHomePath: string): void {
  ...
  // Why: without a snapshot of what CaPilot last wrote (first launch after
  ...
  // in-CaPilot approval. Promoting would resurrect trust the user revoked in
  ...
  for (const [key, state] of runtimeTrust) {
    ...
    if (
      ...
      // CaPilot wrote this entry and nothing touched it since — not an approval.
      continue
    }
    ...
    const hook = Array.isArray(definition?.hooks)
      ...
      : undefined
    // Why: never write trust for CaPilot's managed status hook into the user's
    // real config — mutating ~/.codex for CaPilot's own hooks is exactly what the
