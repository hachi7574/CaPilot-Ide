...
export function syncSystemConfigIntoManagedCodexHome(
  ...
  // user made inside CaPilot-launched Codex (/model, /approvals) must be written
  ...
  // unpromoted runtime change as CaPilot-written would strand it forever.
  snapshotCodexRuntimeSettingsBaseline(
...
function mergeSystemCodexConfigIntoRuntime(runtimeConfig: string, systemConfig: string): string {
  ...
  // trust and project trust are written under CaPilot's managed CODEX_HOME and
