...
// stamped `complete` on first app-server startup), so rollouts that CaPilot's
...
// Codex's DB-driven surfaces. CaPilot never writes Codex's sqlite schema itself.
...
function buildNativeHealInvocation(
  ...
  return {
    command: spawnCmd,
    args: spawnArgs,
    // Why: pin the real home explicitly — nested CaPilot launches can inherit a
