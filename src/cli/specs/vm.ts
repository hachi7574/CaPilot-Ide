...
export const VM_COMMAND_SPECS: CommandSpec[] = [
  {
    ...
    usage:
      'capilot vm recipe doctor <recipe-id> [--repo-path <path>] [--provision|--connect] [--json]',
    allowedFlags: [...GLOBAL_FLAGS, 'recipe-id', 'repo-path', 'provision', 'connect'],
    positionalArgs: ['recipe-id'],
    notes: [
      'Reads environmentRecipes from capilot.yaml in the repo path, validates the selected recipe, and reports agent-friendly checks.',
    ...
    examples: [
      'capilot vm recipe doctor cloud-sandbox',
      'capilot vm recipe doctor cloud-sandbox --repo-path /path/to/repo --json',
      'capilot vm recipe doctor cloud-sandbox --provision --json'
    ]
  }
]
