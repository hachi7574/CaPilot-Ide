...
export const SKILL_COMMAND_SPECS: CommandSpec[] = [
  {
    path: ['skills', 'list'],
    summary: 'List version-matched skill guides bundled with this CaPilot CLI',
    usage: 'capilot skills list [--json]',
    allowedFlags: [...GLOBAL_FLAGS],
    notes: [
      'Reads bundled guide metadata locally without contacting the CaPilot runtime.',
      'With --json, prints a topics array of canonical names and one-line descriptions.',
      'Use `capilot skills get <name>` for the full guide, or `capilot skills install` to install skills.'
    ]
  },
  {
    path: ['skills', 'get'],
    ...
    usage: 'capilot skills get <topic> [--full] [--json]',
    allowedFlags: [...GLOBAL_FLAGS, 'topic', 'full'],
    positionalArgs: ['topic'],
    notes: [
      'Reads bundled guide content locally without contacting the CaPilot runtime.',
    ...
    examples: ['capilot skills get capilot-cli', 'capilot skills get orchestration --full']
  },
  {
    path: ['skills', 'install'],
    summary: 'Install bundled CaPilot skills via the community skills CLI',
    usage:
      'capilot skills install [--skill <name>]... [--all] [--agent <name>[,<name>]] ' +
      '[--local] [--dry-run] [--json]',
    allowedFlags: [...GLOBAL_FLAGS, 'skill', 'all', 'agent', 'local', 'dry-run'],
    notes: [
      'Reads the bundled skill registry locally without contacting the CaPilot runtime.',
      'Resolves to the same `npx skills add <repo> --skill <name> ...` command used by ' +
        'CaPilot Settings, plus the non-interactive flags an unattended host needs ' +
      ...
      'Targets the coding agents CaPilot detects on this host, plus the shared ' +
      ...
      'Use --agent <name>[,<name>...] to choose targets yourself, or --agent universal ' +
        'for the shared directory alone. Required when CaPilot detects no agent.',
    ...
    examples: [
      'capilot skills install',
      'capilot skills install --skill capilot-cli --skill orchestration',
      'capilot skills install --skill capilot-cli --local',
      'capilot skills install --skill capilot-cli --agent claude-code,codex',
      'capilot skills install --all --dry-run'
    ]
  },
  {
    path: ['skills', 'update'],
    summary: 'Update already-installed CaPilot skills via the community skills CLI',
    usage: 'capilot skills update [--skill <name>]... [--all] [--local] [--dry-run] [--json]',
    allowedFlags: [...GLOBAL_FLAGS, 'skill', 'all', 'local', 'dry-run'],
    notes: [
      'Reads the bundled skill registry locally without contacting the CaPilot runtime.',
      'Resolves to the same `npx skills update <names...>` command used by CaPilot Settings, ' +
      ...
      'Only refreshes skills that are already installed; use `capilot skills install` first.',
      'Use --dry-run to print the resolved command without running it.',
    ...
    examples: [
      'capilot skills update',
      'capilot skills update --skill capilot-cli --skill orchestration',
      'capilot skills update --skill capilot-cli --local',
      'capilot skills update --all --dry-run'
    ]
  }
]
