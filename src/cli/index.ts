...
// Why: the SSH relay bridge executes this CLI on the CaPilot host while the
...
export async function main(
  ...
  try {
    ...
    // lookup so users do not get misleading "CaPilot is not running" failures for
...
async function runClaudeTeams(argv: string[], cwd: string): Promise<void> {
  try {
    // Why: everything after `capilot claude-teams` belongs to Claude Code, not
    // CaPilot's own flag parser, so new Claude flags work without CaPilot changes.
