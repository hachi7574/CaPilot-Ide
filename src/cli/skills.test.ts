...
describe('capilot skills CLI', () => {
  ...
  it('shows leaf, group, and root help for skills', async () => {
    ...
    expect(String(logSpy.mock.calls[1]?.[0])).toContain(
      'install            Install bundled CaPilot skills'
    )
    expect(String(logSpy.mock.calls[1]?.[0])).toContain(
      'update             Update already-installed CaPilot skills'
    )
  ...
  it('lists installable skills when no --skill/--all is given', async () => {
    ...
    expect(stdoutText(stdoutSpy)).toBe(
      [
        ...
        '  zeta',
        '',
        'Usage: capilot skills install --skill <name> [--skill <name> ...]',
        '   or: capilot skills install --all',
        ''
      ].join('\n')
    )
      ...
      JSON.stringify(
        {
          ...
          error: {
            ...
            message:
              "capilot skills install --json only supports --dry-run. Real installs stream npx's " +
  ...
  it('lists updatable skills when no --skill/--all is given', async () => {
    ...
    expect(stdoutText(stdoutSpy)).toBe(
      [
        ...
        '  zeta',
        '',
        'Usage: capilot skills update --skill <name> [--skill <name> ...]',
        '   or: capilot skills update --all',
        ''
      ].join('\n')
    )
    expect(spawnMock).not.toHaveBeenCalled()
  ...
  it('refuses a real run when the shell forwards capilot to the CaPilot host', async () => {
    vi.stubEnv('ORCA_CLI_CWD', '/home/alice/wt')
    const errorSpy = vi.spyOn(console, 'error').mockImplementation(() => {})

    await main(['skills', 'install', '--skill', 'alpha'], '/tmp/repo')

    // Why: the SSH relay and WSL bridge run argv on the CaPilot host, so a real
  ...
  it('refuses to install when CaPilot detects no agent, instead of targeting them all', async () => {
  ...
  it('maps detected agents onto the skills CLI namespace, not CaPilot ids', async () => {
    const stdoutSpy = vi.spyOn(process.stdout, 'write').mockImplementation(() => true)
    ...
    // CaPilot's `claude` is `claude-code` and its `rovo` is `rovodev`.
      ...
      JSON.stringify(
        {
          ...
          error: {
            code: 'invalid_argument',
            message:
              "capilot skills update --json only supports --dry-run. Real updates stream npx's " +
              "own output, which isn't JSON."
          },
          _meta: { runtimeId: null }
        },
