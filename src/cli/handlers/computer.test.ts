...
describe('capilot computer observation CLI handlers', () => {
  beforeEach(() => {
    vi.restoreAllMocks()
    callMock.mockReset()
  ...
  it('prints targeted permission setup command help', async () => {
    ...
    expect(output).toContain(
      'capilot computer permissions [--id <accessibility|screenshots>] [--json]'
    )
  ...
  it('prints provider capabilities without resolving a worktree', async () => {
    queueFixtures(
      ...
      okFixture('req_capabilities', {
        platform: 'darwin',
        provider: 'capilot-computer-use-macos',
    ...
    expect(vi.mocked(console.log).mock.calls[0][0]).toContain('capilot-computer-use-macos')
  })
  ...
  it('opens computer permission setup without resolving a worktree', async () => {
    ...
    const output = vi.mocked(console.log).mock.calls[0][0]
    expect(output).toContain('Opened CaPilot Computer Use permission setup')
    expect(output).toContain('/Applications/CaPilot Computer Use.app')
  })
