  ...
  try {
    ...
    if (!metadata || !findTransport(metadata, 'unix', 'named-pipe') || !metadata.authToken) {
      throw new RuntimeClientError(
        'runtime_unavailable',
        `CaPilot runtime metadata is incomplete at ${metadataPath}`
      )
    }
    return metadata
  } catch (error) {
    ...
    throw new RuntimeClientError(
      'runtime_unavailable',
      `Could not read CaPilot runtime metadata at ${metadataPath}. Start the CaPilot app first.`
    )
  }
}
...
export function getDefaultUserDataPath(
  ...
  // Why: in dev mode (and for parallel CaPilot instances), the Electron app writes
  // runtime metadata to a separate userData directory (e.g. `capilot-dev`) to avoid
  ...
  if (platform === 'darwin') {
    return join(homeDir, 'Library', 'Application Support', 'capilot')
  }
  if (platform === 'win32') {
    ...
    if (!appData) {
      throw new RuntimeClientError(
        'runtime_unavailable',
        'APPDATA is not set, so the CaPilot runtime metadata path cannot be resolved.'
      )
    }
    return join(appData, 'capilot')
  }
  ...
  return join(process.env.XDG_CONFIG_HOME || join(homeDir, '.config'), 'capilot')
}
