  ...
  return await new Promise((resolve, reject) => {
    ...
    if (!transport) {
      reject(
        new RuntimeClientError(
          'runtime_unavailable',
          'No compatible transport found in CaPilot runtime metadata.'
        )
      )
      return
    }
    ...
    socket.once('error', () => {
      finish({
        ...
        error: new RuntimeClientError(
          'runtime_unavailable',
          'Could not connect to the running CaPilot app. Restart CaPilot and try again.'
        )
      })
    })
    ...
    socket.once('close', () => {
      finish({
        ...
        error: new RuntimeClientError(
          'runtime_unavailable',
          'The CaPilot runtime closed the connection before responding. Restart CaPilot and try again.'
        )
      })
    })
    socket.on('data', (chunk: string) => {
      ...
      while (newlineIndex !== -1 && !settled) {
        ...
        try {
          ...
          finish({
            ...
            error: new RuntimeClientError(
              ...
              'The CaPilot runtime returned an invalid response frame.'
            )
          })
          return
        }
        ...
        // setTimeout().refresh() is stable since Node 10 (CaPilot ships on
        ...
        // the decode boundary so version skew between the CLI and the CaPilot
        ...
        if (response.id !== requestId) {
          finish({
            ...
            error: new RuntimeClientError(
              'invalid_runtime_response',
              'The CaPilot runtime returned a mismatched response id.'
            )
          })
          return
        }
        if (response._meta?.runtimeId && response._meta.runtimeId !== metadata.runtimeId) {
          finish({
            ...
            error: new RuntimeClientError(
              'runtime_unavailable',
              'The CaPilot runtime changed while the request was in flight. Retry the command.'
            )
          })
          return
        }
