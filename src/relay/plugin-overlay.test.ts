...
describe('PluginOverlayManager', () => {
  ...
  it('materializes OpenCode plugin into <overlay>/plugins/<file>', () => {
    ...
    const expected = join(dir!, 'plugins', 'capilot-opencode-status.js')
    expect(existsSync(expected)).toBe(true)
    expect(readFileSync(expected, 'utf8')).toBe('export const X = 1')
  })

  it('mirrors a preexisting remote OpenCode config dir before adding CaPilot plugin', () => {
    ...
    writeFileSync(join(userConfigDir, 'plugins', 'capilot-opencode-status.js'), 'user same-name')

    manager.setSources({ opencodePluginSource: 'capilot plugin' })
    ...
    expect(readFileSync(join(dir!, 'plugins', 'capilot-opencode-status.js'), 'utf8')).toBe(
      'capilot plugin'
    )
  ...
  it('installs Pi extension into the real agent extensions dir', () => {
    ...
    const file = join(result!.sourceAgentDir!, 'extensions', 'capilot-agent-status.ts')
    expect(result?.statusExtensionPath).toBe(file)
    expect(existsSync(file)).toBe(true)
    expect(readFileSync(file, 'utf8')).toContain('@capilot-managed-pi-extension')
  })

  it("does not overwrite a user's same-named remote Pi extension file", () => {
    const piAgentDir = join(homeDir, '.pi', 'agent')
    const extensionFile = join(piAgentDir, 'extensions', 'capilot-agent-status.ts')
  ...
  it('uses the kind-specific Pi-compatible extension source when available', () => {
    ...
    expect(
      readFileSync(join(piResult!.sourceAgentDir!, 'extensions', 'capilot-agent-status.ts'), 'utf8')
    ).toContain('// pi extension')
    expect(
      readFileSync(join(ompResult!.sourceAgentDir!, 'extensions', 'capilot-agent-status.ts'), 'utf8')
    ).toContain('// omp extension')
  })

  it('installs CaPilot status extension into the remote default Pi agent dir', () => {
    const piAgentDir = join(homeDir, '.pi', 'agent')
  ...
  it('mirrors a preexisting remote Pi agent dir instead of the default', () => {
    ...
    expect(readFileSync(join(dir!, 'extensions', 'capilot-agent-status.ts'), 'utf8')).toContain(
      '// pi extension'
    )
  })
  ...
  it('leaves lazy OMP agent.db in the real remote home on the relay', () => {
    ...
    expect(existsSync(join(homeDir, '.capilot-relay', 'omp-overlays'))).toBe(false)
  ...
  describe('per-agent default source dir (no cross-agent fallback)', () => {
    ...
    it('launching pi with both ~/.pi/agent and ~/.omp/agent present installs into ~/.pi/agent', () => {
      ...
      expect(extensions).toContain('capilot-agent-status.ts')
    ...
    it('launching omp when only ~/.pi/agent exists does NOT mirror Pi state', () => {
      ...
      const extensions = readdirSync(join(dir!, 'extensions')).sort()
      expect(extensions).toEqual(['capilot-agent-status.ts'])
    })
    ...
    it('bare-shell prep does not create missing remote agent homes (#10196)', () => {
      ...
      // Why: bare OMP keeps status via ~/.capilot-relay/… without SOURCE_AGENT_DIR or ~/.omp.
      expect(bareOmp?.sourceAgentDir).toBeUndefined()
      expect(bareOmp?.statusExtensionPath).toEqual(
        expect.stringContaining(join('.capilot-relay', 'omp-managed-status-extension'))
      )
  ...
  it('produces stable overlay dirs for a given id (idempotent re-materialization)', () => {
    ...
    expect(readFileSync(join(dirA, 'plugins', 'capilot-opencode-status.js'), 'utf8')).toBe('second')
  })
  ...
  it('hashes unsafe pane ids into portable overlay directory names', () => {
    ...
    expect(existsSync(join(dir!, 'plugins', 'capilot-opencode-status.js'))).toBe(true)
  })
})
...
describe('resolvePiSourceAgentDir', () => {
  ...
  it('keeps explicit PI_CODING_AGENT_DIR values when they are not CaPilot overlays', () => {
    const env = {
      HOME: mkdtempSync(join(tmpdir(), 'plugin-overlay-env-')),
