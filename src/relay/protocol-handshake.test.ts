...
describe('handshake framing', () => {
  it('round-trips an capilot-relay-handshake envelope through the existing framing', () => {
    const sent = encodeHandshakeFrame({
      type: 'capilot-relay-handshake',
    ...
    expect(msg).toEqual({ type: 'capilot-relay-handshake', version: '0.1.0+deadbeef' })
  })

  it('round-trips an capilot-relay-handshake-ok reply', () => {
    const sent = encodeHandshakeFrame({
      type: 'capilot-relay-handshake-ok',
      version: '0.1.0+deadbeef'
    })
    const frames: DecodedFrame[] = []
    const decoder = new FrameDecoder((f) => frames.push(f))
    decoder.feed(sent)
    const msg = parseHandshakeMessage(frames[0].payload)
    expect(msg).toEqual({ type: 'capilot-relay-handshake-ok', version: '0.1.0+deadbeef' })
  })

  it('round-trips an capilot-relay-handshake-mismatch reply', () => {
    const sent = encodeHandshakeFrame({
      type: 'capilot-relay-handshake-mismatch',
      expected: '0.1.0+aaa',
      got: '0.1.0+bbb'
    })
    const frames: DecodedFrame[] = []
  ...
  it('rejects payloads with unknown type', () => {
    const bogus = Buffer.from(JSON.stringify({ type: 'capilot-something-else', version: 'x' }))
