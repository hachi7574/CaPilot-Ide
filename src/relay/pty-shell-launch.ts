...
const RELAY_SHELL_READY_DIR = '.capilot-relay/shell-ready'
...
function ensureOverlayRestoreWrappers(root: string): void {
  ...
  const zshEnv = `# CaPilot relay zsh overlay wrapper
...
`
  const zshProfile = `# CaPilot relay zsh overlay wrapper
${getZshStartupFileSourceBlock({
  fileName: '.zprofile',
  homeExpression: '"${ORCA_USER_ZDOTDIR:-${ORCA_ORIG_ZDOTDIR:-$HOME}}"'
})}
`
  const zshRc = `# CaPilot relay zsh overlay wrapper
${getZshStartupFileSourceBlock({
  fileName: '.zshrc',
  homeExpression: '"${ORCA_USER_ZDOTDIR:-${ORCA_ORIG_ZDOTDIR:-$HOME}}"',
...
`
  const zshLogin = `# CaPilot relay zsh overlay wrapper
${getZshStartupFileSourceBlock({
  fileName: '.zlogin',
  homeExpression: '"${ORCA_USER_ZDOTDIR:-${ORCA_ORIG_ZDOTDIR:-$HOME}}"',
  interactiveOnly: true
...
`
  const bashRc = `# CaPilot relay bash overlay wrapper
...
__orca_normalize_prompt_command() {
  ...
  # a ";"/whitespace separator; trim it so CaPilot's prepend/append never form ";;".
...
`
  ...
  for (const [path, content] of files) {
    ...
    // Why: relay wrapper files persist under ~/.capilot-relay across app
