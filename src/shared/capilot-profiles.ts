...
const LEGACY_ORCA_BROWSER_SESSION_PARTITION_PREFIX = 'persist:capilot-browser-session-'
...
export type OrcaOrgMember = {
  // Why: null for teammates provisioned server-side who never signed into CaPilot;
...
export function getOrcaProfileBrowserDefaultPartition(profileId: string): string {
  ...
  return `persist:capilot-profile-${getOrcaProfileBrowserPartitionSegment(profileId)}-browser-default`
}
...
export function getOrcaProfileBrowserSessionPartition(
  ...
  return `persist:capilot-profile-${getOrcaProfileBrowserPartitionSegment(
    profileId
