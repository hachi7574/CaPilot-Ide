...
describe('createSequencedSetupAgentCommands', () => {
  ...
  it('wraps POSIX setup and startup commands with a matching nonce marker', () => {
    const result = createSequencedSetupAgentCommands({
      runnerScriptPath: '/repo/.git/capilot/setup-runner.sh',
    ...
    expect(result.setupCommand).toContain('bash /repo/.git/capilot/setup-runner.sh')
    ...
    expect(result.setupCommand).toContain(
      'mv -f /repo/.git/capilot/setup-runner.sh.nonce-123.done.tmp'
    )
    ...
    expect(result.startupCommand).toContain(
      'rm -f /repo/.git/capilot/setup-runner.sh.nonce-123.done /repo/.git/capilot/setup-runner.sh.nonce-123.done.tmp'
    )
  ...
  it('uses launch-specific marker paths for overlapping setup gates', () => {
    ...
    expect(first.setupCommand).toContain('/repo/.git/capilot/setup-runner.sh.first-launch.done')
    expect(first.startupCommand).toContain('/repo/.git/capilot/setup-runner.sh.first-launch.done')
    expect(second.setupCommand).toContain('/repo/.git/capilot/setup-runner.sh.second-launch.done')
    expect(second.startupCommand).toContain('/repo/.git/capilot/setup-runner.sh.second-launch.done')
    expect(first.setupCommand).not.toContain('/repo/.git/capilot/setup-runner.sh.second-launch.done')
    expect(second.setupCommand).not.toContain('/repo/.git/capilot/setup-runner.sh.first-launch.done')
  })
  ...
  it('uses the converted Linux marker path for WSL UNC runners on Windows', () => {
    ...
    expect(result.setupCommand).toContain(
      'bash /home/jin/repo/.git/worktrees/feature/capilot/setup-runner.sh'
    )
    expect(result.setupCommand).toContain(
      '/home/jin/repo/.git/worktrees/feature/capilot/setup-runner.sh.nonce-wsl.done'
    )
  ...
  it('keeps remote POSIX runners in bash even from a Windows client', () => {
    const result = createSequencedSetupAgentCommands({
      runnerScriptPath: '/remote/repo/.git/worktrees/feature/capilot/setup-runner.sh',
      startupCommand: 'claude',
      platform: 'windows',
      nonce: 'nonce-remote'
    })
    ...
    expect(result.setupCommand).toContain(
      'bash /remote/repo/.git/worktrees/feature/capilot/setup-runner.sh'
    )
  ...
  it('preserves WSL shell metadata when sequencing native Windows runners', () => {
    ...
    expect(result.setupCommand).toContain('bash /mnt/c/repo/.git/capilot/setup-runner.sh')
    expect(result.setupCommand).toContain(
      '/mnt/c/repo/.git/capilot/setup-runner.sh.nonce-wsl-shell.done'
    )
    expect(result.startupCommand).toContain(
      '/mnt/c/repo/.git/capilot/setup-runner.sh.nonce-wsl-shell.done'
    )
  ...
  it('wraps native Windows runners in a cmd-pinned setup and startup gate', () => {
    const result = createSequencedSetupAgentCommands({
      runnerScriptPath: 'C:\\repo\\.git\\capilot\\setup-runner.cmd',
    ...
    expect(setupPowerShell).toContain("$runner = 'C:\\repo\\.git\\capilot\\setup-runner.cmd'")
...
function resultPathWsl(): string {
  return '\\\\wsl.localhost\\Ubuntu\\home\\jin\\repo\\.git\\worktrees\\feature\\capilot\\setup-runner.sh'
}

function makeTempDir(): string {
  const dir = mkdtempSync(join(tmpdir(), 'capilot-setup-sequencing-'))
  TEMP_DIRS.push(dir)
  return dir
}
