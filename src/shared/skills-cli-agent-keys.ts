...
/**
 * The community `skills` CLI's own `--agent` key for each agent CaPilot detects.
 *
 ...
 * rather than guessed. CaPilot ids and skills keys agree less often than they look
 * (`claude` is `claude-code`, `rovo` is `rovodev`, `aug` is `augment`), and some
 * near-matches are different products — CaPilot's `aider` CLI is not the CLI's
 * `aider-desk`, and CaPilot's `openclaude` is its `openclaw` in name only, so it
 * follows CaPilot's own rule that OpenClaude reads Claude-owned roots.
 */
export const SKILLS_CLI_AGENT_KEY_BY_TUI_AGENT = {
  claude: 'claude-code',
  'claude-agent-teams': 'claude-code',
  // Why: CaPilot states OpenClaude reads Claude-owned roots (native-chat-agent-profiles).
  ...
  devin: 'devin',
  ante: null,
  // Why: CaPilot detects trae by `traecli`, an alias only TRAE CN ships.
  trae: 'trae-cn'
} satisfies Record<TuiAgent, string | null>

/**
 * The shared `.agents/skills` target every universal agent reads. Always included
 * so agents CaPilot cannot map still receive the skill.
 */
...
/** Map detected CaPilot agents onto `skills --agent` keys, plus the universal target. */
