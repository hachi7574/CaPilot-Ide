...
describe('fingerprintPluginConsent', () => {
  it('drops malformed persisted consent identities and oversized fingerprints', () => {
    expect(
      normalizePluginConsents({
        ...
        'capilot-samples.demo': 'sha256-valid',
        'capilot-samples.large': 'x'.repeat(257)
      })
    ).toEqual({ 'capilot-samples.demo': 'sha256-valid' })
  })
  ...
  it('changes when a panel-only plugin gains a trusted Node worker', () => {
    ...
    const lists = {
      pluginConsents: { 'capilot-samples.demo': panelOnly },
      disabledPlugins: []
    }
    expect(getPluginActivationState('capilot-samples.demo', withWorker, lists)).toBe('pending')
    expect(needsReconsent('capilot-samples.demo', withWorker, lists)).toBe(true)
  })
  ...
  it('requires re-consent when instructional content bytes change', () => {
    ...
    expect(
      getPluginActivationState('capilot-samples.recipes', second, {
        pluginConsents: { 'capilot-samples.recipes': first },
        disabledPlugins: []
      })
    ).toBe('pending')
  })
...
describe('plugin install lockfile consent fingerprints', () => {
  ...
  it('reads the legacy capabilityHash field as a consent fingerprint', () => {
    const parsed = parsePluginLockfile({
      version: 1,
      plugins: { 'capilot-samples.demo': persistedEntry }
    })

    expect(parsed.plugins['capilot-samples.demo']?.consentFingerprint).toBe('sha256-legacy-name')
  })
  ...
  it('drops a lockfile whose record key disagrees with its embedded identity', () => {
    const parsed = parsePluginLockfile({
      version: 1,
      plugins: {
        'capilot-samples.other': persistedEntry
      }
    })
  ...
  it('accepts SHA-256 Git object ids in lockfile provenance', () => {
    const parsed = parsePluginLockfile({
      version: 1,
      plugins: {
        'capilot-samples.demo': { ...persistedEntry, resolvedCommit: 'a'.repeat(64) }
      }
    })

    expect(parsed.plugins['capilot-samples.demo']?.resolvedCommit).toBe('a'.repeat(64))
  })
})
