...
describe('hello CaPilot plugin fixture', () => {
  it('uses an ESM entry that remains loadable outside a type-module package', async () => {
    const root = join(process.cwd(), 'examples', 'plugins', 'hello-capilot')
    const manifest = pluginManifestSchema.parse(
      JSON.parse(await readFile(join(root, 'capilot-plugin.json'), 'utf8'))
    )
