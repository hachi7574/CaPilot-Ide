...
describe('ephemeral VM runtime store', () => {
  ...
  function makeUserDataPath(): string {
    const userDataPath = mkdtempSync(join(tmpdir(), 'capilot-ephemeral-vm-store-'))
  ...
  it('updates lifecycle and cleanup state', () => {
    ...
    const failed = updateEphemeralVmRuntimeStatus(userDataPath, 'capilot-instance-1', {
    ...
    const recovered = updateEphemeralVmRuntimeStatus(userDataPath, 'capilot-instance-1', {
      status: 'cleaned',
