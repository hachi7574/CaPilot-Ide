...
export const WSL_HOOK_RELAY_DIR = '.capilot-wsl/hook-relay'
 ...
 *  namespaces the guest install dir, so concurrent CaPilot instances with
 ...
 *  after an CaPilot restart — the exact re-coordination this exists to serve. */
...
export function wslHookRelayEndpointDir(guestHome: string, instanceKey: string): string {
  ...
  return `${home}/.capilot-wsl/agent-hooks/instance-${instanceKey}`
}
