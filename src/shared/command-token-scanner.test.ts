...
describe('command token scanner', () => {
  ...
  it('preserves quoted command paths with spaces', () => {
    expect(getFirstCommandToken('"C:\\Program Files\\CaPilot\\codex.cmd" --resume')).toBe(
      'C:\\Program Files\\CaPilot\\codex.cmd'
    )
  })

  it('extracts path basenames without allocating path segment arrays', () => {
    expect(getCommandTokenPathBasename('C:\\Program Files\\CaPilot\\codex.cmd')).toBe('codex.cmd')
