...
export type TrackedClaudeSubagent = {
   ...
   *  observed (CaPilot restart). A present complete task list omitting it
 ...
 *    never saw start (CaPilot/relay restart mid-run) → recreate it;
 ...
 *  `background_tasks` list; a parent that went idle before CaPilot restarted never
