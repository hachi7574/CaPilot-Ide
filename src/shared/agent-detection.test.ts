...
describe('Pi-compatible title detection', () => {
  ...
  it.each([
    ...
    ['\u280b π - tmp', 'omp', '\u280b OMP'],
    ['\u280b OMP', 'pi', '\u280b Pi'],
    ['lucky-echidna | \u283c π - Diagnose CaPilot terminal title flicker - test', 'omp', '\u280b OMP'],
...
describe('Cursor agent title identity', () => {
  // Why: the accepted vocabulary is the set of labels CaPilot actually synthesizes for Cursor.
  ...
  it('accepts every label CaPilot synthesizes for Cursor', () => {
  ...
  ])('accepts the native or CaPilot-synthesized Cursor title %j', (title) => {
  ...
  it.each([
    ...
    'Terminal Cursor and CaPilot slows down',
    'cursor-agent',
    'cursor.exe',
    '~/cursor-rules',
    '',
