...
export type Repo = {
  ...
  /** Controls CaPilot's fork-default-branch sync offer for repos with upstream metadata. */
  forkSyncMode?: ForkSyncMode
  /** Canonical identity for the repo remote CaPilot should use for provider-level grouping. */
  gitRemoteIdentity?: GitRemoteIdentity | null
  /** Controls whether worktrees CaPilot did not create appear in the sidebar. */
...
export type Worktree = {
  ...
  manualOrder?: number
  lastActivityAt: number
  /** Set once when CaPilot creates the worktree. Absent for worktrees discovered
  ...
  createdAt?: number
  /** Agent selected when CaPilot originally created the worktree. Used only to
  ...
  baseRef?: string
  /** Remote/branch CaPilot should publish review commits to when it created this worktree. */
...
/** Provenance for workspaces created through `capilot worktree create`. Absent on
...
export type CliWorkspaceProvenance = {
  kind: 'created-by-cli'
  createdAt: number
  /** CaPilot terminal the CLI ran inside, when the caller had one — distinguishes
...
export type GitPushTarget = {
  ...
  branchName: string
  remoteUrl?: string
  /** True when CaPilot added this remote while preparing a fork-PR worktree. */
...
export type WorktreeMeta = {
  ...
  manualOrder?: number
  lastActivityAt: number
  /** See {@link Worktree.createdAt}. Persisted to capilot-data.json. */
  createdAt?: number
  /** See {@link Worktree.createdWithAgent}. Persisted to capilot-data.json. */
  createdWithAgent?: TuiAgent
  ...
  /** Intended create base for stale-base probes. Persisted metadata, not UI drift state. */
  baseRef?: string
  /** True when CaPilot checked out a pre-existing local branch that delete must not prune. */
  preserveBranchOnDelete?: boolean
  /** See {@link Worktree.pushTarget}. Persisted so refreshed worktree lists keep the target. */
  pushTarget?: GitPushTarget
  /** Explicit marker stamped when CaPilot creates the worktree. */
  orcaCreatedAt?: number
  orcaCreationSource?: 'desktop' | 'runtime' | 'cli' | 'ssh'
  /** Workspace layout active when CaPilot created the worktree. */
  ...
  automationProvenance?: AutomationWorkspaceProvenance
  /** System-owned provenance for workspaces created via `capilot worktree create`. */
  cliProvenance?: CliWorkspaceProvenance
}

export type WorktreeOwnership = 'capilot-managed' | 'external' | 'unknown-legacy' | 'agent-scratch'
...
// existing persistence layer writes them to capilot-data.json automatically.
...
export type TerminalTab = {
   ...
   *  Why: agent CLIs overwrite the live title via OSC updates, but CaPilot still
  ...
  startupCwd?: string
  /** Why: the coding-harness agent CaPilot launched in this tab. Lets the tab bar
...
export type BrowserWorkspace = {
  id: string
  worktreeId: string
  /** Stable display label for the outer CaPilot tab ("Browser 1", "Browser 2", …).
  ...
  // CaPilot tab strip still renders one workspace entry. Mirror the active page's
...
export type GitHubWorkItemDetails = {
  // Why: main-process doesn't know CaPilot's Repo.id, so this inner item omits
 ...
 *  system-default (activeAccountId:null) Codex account. CaPilot reads this to
...
/** All AI coding agents CaPilot knows how to launch. Used for the agent picker in the new-workspace
 *  flow and for the default-agent setting. Extend this union as new agents are added. */
export type TuiAgent =
  | 'claude' // Claude Code
  | 'claude-agent-teams' // Claude Code Agent Teams via CaPilot native panes
...
export type GlobalSettings = {
  ...
  /** When enabled, CaPilot renames a workspace's auto-generated creature branch to
  ...
  /** macOS: toggles the additive menu-bar entry (CaPilot survives last-window close); doesn't change Dock behavior. */
  ...
  /** Whether to show the CaPilot app name in the titlebar. */
  ...
  /** Only toggles the sidebar shortcut; CaPilot Mobile stays reachable from Settings. */
  ...
  /** CaPilot-first keeps app shortcuts from TUIs; terminal-first is opt-in to let shell/TUI bindings win. */
  ...
  /** Legacy keyboard-shortcut overrides; new writes go to ~/.capilot/keybindings.json, migrated once when present. */
  ...
  /** Why: durable main-owned pref so CaPilot can prepare shared ~/.codex before the renderer hydrates. */
   ...
   *  History-only: does not change which account/config/hooks CaPilot uses. */
  ...
  /** When true, CaPilot requests local awake assertions while hook-reported agents are working. */
...
export type WorktreeCardProperty =
  ...
  | 'pr'
  | 'automation'
  // Badge marking workspaces created through `capilot worktree create`.
  | 'cli'
  | 'comment'
  | 'ports'
...
export type PersistedUIState = {
  ...
  /** Hide workspaces created through `capilot worktree create`. */
  ...
  /** Whether CaPilot already tried triggering the macOS notification permission dialog; prevents re-firing every launch. */
  ...
  /** Why: Windows-only. Set once on first hide to tray so the "CaPilot is still running" notice shows only once. */
  ...
  /** Once the user has starred CaPilot (any entry point), permanently suppress the nag. */
...
export type AppMemory = UsageValues & {
  ...
  other: UsageValues
  /** Oldest-first memory samples (bytes) for the whole CaPilot app; empty before the first snapshot. */
  history: number[]
}
