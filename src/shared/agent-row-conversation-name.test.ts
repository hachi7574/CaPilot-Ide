...
describe('getAgentRowConversationName', () => {
  ...
  it('rejects spinner+cwd titles instead of surfacing paths as names', () => {
    expect(
      getAgentRowConversationName(makeTab({ title: '⠋ ~/capilot/workspaces' }), 'codex', false)
    ).toBeNull()
    expect(
      getAgentRowConversationName(makeTab({ title: '/Users/dev/repo' }), 'codex', false)
    ).toBeNull()
    expect(
      getAgentRowConversationName(makeTab({ title: 'C:\\repos\\capilot' }), 'codex', false)
    ).toBeNull()
    expect(
      getAgentRowConversationName(makeTab({ title: 'capilot/workspaces' }), 'codex', false)
    ).toBeNull()
    expect(
      getAgentRowConversationName(
        makeTab({ title: '\\\\wsl.localhost\\Ubuntu\\home\\dev\\capilot' }),
        'codex',
        false
      )
    ).toBeNull()
    expect(
      getAgentRowConversationName(makeTab({ title: 'repos\\capilot' }), 'codex', false)
    ).toBeNull()
  })
  ...
  it('rejects identity-echo, management, and placeholder titles', () => {
    ...
    expect(
      getAgentRowConversationName(makeTab({ title: '◇ Ready (capilot)' }), 'gemini', false)
    ).toBeNull()
    expect(
