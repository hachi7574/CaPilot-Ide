...
export function getInitialAutoManagedWorkspaceName({
  ...
  // Why: a prefilled name counts as user input unless it exactly matches the linked-item seed CaPilot generated.
...
export function useComposerState(options: UseComposerStateOptions): UseComposerStateResult {
  ...
  // Why: SSH remotes deploy the CLI shim as plain `capilot`, so the Linux-only `capilot-ide` rename must not apply to remote launch commands.
  ...
  const uploadComposerPaths = useCallback(
    async (
      ...
      const destinationDir = joinPath(targetRepoPath, '.capilot/drops')
