...
describe('composer drop upload result', () => {
  it('separates imported files and folders while counting skipped or failed paths', () => {
    const results: ComposerDropUploadImportResult[] = [
      { status: 'imported', kind: 'file', destPath: '/repo/.capilot/drops/file.txt' },
      { status: 'imported', kind: 'directory', destPath: '/repo/.capilot/drops/folder' },
      { status: 'skipped' },
      { status: 'failed' }
    ]

    expect(collectComposerDropUploadResult(results)).toEqual({
      filePaths: ['/repo/.capilot/drops/file.txt'],
      folderPaths: ['/repo/.capilot/drops/folder'],
      skippedOrFailed: 2
    })
  })
