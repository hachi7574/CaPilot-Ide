...
export const AGENT_SKILL_CLI_PREREQUISITE_NOTICE =
  'Before opening setup, CaPilot may show a system prompt to register the CaPilot CLI command on PATH.'

export const CLI_PREREQUISITE_REGISTRATION_TOAST = 'CaPilot needs to register its CLI on PATH.'
export const CLI_PREREQUISITE_REGISTRATION_TOAST_DESCRIPTION =
  'Approve the system prompt so skill setup can use the CaPilot CLI command.'
    ...
    toast.error(
      error instanceof Error
        ...
        : translate(
            ...
            'Failed to register the CaPilot CLI in PATH.'
          )
    )
    return null
  }
...
function showCliPrerequisiteWarning(status: CliInstallStatus): void {
  if (!status.supported) {
    toast.warning(
      translate(
        ...
        'CaPilot CLI registration is unavailable'
      ),
      {
        description:
          status.detail ??
          translate(
            ...
            'Install the CaPilot CLI before running agent skill setup.'
          )
      }
    )
    return
  ...
  if (status.state !== 'installed') {
    toast.warning(
      translate(
        ...
        'CaPilot CLI registration needs attention'
      ),
      {
        description:
          status.detail ??
  ...
  if (status.pathConfigured === null) {
    toast.warning(
      translate(
        ...
        'CaPilot could not check your Windows user PATH'
      ),
  ...
  if (status.pathConfigured === false) {
    ...
    // CaPilot affordances when that shell can resolve the CaPilot CLI command.
    toast.warning(
      translate(
        'auto.lib.agent.skill.cli.prerequisite.79371593b0',
        'CaPilot CLI is not visible on PATH yet'
      ),
      {
        description:
          status.detail ??
          translate(
            ...
            'Restart your shell or add the CaPilot CLI directory to PATH before setup.'
          )
      }
    )
  }
