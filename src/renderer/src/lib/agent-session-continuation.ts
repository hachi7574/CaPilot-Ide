...
export function buildAgentSessionContinuationPrompt(
  ...
  const sourceLines = [
    ...
    source.sourceLabel ? `CaPilot pane: ${source.sourceLabel}` : null,
  ...
  return [
    'Continue work from the prior CaPilot session using the context below.',
    ...
    ...(statusHints.length > 0 ? ['', 'Latest CaPilot status hints:', ...statusHints] : []),
    '',
