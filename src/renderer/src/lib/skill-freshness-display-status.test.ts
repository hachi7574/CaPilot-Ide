...
describe('getSkillFreshnessDisplayStatus', () => {
  ...
  it.each([
    ...
    // Why: these are CaPilot's own bounds. A large but healthy plugin cache would
  ...
  // bound CaPilot chose, so the only one a person can actually clear.
...
describe('hasSkillCopyNeedingAttention', () => {
  ...
  // copy inside a work directory. CaPilot only updates global skills, so the project copy
  ...
  // presence rather than a freshness claim about a copy CaPilot does not manage.
  ...
  // Why: an unreadable plugin path could hide a copy of anything, but a skill CaPilot
