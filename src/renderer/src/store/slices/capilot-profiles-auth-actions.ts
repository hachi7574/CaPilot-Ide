...
> = (set, get) => ({
  createCloudLinkedOrcaProfile: async (args) => {
    try {
      ...
      if (result.status === 'created') {
        toast.success(
          translate('auto.store.slices.capilot.profiles.319d7cf39b', 'Cloud profile created')
        )
      } else if (result.status === 'reconnect-required') {
        toast.error(
          translate('auto.store.slices.capilot.profiles.d6e764e7db', 'Reconnect this profile')
        )
      } else if (result.status === 'failed') {
        toast.error(
          translate('auto.store.slices.capilot.profiles.f0c9e11a6d', 'Failed to create cloud profile'),
          { description: result.error }
        )
      }
      return result
    } catch (err) {
      console.error('Failed to create CaPilot cloud profile:', err)
      toast.error(
        translate('auto.store.slices.capilot.profiles.f0c9e11a6d', 'Failed to create cloud profile'),
        {
  ...
  connectCurrentOrcaProfile: async () => {
    ...
    try {
      ...
      if (result.status === 'unconfigured') {
        toast.error(
          translate(
            'auto.store.slices.capilot.profiles.8b8fa73174',
            'CaPilot Cloud sign-in is not configured'
          ),
          {
        ...
        toast.error(
          translate('auto.store.slices.capilot.profiles.33290e88ed', 'Failed to connect profile'),
          { description: result.error }
        )
      } else if (result.status === 'connected') {
        toast.success(translate('auto.store.slices.capilot.profiles.9fcb07a796', 'Profile connected'))
      }
      return result
    } catch (err) {
      console.error('Failed to connect CaPilot profile:', err)
  ...
  refreshCurrentOrcaProfileAuth: async () => {
    try {
      ...
      if (result.status === 'reconnect-required') {
        ...
        toast.error(
          translate('auto.store.slices.capilot.profiles.2f6c78a039', 'Failed to refresh profile auth'),
          { description: result.error }
        )
      }
      return result
    } catch (err) {
      console.error('Failed to refresh CaPilot profile auth:', err)
      toast.error(
        translate('auto.store.slices.capilot.profiles.2f6c78a039', 'Failed to refresh profile auth'),
        {
          description: err instanceof Error ? err.message : String(err)
  ...
  signOutCurrentOrcaProfile: async () => {
    try {
      ...
      toast.success(
        translate('auto.store.slices.capilot.profiles.a37b5e6d37', 'Signed out of profile')
      )
      return result
    } catch (err) {
      console.error('Failed to sign out of CaPilot profile:', err)
      toast.error(translate('auto.store.slices.capilot.profiles.83600521e7', 'Failed to sign out'), {
        description: err instanceof Error ? err.message : String(err)
      })
      return null
    }
  ...
  selectOrcaProfileOrg: async (orgId) => {
    try {
      ...
      if (result.status === 'reconnect-required') {
        ...
        toast.error(
          translate('auto.store.slices.capilot.profiles.76deec8f58', 'Failed to switch organization'),
          { description: result.error }
        )
      }
      return result
    } catch (err) {
      console.error('Failed to switch CaPilot profile org:', err)
      toast.error(
        translate('auto.store.slices.capilot.profiles.76deec8f58', 'Failed to switch organization'),
        {
          description: err instanceof Error ? err.message : String(err)
