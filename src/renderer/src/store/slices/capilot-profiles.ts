...
} from './capilot-profiles-auth-actions'
...
export const createOrcaProfilesSlice: StateCreator<AppState, [], [], OrcaProfilesSlice> = (
  ...
  fetchOrcaProfiles: async () => {
    ...
    try {
      ...
      console.error('Failed to fetch CaPilot profiles:', err)
  ...
  fetchOrcaProfileAuthStatus: async () => {
    try {
      ...
      console.error('Failed to fetch CaPilot profile auth status:', err)
      return null
    }
  },
  ...
  createLocalOrcaProfile: async (name) => {
    try {
      ...
      console.error('Failed to create CaPilot profile:', err)
      toast.error(
        translate('auto.store.slices.capilot.profiles.612f7f6861', 'Failed to create profile'),
        {
  ...
  switchOrcaProfile: async (profileId) => {
    ...
    try {
      ...
      console.error('Failed to switch CaPilot profile:', err)
      set({ orcaProfileSwitching: false })
      toast.error(
        translate('auto.store.slices.capilot.profiles.7d4bc516ee', 'Failed to switch profile'),
        {
          description: err instanceof Error ? err.message : String(err)
        }
      )
  ...
  transferOrcaProfileProject: async (args) => {
    try {
      ...
      if (result.status === 'duplicate-target') {
        toast.error(
          translate(
            'auto.store.slices.capilot.profiles.f518e89aa5',
      ...
      console.error('Failed to transfer CaPilot profile project:', err)
      toast.error(
        translate('auto.store.slices.capilot.profiles.f03ae7f27b', 'Failed to transfer project'),
        {
          description: err instanceof Error ? err.message : String(err)
        }
      )
