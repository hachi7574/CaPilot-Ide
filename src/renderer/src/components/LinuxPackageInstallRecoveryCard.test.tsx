...
const SUMMARY = 'CaPilot downloaded the update but could not install the system package automatically.'
const COPIED_NOTE =
  `Command copied. Run it in a system terminal to install ${PACKAGE_FILE_NAME}, ` +
  'then quit and reopen CaPilot.'
const INSTRUCTIONS = {
  ok: true as const,
...
describe('LinuxPackageInstallRecoveryCard copy', () => {
  it('leads with the recovery copy and the three dedicated actions', () => {
    ...
    expect(
      screen.getByText(/a system terminal on the computer where CaPilot is installed/)
    ).toBeTruthy()
    expect(screen.getByText(/quit and reopen CaPilot to run the new version/)).toBeTruthy()
...
describe('LinuxPackageInstallRecoveryCard details', () => {
  ...
  it('scopes the trust note to the moment the command is built', () => {
    ...
    const detail = screen.getByText(/CaPilot checks the downloaded file against the release metadata/)
    expect(detail.textContent).toContain('at the moment it builds this command')
    expect(detail.textContent).toContain(
      'The system package itself is not signature-checked, and CaPilot cannot vouch for the file ' +
        'after that point.'
    )
  })
