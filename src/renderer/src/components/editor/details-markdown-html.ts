...
export function parseDetailsAttributes(rawAttributes: string): Record<string, unknown> {
  ...
  const variantMatch = rawAttributes.match(
    /\sdata-capilot-toggle\s*=\s*(?:"(heading-[1-5])"|'(heading-[1-5])'|(heading-[1-5]))(?:\s|$)/i
  )
  return {
...
export function renderDetailsAttributes(attrs: Record<string, unknown> | undefined): string {
  const attributes = ['class="capilot-details"']

  const variant = parseToggleHeadingVariant(attrs?.variant)
  if (variant) {
    attributes.push(`data-capilot-toggle="${variant}"`)
  }
...
function hasOnlySupportedDetailsAttributes(rawAttributes: string): boolean {
  return (
    rawAttributes
      ...
      .replace(/\s+class\s*=\s*(?:"capilot-details"|'capilot-details'|capilot-details)(?=\s|$)/giu, '')
      .replace(
        /\s+data-capilot-toggle\s*=\s*(?:"heading-[1-5]"|'heading-[1-5]'|heading-[1-5])(?=\s|$)/giu,
        ''
      )
      .trim() === ''
  )
