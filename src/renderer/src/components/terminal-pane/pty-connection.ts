...
export const STARTUP_CWD_FALLBACK_NOTICE =
  '\r\n[CaPilot opened this terminal at the workspace root because its saved start folder no longer exists.]\r\n'
...
export function connectPanePty(
  ...
  const registerEffectiveLaunchConfig = (
    ...
    if (!effectiveLaunchConfig) {
      if (metadata?.launchAgent) {
        // Why: daemon launch identity can outlive the process while CaPilot is
  ...
  const scheduleReattachIdleAgentCursorReset = (): void => {
    ...
    reattachIdleAgentCursorResetTimer = setTimeout(() => {
      ...
      // fire because the agent was already idle before CaPilot restarted.
      queueAgentIdleTerminalModeReset()
  ...
  const dropCommandFinishedStatusIfSameTurn = (
    ...
    const state = useAppStore.getState()
    if (!entry) {
      // Why: an CaPilot-started agent can exit before its first hook status. The
  ...
  const onTerminalKeyDown = (event: KeyboardEvent): void => {
    ...
    // Why: user shell frameworks (bash-preexec/iTerm2) can replace CaPilot's
  ...
  const onBell = (): void => {
    ...
    // standalone BEL some time after daemon snapshot reattach, even when CaPilot
  ...
  // callbacks to the correct CaPilot pane without resolving worktrees from cwd.
  // The key matches the `${tabId}:${leafId}` composite used for cacheTimerByKey
  // and agentStatusByPaneKey. Treat it as opaque outside CaPilot.
  const state = useAppStore.getState()
  ...
  const runDeferredConnect = (): void => {
    ...
    const schedulePendingStartupCommandDelivery = (): void => {
      ...
      if (!sshStartupShellReady) {
        if (sshShellReadyFallbackTimer === null) {
          // Why: some SSH shells cannot emit CaPilot's ready marker. Prefer the
