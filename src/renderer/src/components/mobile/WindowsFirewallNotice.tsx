        ...
        if (
          ...
          toast.success(
            translate(
              ...
              'Windows Firewall now allows CaPilot Mobile on private networks'
            )
          )
          return
        }
              ...
              {networkIsPublic
                ...
                : blockingRuleDetected
                  ? translate(
                      ...
                      'Windows may be blocking CaPilot Mobile'
                    )
                  : translate(
            ...
            <p className="text-xs text-muted-foreground">
              {networkIsPublic
                ? translate(
                    ...
                    'Change this trusted Wi-Fi network to Private before allowing CaPilot Mobile connections.'
                  )
                : blockingRuleDetected
                  ? translate(
                      'auto.components.mobile.WindowsFirewallNotice.blocked-description',
                      'An existing inbound Block rule can override the pairing exception. Repair removes conflicting TCP rules for this CaPilot app, then allows port {{port}} on Private networks.',
                      { port: firewallStatus.port }
                    )
                  : translate(
                      'auto.components.mobile.WindowsFirewallNotice.missing-description',
                      'Windows may block the pairing server. Add a rule for this CaPilot app and TCP port {{port}} on Private networks.',
                      { port: firewallStatus.port }
                    )}
            </p>
          </div>
