          ...
          <p className="text-xs text-muted-foreground">
            {status === 'unavailable' ? (
              ...
              translate(
                ...
                'Bitbucket credentials are configured but could not authenticate. Check the token and repository permissions, then restart CaPilot if environment variables changed.'
              )
            )}
          </p>
            ...
            {status === 'unavailable' ? (
              ...
              <>
                ...
                {translate(
                  ...
                  'only when CaPilot cannot derive the API base URL from the git remote.'
                )}
              </>
            ) : (
              translate(
                'auto.components.settings.token.source.control.integration.cards.40f678df73',
                'Azure DevOps credentials are configured but could not authenticate. Check the token, API base URL, and repository permissions, then restart CaPilot if environment variables changed.'
              )
            )}
          </p>
          <div className="flex items-center gap-2">
            ...
            {status === 'unavailable' ? (
              ...
              <>
                ...
                {translate(
                  ...
                  'only when CaPilot cannot derive the API URL from the remote.'
                )}
              </>
            ) : (
              translate(
                'auto.components.settings.token.source.control.integration.cards.19fb419c12',
                'Gitea credentials are configured but could not authenticate. Check the token, API base URL, and repository permissions, then restart CaPilot if environment variables changed.'
              )
            )}
          </p>
          <div className="flex items-center gap-2">
