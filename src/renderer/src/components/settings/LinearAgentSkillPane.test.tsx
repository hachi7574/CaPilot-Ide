...
const UPDATE_COMMAND = 'npx skills update capilot-linear --global'

const mocks = vi.hoisted(() => ({
  panelProps: [] as Record<string, unknown>[],
  runtime: 'native' as 'native' | 'wsl',
  skillInstalled: true,
  updateSkillName: 'capilot-linear',
...
describe('LinearAgentSkillPane', () => {
  afterEach(async () => {
    ...
    mocks.updateSkillName = 'capilot-linear'
  ...
  it('renders checklist with inlined skill panel and usage examples', () => {
    ...
    for (const example of examples) {
      ...
      expect(example.prompt).toContain('/capilot-linear')
  ...
  it('passes the capilot-linear install/update commands and freshness on a local runtime', async () => {
    await renderPane()
