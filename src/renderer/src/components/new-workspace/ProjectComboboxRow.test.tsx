...
describe('MatchedText', () => {
  it('underlines the matched run', () => {
    act(() => {
      root.render(<MatchedText text="capilot" hits={[0, 1, 2, 3]} />)
    })

    expect(markedText()).toBe('capilot')
  })
  ...
  it('aligns marks with the query when the name starts with an emoji', () => {
    ...
    const [match] = rankProjectOptions([option], 'capilot', [])
  ...
  it('leaves text unmarked when there are no hits', () => {
    act(() => {
      root.render(<MatchedText text="🚀 capilot" hits={[]} />)
    })

    expect(container.querySelectorAll('mark')).toHaveLength(0)
    expect(container.textContent).toBe('🚀 capilot')
  })
})
