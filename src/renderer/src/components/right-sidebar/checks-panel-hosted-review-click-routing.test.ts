...
describe('checks panel hosted review modifier hint destination', () => {
  it('names the system browser when a plain click already opens in CaPilot', () => {
    expect(resolveChecksPanelHostedReviewModifierDestination({ openLinksInApp: true }, true)).toBe(
      'system-browser'
    )
  })

  // Why: inverting is inert while links already open in CaPilot — both meanings of the
  // modifier land on the system browser, so checking inverts first would misname it.
  it('names the system browser when inverting is on and links already open in CaPilot', () => {
    expect(
      resolveChecksPanelHostedReviewModifierDestination(
  ...
  // and the only way to reach CaPilot from this button is undiscoverable.
  it('names CaPilot when inverting is on and links open externally', () => {
    expect(
      resolveChecksPanelHostedReviewModifierDestination(
  ...
  // Why: openHttpLink refuses to route a remote-owned link into CaPilot, and openLinksInApp
  ...
  // not suppress a hint for a click that still reaches CaPilot.
  ...
  // Why: openHttpLink gates routing to CaPilot on a worktree id, so without one the
