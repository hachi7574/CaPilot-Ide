...
export type ChecksPanelHostedReviewModifierDestination = 'system-browser' | 'capilot' | null

// Why: mirrors openHttpLink's routing inputs — with inverting on and Link Routing off the
// modifier now reaches CaPilot here, so gating the hint on openLinksInApp alone hides a live gesture.
export function resolveChecksPanelHostedReviewModifierDestination(
  ...
  // runtime id while the click still routes to CaPilot.
  ...
  return settings?.openLinksInAppModifierInverts === true ? 'capilot' : null
}
