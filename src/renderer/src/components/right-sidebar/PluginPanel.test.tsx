...
const dashboardPanel: ActivePluginPanel = {
  ...
  title: 'Dashboard',
  icon: 'gauge',
  tabKey: 'plugin:capilot-samples.my-plugin/dashboard',
  pluginKey: 'capilot-samples.my-plugin',
...
describe('PluginPanel', () => {
  it('renders the panel HTML in a scripts-only sandboxed iframe', async () => {
    ...
    await renderPanel('plugin:capilot-samples.my-plugin/dashboard')
    ...
    expect(initialIframe?.getAttribute('name')).toBe(
      'capilot-plugin-panel:plugin:capilot-samples.my-plugin/dashboard'
    )
  ...
  it('shows an unavailable state for a tab whose plugin is gone', async () => {
    ...
    await renderPanel('plugin:capilot-samples.removed-plugin/dashboard')
