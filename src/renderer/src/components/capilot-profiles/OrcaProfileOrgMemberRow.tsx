...
import type { OrcaOrgMember, OrcaOrgRole } from '../../../../shared/capilot-profiles'
import {
  ORG_ROLE_OPTIONS,
  orgMemberInitials,
  orgRoleLabel
...
export function OrcaProfileOrgMemberRow({
  ...
  // Why: teammates provisioned server-side who never signed into CaPilot have no
  ...
  const roleControl = canManage ? (
    ...
    >
      <SelectTrigger
        size="sm"
        className="w-28"
        aria-label={translate('auto.components.capilot.profiles.org.members.role.label', 'Role')}
      >
        <SelectValue />
      </SelectTrigger>
      <SelectContent>
      ...
      {neverSignedIn && canManage ? (
        <Tooltip>
          ...
          <TooltipContent side="top">
            {translate(
              'auto.components.capilot.profiles.org.members.not.signed.in',
              "They haven't signed in to CaPilot yet."
            )}
          </TooltipContent>
        </Tooltip>
      ) : (
      ...
      {canManage ? (
        confirmingRemove && !actionsDisabled ? (
          <div className="flex items-center gap-1">
            ...
            >
              {translate('auto.components.capilot.profiles.org.members.remove.confirm', 'Remove')}
            </Button>
            <Button variant="ghost" size="xs" onClick={() => setConfirmingRemove(false)}>
              {translate('auto.components.capilot.profiles.org.members.remove.cancel', 'Cancel')}
            </Button>
          </div>
        ) : (
          <Button
            ...
            aria-label={translate(
              'auto.components.capilot.profiles.org.members.remove.label',
              'Remove teammate'
            )}
          >
            {busy ? (
