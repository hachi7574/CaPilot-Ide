...
function getConnectLabel(authStatus: OrcaProfileAuthStatus | null, connecting: boolean): string {
  if (connecting) {
    return translate('auto.components.capilot.profiles.switcher.signInWaiting', 'Waiting for sign-in…')
  }
  if (authStatus?.configured !== true) {
    return translate(
      'auto.components.capilot.profiles.switcher.cloud.unavailable',
      'CaPilot sign-in unavailable'
    )
  }
  if (authStatus.state === 'reconnect-required') {
    return translate('auto.components.capilot.profiles.switcher.signInAgain', 'Sign in again')
  }
  return translate('auto.components.capilot.profiles.switcher.signIn', 'Sign in to CaPilot')
}
    ...
    <>
      {showOrganizationChoices || showCloudProfileCreation ? (
        <>
          <DropdownMenuSeparator />
          <DropdownMenuLabel>
            {translate('auto.components.capilot.profiles.switcher.organization', 'Organization')}
          </DropdownMenuLabel>
          ...
          {showCloudProfileCreation ? (
            <DropdownMenuSub>
              <DropdownMenuSubTrigger disabled={orgActionDisabled}>
                <Plus />
                {translate(
                  'auto.components.capilot.profiles.switcher.create.profile.for.org',
      ...
      {activeProfile.kind === 'cloud-linked' ? (
        <DropdownMenuItem disabled={profileActionDisabled} onSelect={onRequestSignOut}>
          <LogOut />
          {translate('auto.components.capilot.profiles.switcher.signout', 'Sign out')}
        </DropdownMenuItem>
      ) : null}
    </>
  )
