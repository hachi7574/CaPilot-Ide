...
import { getOrcaProfileProjectLiveWorkSummary } from './capilot-profile-switch-liveness'
    ...
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <Button variant="outline" size="xs" disabled={disabled}>
          ...
          {translate('auto.components.capilot.profiles.management.04e7bd2a23', 'Transfer')}
        </Button>
      </DropdownMenuTrigger>
      <DropdownMenuContent align="end" className="w-56">
        <DropdownMenuLabel>
          {translate('auto.components.capilot.profiles.management.128c7dfe64', 'Copy to')}
        </DropdownMenuLabel>
        {renderTargetItems('copy')}
        <DropdownMenuSeparator />
        <DropdownMenuLabel>
          {translate('auto.components.capilot.profiles.management.df8b7d876b', 'Move to')}
        </DropdownMenuLabel>
...
export function OrcaProfileManagementDialog({
  ...
  const runTransfer = async (
    ...
    if (result?.status === 'transferred') {
      toast.success(
        mode === 'move'
          ? translate('auto.components.capilot.profiles.management.9aa26347b3', 'Project moved')
          : translate('auto.components.capilot.profiles.management.816ce624b6', 'Project copied'),
        {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="gap-4 sm:max-w-[640px]">
        <DialogHeader>
          <DialogTitle>
            {translate('auto.components.capilot.profiles.management.2c45bda8d3', 'Manage profiles')}
          </DialogTitle>
          <DialogDescription>
            {translate(
              'auto.components.capilot.profiles.management.2db945e4a0',
          ...
          <div className="flex items-center gap-2 border-b border-border px-3 py-2">
            ...
            <div className="min-w-0">
              ...
              <div className="truncate text-xs text-muted-foreground">
                {translate(
                  repos.length === 1
                    ? 'auto.components.capilot.profiles.management.projectCountSingular'
                    : 'auto.components.capilot.profiles.management.projectCountPlural',
          ...
          <ScrollArea className="max-h-[360px]">
            {repos.length === 0 ? (
              <div className="px-3 py-8 text-center text-sm text-muted-foreground">
                {translate(
                  'auto.components.capilot.profiles.management.8668cb2946',
        ...
        {targetProfiles.length === 0 ? (
          <div className="text-xs text-muted-foreground">
            {translate(
              'auto.components.capilot.profiles.management.93034915ab',
