...
import {
  ...
  ORG_ROLE_OPTIONS,
  orgRoleLabel
} from './capilot-profile-org-members-labels'
      ...
      <DialogContent className="sm:max-w-[520px]">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            {translate('auto.components.capilot.profiles.org.members.title', 'Organization members')}
          </DialogTitle>
          <DialogDescription>
            {orgName
              ? translate(
                  'auto.components.capilot.profiles.org.members.subtitle.named',
                  'People in {{orgName}} who can collaborate on CaPilot.',
                  { orgName }
                )
              : translate(
                  'auto.components.capilot.profiles.org.members.subtitle.default',
                  'People in your organization who can collaborate on CaPilot.'
                )}
          </DialogDescription>
        </DialogHeader>
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center gap-3 py-8 text-center">
            <p className="text-sm text-muted-foreground">
              {translate(
                'auto.components.capilot.profiles.org.members.load.error',
                "Couldn't load organization members."
              )}
            </p>
            <Button variant="outline" size="sm" onClick={() => void refresh()}>
              <RefreshCw className="size-3.5" />
              {translate('auto.components.capilot.profiles.org.members.retry', 'Try again')}
            </Button>
          </div>
        ) : roster ? (
            ...
            {canManage && roster.pendingInvites.length > 0 ? (
              <div className="flex flex-col gap-1.5">
                <p className="text-[11px] font-semibold tracking-wide text-muted-foreground uppercase">
                  {translate(
                    'auto.components.capilot.profiles.org.members.pending.title',
                    'Pending invites'
                  )}
                </p>
                {roster.pendingInvites.map((invite) => (
                  <div key={invite.email} className="flex items-center gap-2.5 py-1">
                    ...
                    >
                      ...
                      {translate(
                        'auto.components.capilot.profiles.org.members.pending.revoke',
                        'Revoke'
                      )}
                    </Button>
                  </div>
            ...
            {canManage ? (
              <form onSubmit={(event) => void handleInvite(event)} className="flex items-end gap-2">
                <div className="flex min-w-0 flex-1 flex-col gap-1">
                  <label
                    htmlFor="capilot-org-invite-email"
                    className="text-[11px] font-semibold tracking-wide text-muted-foreground uppercase"
                  >
                    {translate('auto.components.capilot.profiles.org.members.invite.title', 'Invite')}
                  </label>
                  <Input
                    id="capilot-org-invite-email"
                    type="email"
                    value={inviteEmail}
                    onChange={(event) => setInviteEmail(event.target.value)}
                    disabled={inviting}
                    placeholder={translate(
                      'auto.components.capilot.profiles.org.members.invite.placeholder',
                ...
                >
                  <SelectTrigger
                    ...
                    aria-label={translate(
                      'auto.components.capilot.profiles.org.members.invite.role',
                      'Invite role'
                    )}
                  >
                    <SelectValue />
                ...
                <Button type="submit" size="sm" disabled={inviting || !inviteEmail.trim()}>
                  ...
                  {translate('auto.components.capilot.profiles.org.members.invite.submit', 'Send')}
                </Button>
              </form>
            ) : null}
          </div>
