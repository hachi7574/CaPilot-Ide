...
export function orgRoleLabel(role: OrcaOrgRole): string {
  switch (role) {
    case 'owner':
      return translate('auto.components.capilot.profiles.org.members.role.owner', 'Owner')
    case 'admin':
      return translate('auto.components.capilot.profiles.org.members.role.admin', 'Admin')
    case 'member':
      return translate('auto.components.capilot.profiles.org.members.role.member', 'Member')
  }
}
...
export function describeOrgMutationError(
  ...
  switch (result.status) {
    case 'forbidden':
      return translate(
        'auto.components.capilot.profiles.org.members.error.forbidden',
        "Your role can't do that."
      )
    case 'conflict':
      return translate(
        'auto.components.capilot.profiles.org.members.error.conflict',
        'Already a member or invited.'
      )
    case 'invalid':
      return result.reason === 'cannot_remove_self'
        ? translate(
            'auto.components.capilot.profiles.org.members.error.remove.self',
            "You can't remove yourself."
          )
        : translate(
            'auto.components.capilot.profiles.org.members.error.own.role',
            "You can't change your own role."
          )
    case 'not-found':
      return translate(
        'auto.components.capilot.profiles.org.members.error.not.found',
        'That teammate is no longer in this organization.'
      )
    case 'reconnect-required':
      return translate(
        'auto.components.capilot.profiles.org.members.error.reconnect',
        'Reconnect your profile to manage members.'
      )
    case 'unconfigured':
      return translate(
        'auto.components.capilot.profiles.org.members.error.unconfigured',
        'Organization management is unavailable in this build.'
      )
    case 'failed':
      return translate(
        'auto.components.capilot.profiles.org.members.error.failed',
