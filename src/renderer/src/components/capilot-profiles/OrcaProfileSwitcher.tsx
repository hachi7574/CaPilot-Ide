...
import type { OrcaCloudOrgSummary, OrcaProfileSummary } from '../../../../shared/capilot-profiles'
...
import { getOrcaAccountIdentity } from './capilot-account-identity'
import { getOrcaProfileSwitchLiveWorkSummary } from './capilot-profile-switch-liveness'
...
function getProfileSubtitle(profile: OrcaProfileSummary): string {
  ...
  return translate('auto.components.capilot.profiles.switcher.b4f9d1125d', 'Local')
}
...
export function OrcaProfileSwitcher({
  ...
  // Why: the CaPilot Cloud account UX isn't ready for production users yet, so the
  ...
  const triggerLabel = multiProfileUi
    ? translate('auto.components.capilot.profiles.switcher.4815f7d163', 'Switch profile')
    : translate('auto.components.capilot.profiles.switcher.account', 'Account')
        ...
        >
          ...
          {showOrgMembers ? (
            <>
              ...
              >
                <Users />
                {translate(
                  'auto.components.capilot.profiles.switcher.org.members',
          ...
          {multiProfileUi ? (
            <>
              ...
              >
                <Settings2 />
                {translate('auto.components.capilot.profiles.switcher.d00d853e2a', 'Manage profiles')}
              </DropdownMenuItem>
              <DropdownMenuItem
                disabled={profileActionDisabled}
                onSelect={() => {
              ...
              >
                <Plus />
                {translate(
                  'auto.components.capilot.profiles.switcher.c106c674fe',
                  'New local profile'
                )}
              </DropdownMenuItem>
            </>
