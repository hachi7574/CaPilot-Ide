...
export function DiffCommentCard({
  ...
  return (
    <div ref={cardRef} className="capilot-diff-comment-card">
      <div className="capilot-diff-comment-content-col">
        {/* Header Row */}
        <div className="capilot-diff-comment-header">
          <div className="capilot-diff-comment-meta-group">{metaText}</div>

          {/* Action buttons pill (only shown if not editing) */}
          {!editing && (
            <div
              className="capilot-diff-comment-actions-pill"
              onMouseDown={(ev) => ev.stopPropagation()}
            >
              {headerActions}
              {headerActions && (url || onSubmitEdit || onDelete) && (
                <span className="capilot-diff-comment-pill-divider" />
              )}
              {url && (
                <>
                  <button
              ...
              {onSubmitEdit && (
                <>
                  ...
                  {onDelete && <span className="capilot-diff-comment-pill-divider" />}
                </>
              )}
              {onDelete && (
                <button
                  type="button"
                  className="capilot-diff-comment-pill-btn capilot-diff-comment-pill-btn-danger"
                  title={translate(
        ...
        {/* Quote Block */}
        {quote ? (
          <div className="capilot-diff-comment-quote">
            <div className="capilot-diff-comment-quote-text">{quote}</div>
          </div>
        ) : null}
        ...
        {editing ? (
          ...
          <div className="capilot-diff-comment-body">{body}</div>
        )}
      </div>
    </div>
  )
