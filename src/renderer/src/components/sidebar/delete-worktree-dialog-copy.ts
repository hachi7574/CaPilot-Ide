  ...
  return {
    ...
    descriptionSuffix: args.isBatchDelete
      ? allFolderWorkspaceDeletes
        ? 'from CaPilot. Project folders on disk will not be deleted.'
        : mixedFolderWorkspaceDeletes
          ? 'from CaPilot. Git worktrees will also be removed from git and disk; folder workspaces will only remove the CaPilot workspace entry.'
          : 'from git and delete their workspace folders.'
      : args.isFolderWorkspaceDelete
        ? 'from CaPilot. The project folder on disk will not be deleted.'
