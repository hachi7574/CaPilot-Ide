...
describe('shouldDeferRemoteFileBrowserPasteResolve', () => {
  it('keeps small path and filter pastes on the immediate resolver path', () => {
    expect(shouldDeferRemoteFileBrowserPasteResolve('/workspaces/capilot/src')).toBe(false)
...
describe('parsePathInput', () => {
  ...
  it('`Documents/capilot` commits `Documents` and filters by `capilot`', () => {
    expect(parsePathInput('Documents/capilot')).toEqual({
      mode: 'path',
      base: 'cwd',
      committedSegments: ['Documents'],
      trailingFilter: 'capilot'
    })
  })
...
describe('resolveSegmentStep', () => {
  const listing: DirEntry[] = [
    ...
    { name: 'capilot-internal', isDirectory: true },
  ...
  it('unique prefix descends', () => {
    expect(resolveSegmentStep('capilot', '/home/neil', listing)).toEqual({
      type: 'descend',
      name: 'capilot-internal'
    })
  })
