---
name: capilot-emulator-android
description: >
  Control an Android emulator / device from inside CaPilot using the `capilot` CLI.
  ...
  (Windows, Linux, macOS). Complements the capilot-emulator (iOS) and capilot-cli skills.
license: Apache-2.0
---

# CaPilot Emulator — Android (adb / emulator powered)

Drive an Android emulator or adb-connected device **from within CaPilot** using
...
Windows, Linux, and macOS — unlike the iOS backend (`capilot-emulator`), which is
...
- iOS simulators → use the `capilot-emulator` skill (macOS only).
...
## Prerequisites (surfaced by CaPilot)

- **Android Studio / Android SDK** installed, with `ANDROID_HOME` (or
  `ANDROID_SDK_ROOT`) set. CaPilot also checks the per-OS default location
...
CaPilot returns a clear message when the SDK is missing
...
```text
┌────────────────────────┐
│ capilot CLI (agents)      │  e.g. ORCA emulator tap 0.5 0.7 --device emulator-5554
└───────────┬────────────┘
            │ RPC
            ▼
...
CaPilot owns backend routing and the per-worktree active-device registry. The
Android backend converts CaPilot's normalized 0–1 coordinates to device pixels and
...
(top-left origin) — never pixels; CaPilot converts using the live screen size.
...
- **All coordinates are normalized 0..1** (top-left origin), never pixels — CaPilot
...
See also: `capilot-emulator` (iOS, macOS-only), `capilot-cli` (terminals, worktrees,
