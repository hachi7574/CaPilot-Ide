...
CaPilot is a **thin wrapper**: you guide, detect, and scaffold; you never own the user's cloud account,
...
**The one branch that shapes everything — connection mode:** **CaPilot-server** (`create` runs `capilot serve`
in the env and emits a `pairingCode`; §7c/§7f) vs **SSH** (`create` runs no server and emits a
`connection.type:"ssh"` block CaPilot dials into; §7g/§7h). Settle this first — it changes the `create`
output shape and half the templates.

**Quick-start (happy path):** interview the user (connection mode CaPilot-server vs SSH, provider, agent CLI,
git auth — §1.2) + read the provider's CLI docs → scaffold `scripts/capilot-vm/` from §7 → run the
base-snapshot script, then the auth script (you invoke these by hand; not via `capilot.yaml`) → wire
`environmentRecipes` in `capilot.yaml` → `capilot vm recipe doctor <id> --json` (free) → then the `--provision`
...
a long time, or need the user at the keyboard. Never create an CaPilot workspace or commit unless asked.

1. **Inspect the repo** for an existing `environmentRecipes` entry, `scripts/capilot-vm/`, a state file, or setup
...
2. **Interview the user up front** — gather these choices and confirm them back before scaffolding
   ...
   - **Connection mode:** how CaPilot attaches to the environment — an **CaPilot server** (the VM runs
     `capilot serve` and CaPilot pairs over its pairing URL; worked example §7f) or **SSH** (CaPilot connects to
   ...
   - **Provider:** Vercel Sandbox, Fly, Modal, an existing SSH host, … For non-obvious providers, also
     ...
     (host/port/user/key or proxy command) or only a provider-mediated interactive shell; CaPilot SSH mode
     needs the former.
...
7. **Wire the recipe** so `capilot.yaml` points create/suspend/resume/destroy at the scripts (§8). The
   workspace composer reads `environmentRecipes` from the project's primary checkout of `capilot.yaml`, **not** from
   a feature branch or worktree. So a recipe added only on a branch won't appear as a "Run on" option
   until that `capilot.yaml` change is committed and merged to the project's primary branch. Tell the user
...
8. **Dry-run doctor** — `capilot vm recipe doctor <recipe-id> --repo-path <repo> --json` (free, static; §9).
   Fix every failure before going live.
9. **[CHECKPOINT] Live self-test** — get the user's OK once, then run
   `capilot vm recipe doctor <recipe-id> --provision --json` as a loop: it runs create → validates →
...
- **Connection mode** (CaPilot server vs SSH) confirmed with the user — see §1 step 2; it shapes the recipe.
...
A repo-local JSON file (e.g. `scripts/capilot-vm/<provider>-state.json`) threads non-secret values between
...
Scaffold under `scripts/capilot-vm/`. These are **shapes** — fill in the provider's real commands. All
...
- **Local-side** (`create`/`suspend`/`resume`/`destroy` + the base-snapshot/auth scripts the user
  ...
  or require WSL/Git-Bash and point `capilot.yaml` at the right launcher.
...
Worked Vercel commands for this phase are in §7f. You run this script by hand (not via `capilot.yaml`),
...
# name = capilot-${ORCA_VM_RECIPE_ID}-${ORCA_VM_INSTANCE_ID} (sanitized, length-capped)
...
# 3. remote exec: start capilot serve in the background and read the recipe JSON it writes (see below)
...
**The exact `capilot serve` invocation and its output (verified — do not improvise the flags).** Inside the
VM, run:
...
**Binary name:** in a VM built from source (the Phase-2 flow), run it as `pnpm exec capilot-dev serve …`
from the repo root — `capilot-dev` is the in-repo entrypoint and is what the §7f example uses. Plain
`capilot serve …` is the same command when the built CLI is installed on the VM's PATH. The flags/output
...
```json
{ "schemaVersion": 1, "pairingCode": "<capilot pairing URL>", "projectRoot": "<the --project-root you passed>" }
```
...
#!/usr/bin/env bash
set -euo pipefail
payload="$(cat)"                       # CaPilot passes lifecycle JSON on stdin
...
# destroy: provider remove "$resource_id"   (or set destroy: none in capilot.yaml)
```
...
name="capilot-${ORCA_VM_RECIPE_ID:-vercel-sandbox}-${ORCA_VM_INSTANCE_ID:-$(date +%s)}"  # sanitize+cap to 63 chars
...
vercel sandbox exec "$name" "${vercel_args[@]}" --timeout 20m \
  ...
  -- bash -lc 'set -euo pipefail; cd "$ORCA_PROJECT_ROOT"; \
    ...
    rm -f /tmp/askpass.sh; \
    c="$(git rev-parse HEAD)"; [ -f .capilot-built ] && [ "$(cat .capilot-built)" = "$c" ] || { \
      ...
      printf "%s" "$c" > .capilot-built; }' >&2

# 3. (remote) start capilot serve in the background, writing recipe JSON to a file; poll until it parses
recipe_json="$(vercel sandbox exec "$name" "${vercel_args[@]}" --timeout 60s \
  ...
  -- bash -lc 'set -euo pipefail; cd "$ORCA_PROJECT_ROOT"; rm -f /tmp/capilot-recipe.json /tmp/capilot-serve.log; \
    nohup pnpm exec capilot-dev serve --port "$ORCA_PORT" --project-root "$ORCA_PROJECT_ROOT" \
      --pairing-address "$ORCA_PAIRING_ADDRESS" --recipe-json >/tmp/capilot-recipe.json 2>/tmp/capilot-serve.log </dev/null & \
    pid=$!; for _ in $(seq 1 80); do \
      node -e "JSON.parse(require(\"node:fs\").readFileSync(\"/tmp/capilot-recipe.json\",\"utf8\"))" >/dev/null 2>&1 && { cat /tmp/capilot-recipe.json; exit 0; }; \
      kill -0 "$pid" 2>/dev/null || { cat /tmp/capilot-serve.log >&2; exit 1; }; sleep 0.25; \
    done; cat /tmp/capilot-serve.log >&2; echo "serve recipe JSON timed out" >&2; exit 1')"
...
`userData.resourceId` from stdin (§7d). This is the **CaPilot-server** connection mode (the recipe emits a
...
- **`create` does NOT run `capilot serve` and does NOT emit a `pairingCode`.** CaPilot itself connects to the
  host over its SSH relay, brings up the git + filesystem providers, and imports the repo. The script's
  only job is to make the host ready and **print SSH connection details** CaPilot will dial.
- The result uses a `connection` block with `type: "ssh"` and a `target`, **not** the flat
  `pairingCode`/`projectRoot` shape. Exact shape (CaPilot rejects anything else):
...
- `relayGracePeriodSeconds` (optional): how long CaPilot keeps the SSH relay alive after the workspace
...
# 1. ensure the repo is present and at the right commit on the host (NO capilot serve here)
...
# 2. print the SSH connection block (NO pairingCode, NO capilot serve). host/port/username tell CaPilot's
...
`destroy: none` and omit suspend/resume. (CaPilot still disconnects/reconnects its own SSH relay on
...
`connection.type:"ssh"` block above instead of starting `capilot serve`.
...
- If committing from an interactive shell, force the runtime entrypoint back to `sshd`:
  `docker commit --change='ENTRYPOINT ["/usr/local/bin/capilot-docker-ssh-entrypoint"]' …`.
...
require WSL/Git-Bash (and point `capilot.yaml` at e.g. `bash ./scripts/capilot-vm/<name>.sh` via a `.cmd`
...
# CaPilot-server mode: @{ schemaVersion=1; pairingCode=$pairingCode; projectRoot=$projectRoot; userData=@{...} }
...
environmentRecipes:
  - id: cloud-sandbox
    name: Cloud Sandbox
    create: ./scripts/capilot-vm/cloud-sandbox-create.sh
    suspend: ./scripts/capilot-vm/cloud-sandbox-suspend.sh
    resume: ./scripts/capilot-vm/cloud-sandbox-resume.sh
    destroy: ./scripts/capilot-vm/cloud-sandbox-destroy.sh
```
...
**CaPilot-server mode** — boot the env, start `capilot serve` in it, and print serve's result:

```json
{
  "schemaVersion": 1,
  "pairingCode": "capilot-pairing-code-or-url",
  "projectRoot": "/absolute/path/to/repo/on/remote",
  "userData": { "provider": "example", "resourceId": "provider-resource-id" }
}
```

Here `pairingCode` (from `capilot serve --recipe-json`) and `projectRoot` are required; `schemaVersion` (`1`)
and `userData` are optional.

**SSH mode** — do **not** run `capilot serve`; print the `connection.type:"ssh"` block instead (full shape +
...
Start CaPilot remotely with `capilot serve --port "$PORT" --project-root "$ABS_ROOT" --pairing-address
...
`capilot vm recipe doctor <recipe-id> --repo-path <repo> --json` validates **static wiring only** — it does
...
`capilot vm recipe doctor <recipe-id> --repo-path <repo> --provision --json` actually runs the recipe end
...
- Don't make CaPilot own provider lifecycle beyond invoking the configured scripts.
- Don't commit or create an CaPilot workspace unless asked.
