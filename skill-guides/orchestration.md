...
name: orchestration
description: >-
  Use CaPilot orchestration for structured multi-agent coordination: threaded
  messages, blocking ask/reply flows, task dispatch, worker_done/escalation
  waits, task DAGs, decision gates, coordinator loops, or decomposing work
  across agents. Use `capilot-cli` instead for full ownership handoffs, including
  ...
  supervise, monitor, wait for results, or coordinate a DAG. Use `capilot-cli` for
  ordinary terminal control, lightweight terminal prompts, shell commands, CaPilot
  worktree management, reading or waiting on terminals, and automation of the
  browser embedded inside CaPilot. Use Computer Use for browser windows, webviews,
  CaPilot app UI, or desktop UI outside CaPilot's embedded browser.
---

# CaPilot Inter-Agent Orchestration

Orchestration is CaPilot's structured coordination layer for agent messages, task ownership, dispatch state, and worker completion tracking.

Use this skill when coordination state matters. For lightweight terminal prompts or basic worktree/terminal/built-in-browser control, use `capilot-cli`.

## Tool Boundary

If a task says to use CaPilot orchestration, the coordinator must create or bind a Run, create the Task with `capilot orchestration task-create`, then attach the worker with either the preferred `capilot orchestration worker-start` composition or the low-level `capilot orchestration dispatch --inject` path.

Do not substitute non-CaPilot subagent tools, generic agent-spawn APIs, or chat-only parallel worker features. Those may create useful workers, but they do not create CaPilot task/dispatch provenance, injected lifecycle preambles, `worker_done` authority, or decision gates.

Before claiming a worker was orchestrated, verify the task/dispatch exists:

```bash
capilot orchestration task-list --json
capilot orchestration dispatch-show --task <task_id> --json
```

If the work was accidentally run outside CaPilot orchestration, say so plainly. To repair provenance, rerun or revalidate the needed work through a fresh CaPilot terminal plus injected dispatch; do not retroactively describe the external worker as orchestrated.
...
- `capilot status --json` should show a running runtime.
- `capilot` must be on PATH (`capilot-ide` on Linux).
- The orchestration experimental feature must be enabled in Settings > Experimental.
- `capilot orchestration` commands are RPC calls to the running CaPilot runtime.

## Contract Migration

CaPilot adopts a live pre-update orchestration assignment into an ordinary Run. Adoption preserves the existing agent process, PTY/session, terminal handle, tab/leaf/pane, worktree or folder workspace, Task, and Dispatch; it never restarts or replaces the worker. The retired scheduler is not revived, and a newly created attempt uses the current grammar.
...
```bash
capilot orchestration run-use --id <adopted_run_id> --takeover-legacy --json
capilot orchestration check --run <adopted_run_id> --json
```
...
New orchestration messages and tasks belong to one explicitly bound Run. A Run is only a durable namespace and coordinator inbox; it never schedules or places workers. Lifecycle authority comes from the active Dispatch, and terminal handles remain routing metadata rather than durable identity. Send `worker_done` and `heartbeat` from the worker's own terminal; CaPilot routes them to that Dispatch's Run.
...
- Do not use `capilot orchestration dispatch --inject` for full handoffs. It injects a coordinator preamble that tells the worker to send `worker_done`, heartbeat, and `ask` messages, then end its turn under the original terminal's dispatch lifecycle.
- Do not run `capilot orchestration task-create`, `capilot orchestration dispatch --inject`, or `capilot orchestration check --wait` for full handoffs. Do not peek at terminal output after prompt delivery to monitor progress.
...
```bash
capilot orchestration send --subject <text> [--to <run:id|dispatch:id|legacy_handle>] [--from <handle>] [--body <text>] [--type <type>] [--priority <level>] [--thread-id <id>] [--payload <json>] [--json]
capilot orchestration check [--terminal <handle>] [--ack <delivery_id>] [--peek|--all] [--types <type,...>] [--format] [--wait] [--timeout-ms <n>] [--json]
capilot orchestration reply --id <msg_id> --body <text> [--from <handle>] [--json]
capilot orchestration ask (--question <text>|--resume <msg_id>) [--options <csv>] [--timeout-ms <n>] [--from <handle>] [--json]
capilot orchestration inbox [--limit <n>] [--json]
```

Rules:

- Omit `--from` unless impersonating another terminal; CaPilot auto-resolves it from the current terminal.
...
- Use `dispatch:<id>` for coordinator guidance to one supervised worker. CaPilot routes that stable address locally or through the connected-server relay; do not substitute a remote terminal handle.
- Terminal handles remain appropriate for low-level pre-Dispatch messaging. Prefer `agentTerminalHandle` from the create response, fall back to `startupTerminal.handle` for older runtimes, then re-resolve with `capilot terminal list --worktree ... --json` if missing or stale. Continue with the replacement handle only; never dual-send to old and new handles.
- `capilot orchestration check --peek --format --json` returns locally formatted unread mail without consuming it; it never writes to terminal input or remotely wakes another terminal. Use `orchestration dispatch --inject` to deliver a tracked task, or `terminal send` when an existing agent needs a free-form prompt.
- While supervising workers manually, use `check --wait --types worker_done,escalation,question --timeout-ms <n>` instead of sleep/poll loops. Process the whole Delivery, reply to `question` messages with `capilot orchestration reply --id <msg_id> --body <answer> --json`, then acknowledge and keep waiting.
...
- `heartbeat` is also Dispatch-scoped. Include both IDs and omit `--to` so CaPilot uses the owning Run; use `status` for broad progress updates.
...
```bash
capilot orchestration run-create --objective <text> --json
capilot orchestration task-create --spec <text> [--deps <json_array>] [--parent <task_id>] [--json]
capilot orchestration task-list [--status <status>] [--ready] [--brief] [--json]
capilot orchestration task-update --id <task_id> --status <status> [--result <json>] [--json]
capilot orchestration dispatch --task <task_id> --to <handle> [--from <handle>] [--inject] [--json]
capilot orchestration dispatch-show --task <task_id> [--json]
```
...
- If the target is a bare shell, omit `--inject`, dispatch for tracking if needed, then send the prompt manually with `capilot terminal send --terminal <handle> --text <prompt> --enter --json`.
...
Use `worker-start` for the normal supervised path. It composes the existing worktree, terminal, readiness, and dispatch primitives while returning exact created/reused effects. Agents still choose placement and concurrency; CaPilot does not schedule workers or infer conflicts.

Create the Run and every independent Task first, then start all independent workers before waiting:

```bash
capilot orchestration run-create --objective "<objective>" --json
capilot orchestration task-create --spec "<worker A task>" --json
capilot orchestration task-create --spec "<worker B task>" --json
capilot orchestration worker-start --task <task_a> --worktree current --agent codex --json
capilot orchestration worker-start --task <task_b> --worktree current --agent claude --json
```
...
```bash
capilot orchestration worker-start --task <task_id> --worktree new-child --name <name> --agent codex --setup run --json
# Independent/top-level:
capilot orchestration worker-start --task <task_id> --worktree new-top-level --name <name> --agent codex --setup run --json
```
...
To run the worker on another connected CaPilot server, add `--on <saved-environment>`. The Run and Tasks remain authoritative on the current server; later commands route by Dispatch ID, so never repeat `--on`:

```bash
# Mac Run home -> Windows worker (the reverse is identical from a Windows Run home)
capilot orchestration worker-start --task <task_id> --on windows --worktree new-top-level --repo <exact_remote_repo_selector> --name <name> --agent codex --setup run --json
capilot orchestration worker-show --dispatch <dispatch_id> --json
capilot orchestration worker-read --dispatch <dispatch_id> --limit 50 --json
capilot orchestration send --to dispatch:<dispatch_id> --subject "Follow-up" --body "<attempt-specific guidance>" --json
```
...
`orchestration check` receives it even when the Dispatch is on another connected CaPilot server.

`worker-read` defaults to `--source auto`: CaPilot returns the exact hook-reported Codex, Claude, OpenClaude, or Grok transcript when it can prove the worker session, otherwise it returns bounded terminal output with `source: "terminal"` and a typed `fallbackReason`. Continue with the returned top-level `cursor`; it stays pinned to that exact source. If CaPilot reports `source_changed`, start a fresh read without the old cursor. Never supply or guess a provider session ID or transcript path.

Wait until every expected Dispatch settles, not for a fixed number of batches:

```bash
capilot orchestration check --wait --types worker_done,escalation,question --timeout-ms 900000 --json
# Process every message in the returned Delivery, then atomically ack and continue:
capilot orchestration check --ack <delivery_id> --wait --types worker_done,escalation,question --timeout-ms 900000 --json
```

Workers report exactly once using the IDs and capability injected by CaPilot; they do not supply Run/server/terminal identity:

```bash
capilot orchestration send --type worker_done --subject "<status>" --body "<what changed, findings, and what remains>" --task-id <task_id> --dispatch-id <dispatch_id> --outcome succeeded --files-modified "path/a,path/b" --json
...
```bash
capilot orchestration ask --question "<question>" --options "yes,no" --timeout-ms 600000 --json
capilot orchestration ask --resume <message_id> --timeout-ms 600000 --json
# Coordinator:
capilot orchestration reply --id <message_id> --body "<answer>" --json
```
...
```bash
capilot orchestration gate-create --task <task_id> --question <text> [--options <json_array>] [--json]
capilot orchestration gate-resolve --id <gate_id> --resolution <text> [--json]
capilot orchestration gate-list [--task <task_id>] [--status <status>] [--json]
```
...
Recovery only: `capilot orchestration reset --tasks|--messages|--all --json` clears the selected local orchestration database state. Do not run it during active coordination unless explicitly abandoning that state.
...
```bash
capilot worktree create --name <task-name> --no-parent --agent codex --prompt "<task brief>" --setup run --json
```

Before creating a new worktree from an active feature branch, decide and state whether the desired CaPilot lineage is child or top-level. Use child worktree lineage only when the new work is conceptually stacked under or dependent on the active worktree. For independent repo-wide fixes, standalone feature work, or unrelated follow-up tasks, create a top-level worktree with `--no-parent`.

Existing terminal handoff:

```bash
capilot terminal send --terminal <handle> --text "<task brief>" --enter --json
```

Custom Codex model/effort handoff:

`capilot worktree create --agent codex --prompt ...` launches the known Codex agent but does not accept Codex-specific `--model` or `-c model_reasoning_effort=...` arguments. When the user asks for a specific Codex model or effort, create the independent worktree first, launch Codex with the requested command in that worktree, wait only for TUI readiness if prompt delivery would otherwise race startup, send the prompt, and stop.
...
Use the exact full `<repo-id>::<path>` worktree id returned by `capilot worktree create --json`; a bare repo id cannot target the new worktree.

```bash
capilot worktree create --name <task-name> --no-parent --setup run --json
capilot terminal create --worktree id:<newFullWorktreeId> --title <task-name> --command 'codex --model gpt-5.5 -c model_reasoning_effort="xhigh"' --json
capilot terminal wait --terminal <handle> --for tui-idle --timeout-ms 60000 --json
capilot terminal send --terminal <handle> --text "<task brief>" --enter --json
```

Wait only for `tui-idle` when needed to avoid losing the prompt. Do not monitor task completion.

`--no-parent` only controls CaPilot lineage; it does not choose the Git base. If the work should start from the repo default base, omit `--base-branch` so CaPilot uses that default, or explicitly pass the repo default base (`origin/main`, `origin/master`, or the `capilot repo show --repo <selector> --json` value); never base it on the current feature branch unless the user explicitly asks for stacked work or "branch from current". Put current-branch context in the prompt instead.
...
```bash
capilot terminal create --worktree active --title <task-name> --command "codex" --json
capilot terminal wait --terminal <handle> --for tui-idle --timeout-ms 60000 --json
capilot orchestration dispatch --task <task_id> --to <handle> --inject --json
```
...
When a new worktree is allowed, use child lineage for isolated work that is stacked under or dependent on the active worktree, and use `--no-parent` when it is not stacked. Decide the Git base separately: `--no-parent` makes the worktree top-level in CaPilot, while omitted `--base-branch` uses the repo default base.
...
```bash
capilot worktree create --name <task-name> --agent codex --setup run --json
...
Use `capilot worktree create --prompt ...` or `capilot terminal send ...` for full handoffs or untracked/lightweight prompts. Those paths do not attach `taskId`/`dispatchId`; the worker should not send lifecycle messages unless the prompt supplies a live orchestration preamble.
...
```bash
capilot terminal list [--worktree <selector>] [--json]
capilot terminal create [--worktree <selector>] [--title <text>] [--command <cmd>] [--json]
capilot terminal split --terminal <handle> [--direction horizontal|vertical] [--command <cmd>] [--json]
capilot terminal wait --terminal <handle> --for tui-idle --timeout-ms <n> --json
capilot terminal read --terminal <handle> --json
capilot terminal send --terminal <handle> --text <text> --enter --json
```

If an older CLI rejects `worktree create --agent`, create the worktree normally, then run `capilot terminal create --worktree <selector> --command "codex" --json` or `--command "claude"`.
...
- Workers with a valid live preamble must send `worker_done` exactly once from their own terminal with an explicit `--outcome succeeded` or `--outcome failed`:
  `capilot orchestration send --type worker_done --subject "<short status>" --body "<3-sentence summary: what you did, what you found, what's left>" --task-id <task_id> --dispatch-id <dispatch_id> --outcome succeeded --files-modified "path/a" --report-path "<optional>" --json`
- A failed outcome is still a terminal report, but CaPilot records both the Dispatch and Task as failed. Never encode failure only in the subject/body.
- After sending `worker_done`, end your turn and idle at the agent prompt. Do not poll or keep calling `capilot orchestration check`; the coordinator re-engages you with a fresh preamble + TASK block delivered as new terminal input.
- For long tasks, send heartbeat/status only when the preamble asks for it, including both IDs:
  `capilot orchestration send --type heartbeat --subject "alive" --payload '{"taskId":"<task_id>","dispatchId":"<dispatch_id>","phase":"implementing"}' --json`
...
```bash
capilot terminal create --worktree active --title login-css-worker --command "claude" --json
capilot terminal wait --terminal <handle> --for tui-idle --timeout-ms 60000 --json
capilot orchestration task-create --spec "Fix the login button CSS" --json
capilot orchestration dispatch --task <task_id> --to <handle> --inject --json
capilot orchestration check --wait --types worker_done,escalation,question --timeout-ms 900000 --json
```
