#!/bin/bash
# Why: register the bundled `capilot-ide` CLI on PATH at package-install time.
# The in-app "Install CLI" action (CliInstaller) can never run on a headless
# server, so without this symlink `capilot serve` is unreachable from the shell on
...
# symlink works. We discover the install dir instead of hardcoding /opt/CaPilot
# because electron-builder's directory name can vary by productName sanitization.
set -e

link="/usr/bin/capilot-ide"

for dir in /opt/CaPilot /opt/capilot-ide /opt/capilot; do
  ...
  shim="$dir/resources/bin/capilot-ide"
  if [ -x "$shim" ]; then
    # Only manage our own symlink; never clobber an unrelated /usr/bin/capilot-ide.
