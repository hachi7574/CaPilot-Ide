...
const USER_DATA =
  process.env.ORCA_USER_DATA ?? `${process.env.HOME}/Library/Application Support/capilot-dev`
...
const devices = readJson<Array<{ token: string }>>(join(USER_DATA, 'capilot-devices.json'))
const token = devices[0]?.token
const keypair = readJson<{ publicKeyB64: string }>(join(USER_DATA, 'capilot-e2ee-keypair.json'))
