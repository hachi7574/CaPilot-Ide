...
function worktree(overrides: Partial<Worktree> = {}): Worktree {
  const worktreePath = join('/tmp', 'capilot', 'worktrees', 'feature')
  return {
    workspaceKind: 'git',
...
describe('buildSections', () => {
  ...
  it('renders empty repo sections from repo placeholders in repo grouping', () => {
    const sections = buildSections(
      [worktree({ repoId: 'repo-1', repo: 'capilot' })],
      'manual',
      { filterRepoIds: new Set(), hideSleeping: false, hideDefaultBranch: false },
      '',
      'repo',
    ...
    expect(withoutSectionListKeys(sections)).toEqual([
      {
        key: 'repo:repo-1',
        title: 'capilot',
        data: [worktree({ repoId: 'repo-1', repo: 'capilot' })]
      },
