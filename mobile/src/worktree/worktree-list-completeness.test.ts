...
// Same-named repos on different hosts (a local "capilot" and a runtime "capilot")
...
describe('every worktree stays visible regardless of repo.list contents', () => {
  it('shows worktrees whose repoId is missing from repo.list', () => {
    ...
    const worktrees = [
      worktree('capilot-local', 'a'),
      worktree('capilot-local', 'b'),
      worktree('capilot-runtime', 'c')
    ]
    const repoIdsByName = new Map([['capilot-runtime', 'capilot-runtime']])
  ...
  it('shows every worktree when the same repo name maps to two host-specific ids', () => {
    // The exact trigger: a local "capilot" and a runtime "capilot". repo.list returns
    // both under the same displayName; a name-keyed map keeps only the last id.
    const worktrees = [
      worktree('capilot-local', 'local-1', 'capilot'),
      worktree('capilot-local', 'local-2', 'capilot'),
      worktree('capilot-runtime', 'runtime-1', 'capilot')
    ]
    // Name-keyed map collapses "capilot" to a single id, as the host screen builds it.
    const repoIdsByName = new Map([['capilot', 'capilot-runtime']])
