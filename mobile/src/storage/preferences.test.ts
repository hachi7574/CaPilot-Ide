...
describe('session view preference', () => {
  ...
  it('defaults to terminal and persists the chat default', async () => {
    ...
    expect(AsyncStorage.getItem).toHaveBeenCalledWith('capilot:defaultSessionView')

    await saveDefaultSessionView('chat')
    expect(AsyncStorage.setItem).toHaveBeenCalledWith('capilot:defaultSessionView', 'chat')
  ...
  it('loads and updates per-tab overrides under a host-and-worktree scoped key', async () => {
    ...
    expect(AsyncStorage.getItem).toHaveBeenCalledWith(
      'capilot:nativeChatTabs:host%2Fone:folder%3AC%3A%5Crepo'
    )
  ...
  it('serializes default writes across callers and makes reads wait for the latest', async () => {
    ...
    expect(AsyncStorage.setItem).toHaveBeenNthCalledWith(1, 'capilot:defaultSessionView', 'chat')
    expect(AsyncStorage.setItem).toHaveBeenNthCalledWith(2, 'capilot:defaultSessionView', 'terminal')
  })
  ...
  it('does not globally block updates for a different host and worktree', async () => {
    ...
    expect(AsyncStorage.setItem).toHaveBeenCalledWith(
      'capilot:nativeChatTabs:other-host:worktree',
  ...
  it('repairs invalid preference data on the next user mutation', async () => {
    ...
    expect(AsyncStorage.setItem).toHaveBeenCalledWith(
      'capilot:nativeChatTabs:host:worktree',
...
describe('push notification preference', () => {
  ...
  it('persists the onboarding decision in the existing mobile toggle', async () => {
    ...
    expect(AsyncStorage.setItem).toHaveBeenCalledWith('capilot:pushNotificationsEnabled', 'true')

    await savePushNotificationsEnabled(false)
    expect(AsyncStorage.setItem).toHaveBeenCalledWith('capilot:pushNotificationsEnabled', 'false')
  })
})
...
describe('terminal autocomplete preference', () => {
  ...
  it('defaults to disabled when unset', async () => {
    ...
    expect(AsyncStorage.getItem).toHaveBeenCalledWith('capilot:terminalAutocompleteEnabled')
  })
  ...
  it('persists the selected value', async () => {
    ...
    expect(AsyncStorage.setItem).toHaveBeenCalledWith('capilot:terminalAutocompleteEnabled', 'true')

    await saveTerminalAutocompleteEnabled(false)

    expect(AsyncStorage.setItem).toHaveBeenCalledWith('capilot:terminalAutocompleteEnabled', 'false')
  })
})
...
describe('terminal live input disabled handles preference', () => {
  ...
  it('persists disabled handles per host and worktree', async () => {
    ...
    expect(AsyncStorage.setItem).toHaveBeenCalledWith(
      'capilot:terminalLiveInputDisabled:host%2Fone:folder%3AC%3A%5Crepo',
...
describe('terminal link open mode preference', () => {
  ...
  it('defaults to CaPilot browser when unset', async () => {
    vi.mocked(AsyncStorage.getItem).mockResolvedValue(null)

    await expect(loadTerminalLinkOpenMode()).resolves.toBe('capilot-browser')
    expect(AsyncStorage.getItem).toHaveBeenCalledWith('capilot:terminalLinkOpenMode')
  })
  ...
  it('falls back to CaPilot browser when storage cannot be read', async () => {
    vi.mocked(AsyncStorage.getItem).mockRejectedValue(new Error('storage unavailable'))
  ...
  it('persists the selected mode', async () => {
    ...
    expect(AsyncStorage.setItem).toHaveBeenCalledWith('capilot:terminalLinkOpenMode', 'phone-browser')
  })
})
