...
export function ProtocolBlockScreen({ verdict }: Props) {
  ...
  const title = isMobileTooOld ? 'Update CaPilot Mobile' : 'Update CaPilot on your computer'
  const body = isMobileTooOld
    ? `This desktop needs a newer CaPilot Mobile app. Update CaPilot Mobile from ${mobileUpdateTarget.storeName}, then try this host again.`
    : 'This paired desktop app is too old for your current CaPilot Mobile app. Update CaPilot on your computer, then try this host again.'
  const recoveryNote =
